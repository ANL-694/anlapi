...
export function resolveDocumentTitle(routeTitle: unknown, siteName?: string, titleKey?: string): string {
  const normalizedSiteName = typeof siteName === 'string' && siteName.trim() ? siteName.trim() : 'ANL Gateway'
...
export function resolveRouteDocumentTitle(
  ...
  const isCustomPage = route.name === 'CustomPage' || route.name === 'AdminCustomPage'
  const menuItem = isCustomPage && id
