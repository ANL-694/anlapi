...
const routes: RouteRecordRaw[] = [
  ...
  {
    path: '/admin/account-share-revenue',
    name: 'AdminAccountShareRevenue',
    component: () => import('@/views/admin/AccountShareRevenueView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: true,
      title: 'Account Sharing Revenue',
      titleKey: 'admin.accountShare.title',
      descriptionKey: 'admin.accountShare.description'
    }
  },
  {
    path: '/admin/plugins',
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (
    (to.meta.requiresPayment ||
      ...
      to.meta.requiresFreeModels ||
      to.meta.requiresCarpool) &&
  ...
  const externalPurchaseEnabled =
    ...
    Boolean(appStore.cachedPublicSettings.purchase_subscription_url?.trim())
  ...
  if (
    to.meta.requiresFreeModels &&
    appStore.publicSettingsLoaded &&
    appStore.cachedPublicSettings?.free_models_enabled === false
  ) {
    next(authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
  ...
  if (
    to.meta.requiresCarpool &&
    appStore.publicSettingsLoaded &&
    appStore.cachedPublicSettings?.carpool_enabled === false
  ) {
    next(authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
  ...
  // Subscription is opt-out: only an explicitly loaded false disables the route.
  if (
    to.meta.requiresSubscription &&
    appStore.publicSettingsLoaded &&
