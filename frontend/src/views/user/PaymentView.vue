<template>
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <ErrorState v-else-if="checkoutError" :retry="true" @retry="loadCheckout" />
      <template v-else>
        ...
        <template v-else>
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <!-- Plan list -->
            <template v-else>
              <div v-if="subscriptionPlanGroups.length === 0" class="card py-16 text-center">
                <Icon name="gift" size="xl" class="mx-auto mb-3 text-gray-300 dark:text-dark-600" />
                <p class="text-gray-500 dark:text-gray-400">{{ t('payment.noPlans') }}</p>
              </div>
              <div v-else class="space-y-5">
                <section
                  v-for="group in subscriptionPlanGroups"
                  :key="group.key"
                  class="space-y-3"
                ...
                >
                  <div class="flex flex-wrap items-center justify-between gap-2">
                    <div class="min-w-0">
                      <h3 :id="`${group.key}-title`" class="text-sm font-semibold text-gray-800 dark:text-gray-100">
                        {{ group.name }}
                      </h3>
                      <p v-if="group.platform" class="text-xs text-gray-400 dark:text-gray-500">
...
const availablePlans = computed(() => normalizeSubscriptionPlans(checkout.value.plans))
const subscriptionPlanGroups = computed(() => groupSubscriptionPlans(availablePlans.value))
...
const renewalPlans = computed(() => {
  ...
  return availablePlans.value.filter(p => p.group_id === renewGroupId.value)
})
...
async function resumeWechatPaymentFromQuery() {
  const resume = parseWechatResumeRoute(route.query, availablePlans.value, validAmount.value)
  if (!resume) {
    return
  }
  ...
  if (resume.orderType === 'subscription' && resume.planId) {
    selectedPlan.value = availablePlans.value.find(plan => plan.id === resume.planId) ?? null
  }
...
async function loadCheckout() {
  ...
  let res: Awaited<ReturnType<typeof paymentAPI.getCheckoutInfo>>
  try {
    res = await paymentAPI.getCheckoutInfo()
  } catch (err: unknown) {
  ...
  try {
    checkout.value = {
      ...res.data,
      plans: normalizeSubscriptionPlans(res.data.plans),
    }
    ...
    if (route.query.tab === 'subscription') {
      ...
      if (route.query.group) {
        ...
        const groupPlans = availablePlans.value.filter(p => p.group_id === groupId)
