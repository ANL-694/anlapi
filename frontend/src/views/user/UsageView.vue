<template>
  <AppLayout>
    <div class="space-y-6">
      <div v-if="statsLoading && !usageStats" class="flex min-h-32 items-center justify-center rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900">
        <div class="h-7 w-7 animate-spin rounded-full border-4 border-primary-500 border-t-transparent" role="status" :aria-label="t('common.loading')" />
      </div>
      <ErrorState v-else-if="statsError && !usageStats" :retry="true" @retry="loadStats" />
      <div v-else class="space-y-2">
        <ErrorState v-if="statsError" :retry="true" @retry="loadStats" />
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div class="space-y-2">
            <ErrorState v-if="modelStatsError" :retry="true" @retry="loadModelStats" />
          ...
          <div class="space-y-2">
            <ErrorState v-if="chartsError" :retry="true" @retry="loadChartData" />
      ...
      <template v-if="activeTab === 'usage'">
        <ErrorState v-if="logsError && usageLogs.length === 0" :retry="true" @retry="loadLogs" />
        <div v-else class="space-y-2">
          <ErrorState v-if="logsError" :retry="true" @retry="loadLogs" />
          <UsageTable
            :data="usageLogs"
            :loading="loading"
      ...
      <template v-else-if="errorViewEnabled">
        <ErrorState v-if="errorRequestsError && errorRows.length === 0" :retry="true" @retry="loadErrors" />
        <div v-else class="space-y-2">
          <ErrorState v-if="errorRequestsError" :retry="true" @retry="loadErrors" />
...
const errorRequestsError = ref(false)
...
const loadErrors = async () => {
  ...
  try {
    ...
    errorRequestsError.value = false
  } catch (error) {
    if (seq !== errorReqSeq) return
    console.error('[UsageView] loadErrors failed:', error)
    errorRequestsError.value = true
