...
const fetchHistory = async (page = 1) => {
  ...
  try {
    ...
    // Keep the view resilient to older/mock clients that return a plain list.
    const pageResult = Array.isArray(result)
      ? { items: result, total: result.length }
      : result
    history.value = Array.isArray(pageResult?.items) ? pageResult.items : []
    historyTotal.value = Number.isFinite(pageResult?.total)
      ? pageResult.total
