<template>
  <AppLayout>
        ...
        >
          ...
          <template #cell-key="{ value, row }">
            <div class="flex items-center gap-2">
              ...
              <button
                v-if="getRevealedKey(row)"
                @click="copyToClipboard(getRevealedKey(row), row.id)"
              ...
              <span
                v-else
                class="inline-flex rounded-lg p-1 text-gray-400 dark:text-dark-500"
                :title="t('keys.keyValueUnavailable')"
                :aria-label="t('keys.keyValueUnavailable')"
              >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <!-- Test API Key Model Button -->
              <button
                type="button"
                :aria-label="t('keys.testModel')"
                :title="t('keys.testModel')"
              ...
              >
                ...
                <span class="text-xs">{{ t('keys.testModel') }}</span>
              </button>
    ...
    <!-- The full key is available only from the current in-memory session. -->
    <BaseDialog
      :show="newlyCreatedKey !== null"
      :title="t('keys.keyCreatedRevealTitle')"
      width="normal"
      @close="closeNewlyCreatedKey"
    >
      <div v-if="newlyCreatedKey" class="space-y-4">
        <p class="text-sm text-amber-700 dark:text-amber-300">
          {{ t('keys.keyCreatedRevealWarning') }}
        </p>
        <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
          <div class="mb-1 text-xs text-gray-500 dark:text-gray-400">{{ newlyCreatedKey.name }}</div>
          <code class="block break-all font-mono text-sm text-gray-900 dark:text-gray-100">{{ newlyCreatedKey.key }}</code>
        </div>
        <p class="text-xs text-gray-500 dark:text-gray-400">
          {{ t('keys.keyCreatedRevealHint') }}
        </p>
        <div class="flex justify-end">
          <button type="button" class="btn btn-primary" @click="copyNewlyCreatedKey">
            <Icon name="clipboard" size="sm" class="mr-2" />
            {{ copiedKeyId === newlyCreatedKey.id ? t('keys.copied') : t('keys.copyToClipboard') }}
          </button>
        </div>
      </div>
    </BaseDialog>
...
const newlyCreatedKey = ref<{ id: number; name: string; key: string } | null>(null)
...
const copyToClipboard = async (text: string, keyId: number) => {
  if (!text || text.includes('...') || text.endsWith('***')) {
    appStore.showWarning(t('keys.keyValueUnavailable'))
    return
  }
...
const getRevealedKey = (key: ApiKey): string => revealedKeys.value[key.id] || ''

const closeNewlyCreatedKey = () => {
  newlyCreatedKey.value = null
}

const copyNewlyCreatedKey = async () => {
  if (!newlyCreatedKey.value) return
  await copyToClipboard(newlyCreatedKey.value.key, newlyCreatedKey.value.id)
}
...
const openUseKeyModal = (key: ApiKey) => {
  const revealedKey = getRevealedKey(key)
  if (!revealedKey) {
    appStore.showWarning(t('keys.keyValueUnavailable'))
    return
  }
  selectedKey.value = { ...key, key: revealedKey }
...
const handleSubmit = async () => {
  ...
  try {
    if (showEditModal.value && selectedKey.value) {
      ...
      if (createdKey.key && !createdKey.key.includes('...') && !createdKey.key.endsWith('***')) {
        revealedKeys.value[createdKey.id] = createdKey.key
        newlyCreatedKey.value = {
          id: createdKey.id,
          name: createdKey.name || formData.value.name,
          key: createdKey.key
        }
      }
...
const importToCcswitch = (row: ApiKey) => {
  const revealedKey = getRevealedKey(row)
  if (!revealedKey) {
    appStore.showWarning(t('keys.keyValueUnavailable'))
    return
  }
  const usableRow = { ...row, key: revealedKey }
  const platform = row.group?.platform || 'anthropic'

  // For antigravity platform, show client selection dialog
  if (platform === 'antigravity') {
    pendingCcsRow.value = usableRow
    showCcsClientSelect.value = true
    return
  }

  // For other platforms, execute directly
  executeCcsImport(usableRow, platform === 'gemini' ? 'gemini' : 'claude')
}
...
const executeCcsImport = (row: ApiKey, clientType: CcSwitchClientType) => {
  ...
  const providerName = (publicSettings.value?.site_name || 'ANL Gateway').trim() || 'ANL Gateway'
