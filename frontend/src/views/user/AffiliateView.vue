      ...
      <template v-else-if="detail">
        <section class="card p-5">
          <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('affiliate.period.title') }}</h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-dark-400">{{ t('affiliate.period.description') }}</p>
            </div>
            <div class="flex flex-wrap items-end gap-2">
              <div class="flex rounded-lg border border-gray-200 bg-white p-1 dark:border-dark-700 dark:bg-dark-800" role="group">
                <button v-for="option in periodOptions" :key="option.value" type="button" class="rounded-md px-3 py-1.5 text-sm font-medium" :class="periodPreset === option.value ? 'bg-primary-600 text-white' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700'" @click="selectPeriod(option.value)">
                  {{ option.label }}
                </button>
              </div>
              <label class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('affiliate.period.start') }}
                <input v-model="periodStartDate" type="date" class="input mt-1 h-9" @change="applyCustomPeriod" />
              </label>
              <label class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('affiliate.period.end') }}
                <input v-model="periodEndDate" type="date" class="input mt-1 h-9" @change="applyCustomPeriod" />
              </label>
            </div>
          </div>
        </section>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div class="card p-5">
            <p class="text-sm text-gray-500 dark:text-dark-400">{{ t('affiliate.stats.periodRebate') }}</p>
            <p class="mt-2 text-2xl font-semibold text-emerald-600 dark:text-emerald-400">
              {{ formatCurrency(detail.period_rebate) }}
            </p>
            <p class="mt-1 text-xs text-gray-400 dark:text-dark-500">{{ periodLabel }}</p>
          </div>
          <div class="card p-5">
            ...
            <table class="w-full min-w-[900px] text-left text-sm">
              <thead>
                <tr class="border-b border-gray-200 text-gray-500 dark:border-dark-700 dark:text-dark-400">
                  ...
                  <th class="px-3 py-2 font-medium text-right">{{ t('affiliate.invitees.columns.periodConsumption') }}</th>
                  <th class="px-3 py-2 font-medium text-right">{{ t('affiliate.invitees.columns.periodRebate') }}</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-3 py-3 text-right text-blue-600 dark:text-blue-400">{{ formatCurrency(item.period_rebate) }}</td>
...
const periodOptions = computed(() => [
  { value: 'today' as const, label: t('affiliate.period.today') },
  { value: 'last7' as const, label: t('affiliate.period.last7') },
])

const periodLabel = computed(() => {
  if (!detail.value?.period_start_at || !detail.value?.period_end_at) return ''
  return `${formatDateTime(detail.value.period_start_at)} - ${formatDateTime(detail.value.period_end_at)}`
})
...
function selectPeriod(preset: Exclude<PeriodPreset, 'custom'>): void {
...
function buildPeriodParams(): { period_start_at: string; period_end_at: string } {
  const start = parseDateInputStart(periodStartDate.value) ?? startOfLocalDay(new Date())
  const endStart = parseDateInputStart(periodEndDate.value) ?? start
  const end = addDays(endStart, 1)
  return { period_start_at: start.toISOString(), period_end_at: end.toISOString() }
}
