...
describe('PaymentView subscription plan grid', () => {
  it('shows a retryable error instead of an empty checkout when loading fails', async () => {
    routeState.path = '/purchase'
    routeState.query = {}
    getCheckoutInfo
      .mockReset()
    ...
    const wrapper = shallowMount(PaymentView, {
      global: {
        stubs: {
          ...
          ErrorState: {
            props: { retry: Boolean },
            template: '<button data-testid="checkout-error" @click="$emit(\'retry\')">retry</button>',
          },
          Teleport: true,
          Transition: false,
        },
    ...
    await wrapper.find('[data-testid="checkout-error"]').trigger('click')
    await flushPromises()
  ...
  it('groups valid plans and does not render duplicate, hidden, or malformed entries', async () => {
    vi.useRealTimers()
    routeState.path = '/purchase'
    routeState.query = { tab: 'subscription' }
    routerReplace.mockReset().mockResolvedValue(undefined)
    ...
    const basePlan = checkoutInfoWithPlansFixture().data.plans[0]
    const plans = [
      { ...basePlan, id: 1, group_id: 3, group_name: 'OpenAI', name: 'Monthly' },
      { ...basePlan, id: 8, group_id: 3, group_name: 'OpenAI', name: 'Monthly' },
      { ...basePlan, id: 2, group_id: 3, group_name: 'OpenAI', name: 'Hidden', for_sale: false },
      { ...basePlan, id: 3, group_id: 3, group_name: 'OpenAI', name: '', price: 10 },
      { ...basePlan, id: 4, group_id: 9, group_name: 'Anthropic', group_platform: 'anthropic', name: 'Annual', price: 90 },
    ]
    getCheckoutInfo.mockReset().mockResolvedValue(checkoutInfoFixture({ plans }))
    bridgeInvoke.mockReset()
    window.localStorage.clear()
    ...
    expect(cards.map(card => card.props('plan').id)).toEqual([1, 4])
