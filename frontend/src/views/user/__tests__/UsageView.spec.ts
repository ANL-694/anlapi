...
describe('user UsageView', () => {
  ...
  it('does not render zero-valued stat cards while the first stats request is pending', async () => {
    let resolveStats!: (value: Record<string, unknown>) => void
    getStats.mockReturnValueOnce(new Promise(resolve => { resolveStats = resolve }))

    const wrapper = mountUsageView()
    await flushPromises()

    expect((wrapper.vm as any).statsLoading).toBe(true)
    ...
    expect((wrapper.vm as any).statsLoading).toBe(false)
    expect((wrapper.vm as any).usageStats).not.toBeNull()
    wrapper.unmount()
  })

  it('keeps existing history visible and exposes a retry state when a refresh fails', async () => {
    const wrapper = mountUsageView()
    await flushPromises()

    expect((wrapper.vm as any).usageLogs).toHaveLength(1)
    query.mockRejectedValueOnce(new Error('history unavailable'))

    await (wrapper.vm as any).refreshData()
    await flushPromises()

    expect((wrapper.vm as any).logsError).toBe(true)
    expect((wrapper.vm as any).usageLogs).toHaveLength(1)
    expect(wrapper.findComponent(ErrorState).exists()).toBe(true)

    query.mockResolvedValueOnce({ items: [usageLog, { ...usageLog, id: 2 }], total: 2, pages: 1 })
    await (wrapper.vm as any).loadLogs()
    await flushPromises()

    expect((wrapper.vm as any).logsError).toBe(false)
    expect((wrapper.vm as any).usageLogs).toHaveLength(2)
    wrapper.unmount()
  })

  it('keeps chart data distinct from an error response instead of converting failure to empty success', async () => {
    getDashboardSnapshotV2.mockRejectedValueOnce(new Error('chart unavailable'))

    const wrapper = mountUsageView()
    await flushPromises()

    expect((wrapper.vm as any).chartsError).toBe(true)
    expect((wrapper.vm as any).trendData).toEqual([])
    expect((wrapper.vm as any).groupStats).toEqual([])

    getDashboardSnapshotV2.mockResolvedValueOnce({
      generated_at: '2026-03-08T00:00:00Z',
      start_date: '2026-03-08',
      end_date: '2026-03-08',
      granularity: 'hour',
      trend: [{ date: '00:00', input_tokens: 1, output_tokens: 2, cache_creation_tokens: 0, cache_read_tokens: 0 }],
      groups: [],
    })
    await (wrapper.vm as any).loadChartData()
    await flushPromises()

    expect((wrapper.vm as any).chartsError).toBe(false)
    expect((wrapper.vm as any).trendData).toHaveLength(1)
    wrapper.unmount()
  })

  it('ignores a stale error-request response after filters trigger a newer request', async () => {
    let resolveFirst!: (value: { items: unknown[]; total: number; page: number; page_size: number; pages: number }) => void
    let resolveSecond!: (value: { items: unknown[]; total: number; page: number; page_size: number; pages: number }) => void
    listMyErrorRequests
    ...
    const firstLoad = (wrapper.vm as any).loadErrors()
    const secondLoad = (wrapper.vm as any).loadErrors()
    resolveSecond({ items: [{ id: 2, model: 'new-model' }], total: 1, page: 1, page_size: 20, pages: 1 })
    await flushPromises()
    expect((wrapper.vm as any).errorRows).toEqual([{ id: 2, model: 'new-model' }])
    expect((wrapper.vm as any).errorLoading).toBe(false)

    resolveFirst({ items: [{ id: 1, model: 'stale-model' }], total: 1, page: 1, page_size: 20, pages: 1 })
    await firstLoad
    await secondLoad
    await flushPromises()
    ...
    expect((wrapper.vm as any).errorRequestsError).toBe(false)
    wrapper.unmount()
  })
