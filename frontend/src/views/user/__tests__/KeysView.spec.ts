...
const messages: Record<string, string> = {
  ...
  'keys.testModel': 'Test model',
  'keys.usage': 'Usage',
  'keys.keyValueUnavailable': 'The full API key is unavailable',
}
...
describe('user KeysView column settings', () => {
  ...
  it('does not test a key when only the masked value is available', async () => {
    const wrapper = await mountView()

    const testButton = wrapper.get('button[aria-label="Test model"]')
    await testButton.trigger('click')
    await nextTick()

    const testModal = wrapper.getComponent(ApiKeyTestModalStub)
    expect(testModal.props('show')).toBe(false)
    expect(showWarning).toHaveBeenCalledWith('The full API key is unavailable')
  })
})
