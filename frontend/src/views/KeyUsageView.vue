...
const subscriptionsEnabled = computed(() => resolveFeatureFlag(appStore.cachedPublicSettings, FeatureFlags.subscription))
...
let ringAnimationCancelled = false
  ...
  nextTick(() => {
    ...
    const startFrame = (callback: (time: number) => void) => {
      if (typeof requestAnimationFrame === 'function') {
        ...
        ringAnimationTimer = setTimeout(() => callback(performance.now()), 16) as unknown as ReturnType<typeof setTimeout>
      }
    }
    startFrame(() => {
      if (ringAnimationCancelled) return
      ringAnimationTimer = setTimeout(() => {
        ...
        function tick() {
          ...
          if (p < 1 && !ringAnimationCancelled) startFrame(tick)
        }
        if (!ringAnimationCancelled) startFrame(tick)
      }, 50)
    })
  })
}
...
function cancelRingAnimation() {
  ringAnimationCancelled = true
  if (ringAnimationTimer) {
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    ...
    rows.push({
      ...
      label: t(subscriptionsEnabled.value ? 'keyUsage.subscriptionType' : 'keyUsage.billingType'), value: data.planName || t('keyUsage.walletBalance'), valueClass: '',
    })
