<template>
  <AppLayout>
      ...
      <template #filters>
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            <div v-if="selectedSubscriptions.length" class="flex flex-wrap items-center gap-2" data-test="bulk-actions">
              <button data-test="bulk-extend" type="button" class="btn btn-secondary btn-sm" :disabled="eligibleBulk('extend').length === 0" @click="openBulkAction('extend')">{{ t('admin.subscriptions.bulk.extend') }}</button>
              <button data-test="bulk-reset_quota" type="button" class="btn btn-secondary btn-sm" :disabled="eligibleBulk('reset_quota').length === 0" @click="openBulkAction('reset_quota')">{{ t('admin.subscriptions.bulk.reset') }}</button>
              <button data-test="bulk-revoke" type="button" class="btn btn-secondary btn-sm" :disabled="eligibleBulk('revoke').length === 0" @click="openBulkAction('revoke')">{{ t('admin.subscriptions.revoke') }}</button>
              <button data-test="bulk-restore" type="button" class="btn btn-secondary btn-sm" :disabled="eligibleBulk('restore').length === 0" @click="openBulkAction('restore')">{{ t('admin.subscriptions.restore') }}</button>
            </div>
            <button
    ...
    <BulkSubscriptionActionDialog
      :show="showBulkActionDialog"
      :action="bulkActionType"
      :subscriptions="bulkActionTargets"
    ...
    >
      ...
      >
        <div>
          <label class="input-label">{{ t('admin.subscriptions.form.user') }}</label>
            <label class="mb-2 flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
              <input v-model="assignMultiple" type="checkbox" class="h-4 w-4" />
              {{ t('admin.subscriptions.bulk.assignMultiple') }}
            </label>
            ...
            >
              ...
              <div v-if="assignMultiple && selectedAssignUsers.length && !assignCompleted" data-test="assign-users" class="mt-2 flex flex-wrap gap-2">
                <span v-for="user in selectedAssignUsers" :key="user.id" class="rounded bg-primary-50 px-2 py-1 text-xs text-primary-700 dark:bg-primary-900/30 dark:text-primary-300">{{ user.email }}</span>
              </div>
            <!-- User Dropdown -->
            <div
        ...
        <div v-if="assignResultErrors.length" data-test="batch-assign-result" class="mt-3 rounded bg-amber-50 p-3 text-sm text-amber-800">{{ assignResultErrors.join('; ') }}</div>
      </form>
      <template #footer>
...
import type { UserSubscription, Group, GroupPlatform, SubscriptionType, BulkAssignSubscriptionRequest } from '@/types'
...
const bulkActionTargets = ref<UserSubscription[]>([])
...
const assignMultiple = ref(false)
const selectedAssignUsers = ref<SimpleUser[]>([])
const assignResultErrors = ref<string[]>([])
const assignCompleted = ref(false)
...
const selectedSubscriptions = computed(() => {
  const visible = new Map(subscriptions.value.map(subscription => [subscription.id, subscription]))
  return selectedKeys.value
    .map(key => visible.get(Number(key)))
    .filter((subscription): subscription is UserSubscription => Boolean(subscription))
})

const eligibleBulk = (action: SubscriptionBulkAction) => selectedSubscriptions.value.filter(subscription => {
  if (action === 'extend') return subscription.status === 'active' || subscription.status === 'expired'
  if (action === 'reset_quota' || action === 'revoke') return subscription.status === 'active'
...
const handleSelectedKeys = (keys: Array<string | number>) => {
  ...
  selectedKeys.value = [...new Set(keys.map(Number))].filter(id => visibleIds.has(id))
}

const openBulkAction = (action: SubscriptionBulkAction) => {
  const targets = eligibleBulk(action)
  if (!targets.length) return
  bulkActionType.value = action
  bulkActionTargets.value = targets
  showBulkActionDialog.value = true
}

const handleBulkActionCompleted = (result: SubscriptionBulkActionResult) => {
  const succeeded = new Set(result.results.filter(item => item.success).map(item => item.subscription_id))
  selectedKeys.value = selectedKeys.value.filter(key => !succeeded.has(Number(key)))
  showBulkActionDialog.value = true
  loadSubscriptions()
}
...
const searchUsers = async () => {
  ...
  if (!assignMultiple.value && selectedUser.value && keyword !== selectedUser.value.email) {
  ...
  try {
    const userList = (adminAPI.users as any)?.list
    const response = userList
      ? await userList({ search: keyword, page: 1, page_size: 20 })
      : await (adminAPI.usage as any)?.searchUsers?.(keyword)
    userSearchResults.value = Array.isArray(response) ? response : (response?.items ?? [])
  } catch (error) {
    console.error('Failed to search users:', error)
    userSearchResults.value = []
  } finally {
...
const selectUser = (user: SimpleUser) => {
  if (assignMultiple.value) {
    if (!selectedAssignUsers.value.some(item => item.id === user.id)) {
...
const closeAssignModal = () => {
  ...
  assignMultiple.value = false
  selectedAssignUsers.value = []
  assignResultErrors.value = []
  assignCompleted.value = false
...
const handleAssignSubscription = async () => {
  const userIds = assignMultiple.value
    ? selectedAssignUsers.value.map(user => user.id)
  ...
  try {
    if (assignMultiple.value) {
      ...
      const result = await adminAPI.subscriptions.bulkAssign(request)
      assignResultErrors.value = result.errors ?? []
      const succeeded = new Set((result.subscriptions ?? []).map(subscription => subscription.user_id))
      selectedAssignUsers.value = selectedAssignUsers.value.filter(user => !succeeded.has(user.id))
      if (result.failed_count === 0 || selectedAssignUsers.value.length === 0) {
        selectedAssignUsers.value = []
        assignMultiple.value = false
        assignCompleted.value = true
