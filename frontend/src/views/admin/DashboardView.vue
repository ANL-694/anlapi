<template>
  <AppLayout>
    <UiPage width="wide" density="compact" class="dashboard-page dashboard-page--admin">
      <!-- Loading State -->
      ...
      <template v-else-if="stats">
        <div v-if="snapshotError" class="mb-4">
          <ErrorState :retry="true" @retry="loadDashboardStats" />
        </div>
        <!-- Row 1: Core Stats -->
        ...
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div v-if="snapshotError" class="card p-4">
              <ErrorState :retry="true" @retry="loadDashboardStats" />
            </div>
            ...
            <TokenUsageTrend v-else :trend-data="trendData" :loading="chartsLoading" />
          </div>
          ...
          <div class="card p-4">
            ...
            <div class="h-64">
              ...
              <ErrorState v-else-if="userTrendError" :retry="true" @retry="loadUsersTrend" />
      ...
      <ErrorState v-else :retry="true" @retry="loadDashboardStats" />
    </UiPage>
  </AppLayout>
</template>
...
const rankingTotalActualCost = ref<number | null>(null)
const rankingTotalRequests = ref<number | null>(null)
const rankingTotalTokens = ref<number | null>(null)
let chartLoadSeq = 0
...
const formatNumber = (value: number | null | undefined): string => {
  ...
  return safeValue === null ? '--' : safeValue.toLocaleString()
}
...
const loadUserSpendingRanking = async () => {
  ...
  try {
    ...
    rankingTotalActualCost.value = typeof response.total_actual_cost === 'number' ? response.total_actual_cost : null
    rankingTotalRequests.value = typeof response.total_requests === 'number' ? response.total_requests : null
    rankingTotalTokens.value = typeof response.total_tokens === 'number' ? response.total_tokens : null
  } catch (error) {
    if (currentSeq !== rankingLoadSeq) return
