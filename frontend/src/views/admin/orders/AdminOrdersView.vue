      ...
      <form id="manual-fulfill-form" class="space-y-4" @submit.prevent="handleManualFulfill">
        <div v-if="manualFulfillTarget" class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-800">
          ...
          <div class="mt-2 flex items-center justify-between gap-3">
            ...
            <span class="font-medium text-gray-900 dark:text-white">{{ paymentAmountSymbol(manualFulfillTarget) }}{{ manualFulfillTarget.pay_amount.toFixed(2) }}</span>
          </div>
          <div class="mt-2 flex items-center justify-between gap-3">
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('payment.admin.manualPaidAmount') }}</label>
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('payment.admin.manualTradeNo') }}</label>
          <input v-model.trim="manualFulfillForm.trade_no" type="text" maxlength="128" class="input" />
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('payment.admin.manualFulfillReason') }}</label>
