...
describe('admin AccountsView bulk edit scope', () => {
  beforeEach(() => {
    ...
    getQuotaDashboard.mockResolvedValue({ platform: [], mine: [], errors: [] })
  ...
  it('opens bulk edit in filtered-results mode from the bulk actions dropdown', async () => {
    ...
    await flushPromises()
    expect(getQuotaDashboard).toHaveBeenCalledTimes(1)
  ...
  it('uses the backend default when the probe settings response is empty', async () => {
    listAccounts.mockResolvedValue({
      items: [
        {
          id: 1,
    ...
    getUpstreamBillingProbeSettings.mockResolvedValue(undefined)
    ...
    expect(wrapper.get('[data-test="upstream-billing-cell"]').attributes('data-global-enabled')).toBe('true')
  })
