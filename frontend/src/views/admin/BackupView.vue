      ...
      <div class="card p-6">
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            <input v-model.trim="imageStorageForm.bucket" class="input w-full" :placeholder="imageStorageForm.reuse_backup_s3 ? t('admin.backup.imageStorage.bucketInherited') : ''" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.imageStorage.prefix') }}</label>
            <input v-model.trim="imageStorageForm.prefix" class="input w-full" placeholder="images/" />
          </div>

          <template v-if="!imageStorageForm.reuse_backup_s3">
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.endpoint') }}</label>
              <input v-model.trim="imageStorageForm.endpoint" class="input w-full" placeholder="https://&lt;account_id&gt;.r2.cloudflarestorage.com" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.region') }}</label>
              <input v-model.trim="imageStorageForm.region" class="input w-full" placeholder="auto" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.accessKeyId') }}</label>
              <input v-model.trim="imageStorageForm.access_key_id" class="input w-full" autocomplete="off" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.secretAccessKey') }}</label>
          ...
          <div>
            ...
            <input v-model.trim="imageStorageForm.public_base_url" class="input w-full" :placeholder="t('admin.backup.imageStorage.publicBaseUrlPlaceholder')" />
          </div>
          <div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          <button type="button" class="btn btn-secondary btn-sm" :disabled="testingImageStorage || savingImageStorage" @click="testImageStorage">
            {{ testingImageStorage ? t('common.loading') : t('admin.backup.s3.testConnection') }}
          </button>
          <button type="button" class="btn btn-primary btn-sm" :disabled="savingImageStorage || testingImageStorage" @click="saveImageStorageConfig">
            {{ savingImageStorage ? t('common.saving') : t('common.save') }}
          </button>
        </div>
      </div>
...
import type { BackupS3Config, BackupScheduleConfig, BackupRecord, ImageStorageConfig, UsageRetentionConfig } from '@/api/admin/backup'
