...
import { CreateAccountModal, EditAccountModal, BulkEditAccountModal, SyncFromCrsModal, TempUnschedStatusModal, AccountQuotaDashboard } from '@/components/account'
...
import type { Account, AccountListItem, AccountPlatform, AccountSchedulerGroupScore, AccountType, AccountUsageInfo, Proxy as AccountProxy, AdminGroup, WindowStats, ClaudeModel, UpstreamBillingProbeSnapshot, AccountQuotaDashboard as AccountQuotaDashboardData } from '@/types'
...
const loadQuotaDashboard = async () => {
  quotaDashboardLoading.value = true
  quotaDashboardError.value = false
  if (typeof adminAPI.accounts.getQuotaDashboard !== 'function') {
    quotaDashboardLoading.value = false
    return
  }
  try {
    quotaDashboard.value = await adminAPI.accounts.getQuotaDashboard()
  } catch (error) {
...
const handleManualRefresh = async () => {
  await Promise.all([load(), loadQuotaDashboard(), loadUpstreamBillingProbeGlobalState()])
...
const handleRefresh = async (a: Account) => {
  try {
    ...
    patchAccountInList(updated.account)
    if (updated.warning || updated.message) {
      appStore.showWarning(updated.message || t('admin.accounts.failedToRefresh'))
    }
    enterAutoRefreshSilentWindow()
  } catch (error) {
