    ...
    <div class="app-header-inner">
      ...
      <div class="app-header-leading">
        ...
        <div class="app-header-context">
          <div class="app-header-title-row">
            <span
              class="app-header-workspace-badge"
              :class="{ 'app-header-workspace-admin': isAdminWorkspace }"
              data-testid="header-workspace-badge"
            >
              <Icon :name="isAdminWorkspace ? 'cog' : 'user'" size="xs" />
              {{ workspaceLabel }}
            </span>
        ...
        <div
          v-if="authStore.isAdmin && !authStore.isSimpleMode"
          class="app-header-workspace-switch hidden lg:grid"
          role="tablist"
          :aria-label="t('nav.workspace')"
          data-testid="header-workspace-switch"
        >
          <button
            type="button"
            role="tab"
            data-workspace="user"
            :aria-label="t('nav.userWorkspace')"
            :aria-selected="!isAdminWorkspace"
            :class="{ 'app-header-workspace-active': !isAdminWorkspace }"
            @click="switchWorkspace('user')"
          >
            <Icon name="user" size="xs" />
            <span>{{ t('nav.userWorkspace') }}</span>
          </button>
          <button
            type="button"
            role="tab"
            data-workspace="admin"
            :aria-label="t('nav.adminWorkspace')"
            :aria-selected="isAdminWorkspace"
            :class="{ 'app-header-workspace-active': isAdminWorkspace }"
      ...
      <div class="app-header-actions">
        ...
        <a
          ...
          class="app-header-action hidden sm:inline-flex"
        >
        ...
        <div
          v-if="user"
          class="app-header-balance group relative hidden sm:flex"
        >
          <svg
...
const workspaceLabel = computed(() => t(isAdminWorkspace.value ? 'nav.adminWorkspace' : 'nav.userWorkspace'))
...
const pageTitle = computed(() => {
  ...
  if (route.name === 'CustomPage' || route.name === 'AdminCustomPage') {
    const id = route.params.id as string
    const publicItems = appStore.cachedPublicSettings?.custom_menu_items ?? []
    const menuItem = isAdminWorkspace.value
...
function switchWorkspace(target: 'user' | 'admin') {
  const destination = target === 'admin' ? '/admin/dashboard' : '/dashboard'
...
.app-header-avatar,
.app-header-workspace-switch button {
  display: flex;
  align-items: center;
}
...
.app-header-workspace-switch button {
  ...
  gap: 0.3125rem;
  padding: 0 0.5rem;
  border-radius: calc(var(--ui-radius-md) - 0.125rem);
...
.app-header-workspace-switch button:hover {
  color: var(--app-text);
}

.app-header-workspace-switch .app-header-workspace-active {
