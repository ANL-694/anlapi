<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      >
        <img :src="siteLogo || DEFAULT_FAVICON" alt="ANL Gateway" class="h-full w-full object-contain" />
      </router-link>
    ...
    <div
      v-if="isAdmin && !authStore.isSimpleMode"
    ...
    >
      <button
        ...
        :title="sidebarCollapsed ? t('nav.userWorkspace') : undefined"
      ...
      <button
        ...
        :aria-selected="isAdminWorkspace"
        :title="sidebarCollapsed ? t('nav.adminWorkspace') : undefined"
        :class="{ 'sidebar-workspace-active': isAdminWorkspace }"
...
import { DEFAULT_FAVICON } from '@/utils/branding'
...
const workspace = computed(() => resolveAppWorkspace(route))
...
const homePath = computed(() => (isAdminWorkspace.value ? '/admin/dashboard' : '/dashboard'))

// Track which parent nav groups are expanded
const expandedGroups = ref<Set<string>>(new Set())
const collapsedGroups = ref<Set<string>>(new Set())
// Official 0.2.1 keeps an explicit per-group override concept; ANL retains
// its Set-based state for backwards-compatible workspace behavior.
  ...
  items.push(
    { path: '/keys', label: t('nav.apiKeys'), icon: KeyIcon },
    {
      ...
      children: [
        ...
        { path: '/accounts/free-models', label: t('nav.freeModels'), icon: GiftIcon, hideInSimpleMode: true, featureFlag: flagFreeModels },
    ...
    { path: '/usage', label: t('nav.usage'), icon: ChartIcon, hideInSimpleMode: true },
    {
      ...
      expandOnly: true,
      children: [
        { path: '/models', label: t('nav.modelMarket'), icon: FolderIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
    ...
    { path: '/orders', label: t('nav.myOrders'), icon: OrderListIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    {
      ...
      children: [
        ...
        { path: '/orders', label: t('nav.myOrders'), icon: OrderIcon, hideInSimpleMode: true, featureFlag: flagPayment },
...
function pruneNavItems(items: NavItem[], hideSimpleMode: boolean): NavItem[] {
  ...
  for (const item of items) {
    ...
    if (item.children) {
      const children = pruneNavItems(item.children, hideSimpleMode)
...
// Shared visibility projection used by lightweight integrations and user-workspace rendering.
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/account-share-revenue', label: t('nav.accountShareRevenue'), icon: ChartIcon, hideInSimpleMode: true },
    { path: '/admin/withdrawals', label: t('nav.withdrawalManagement'), icon: CreditCardIcon, hideInSimpleMode: true },
    { path: '/admin/modules', label: t('nav.modules'), icon: CogIcon, hideInSimpleMode: true },
  ...
  if (authStore.isSimpleMode) {
    ...
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push({ path: `/admin/custom/${cm.id}`, label: cm.label, icon: null, iconSvg: cm.icon_svg, url: cm.url, openInNewWindow: cm.open_in_new_window })
    }
    return filtered
  }
  ...
  for (const cm of customMenuItemsForAdmin.value) {
    visible.push({ path: `/admin/custom/${cm.id}`, label: cm.label, icon: null, iconSvg: cm.icon_svg, url: cm.url, openInNewWindow: cm.open_in_new_window })
  }
  return visible
})
...
function buildNewWindowUrl(item: NavItem): string {
  ...
  return buildEmbeddedUrl(item.url, authStore.user?.id, authStore.token, detectTheme(), 'zh-CN')
}
...
function isActive(path: string): boolean {
  if (path === '/accounts') return route.path === path
...
function isGroupExpanded(item: NavItem): boolean {
  ...
  const explicitlyExpanded = expandedGroups.value.has(item.path)
  return explicitlyExpanded ? true : isGroupActive(item)
}

function toggleGroup(item: NavItem) {
  const wasExpanded = isGroupExpanded(item)
  groupExpandOverrides.value.set(item.path, !wasExpanded)
  if (wasExpanded) {
...
function handleGroupClick(item: NavItem) {
  if (sidebarCollapsed.value) {
    ...
    collapsedGroups.value.delete(item.path)
    expandedGroups.value.add(item.path)
    if (!item.expandOnly && route.path !== item.path) void router.push(item.path)
    return
  }
  if (item.expandOnly) {
    toggleGroup(item)
