    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div class="grid grid-cols-1 gap-2 sm:min-w-[18rem] sm:grid-cols-2">
        <div class="rounded-lg bg-primary-50 px-4 py-3 dark:bg-primary-900/20">
          <p class="text-xs text-primary-600 dark:text-primary-300">{{ t('withdrawal.availableBalance') }}</p>
        ...
        <div class="rounded-lg bg-gray-50 px-4 py-3 dark:bg-dark-800/70">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('withdrawal.status') }}</p>
          <p class="mt-1 text-sm font-semibold" :class="hasPendingWithdrawal ? 'text-yellow-700 dark:text-yellow-300' : 'text-green-700 dark:text-green-300'">
            {{ hasPendingWithdrawal ? t('withdrawal.pending') : t('withdrawal.canSubmit') }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="mt-5 grid min-w-0 items-start gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1fr)_minmax(0,1.05fr)]">
      <div class="min-w-0 rounded-lg border border-gray-100 bg-gray-50/70 p-4 dark:border-dark-700 dark:bg-dark-900/30">
        <div class="flex items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('withdrawal.pendingHint') }}</p>
          </div>
        ...
        <div class="mt-4">
          <p class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('withdrawal.receiptCode') }}</p>
        ...
        <div class="mt-4 rounded-lg border border-gray-100 bg-white p-3 text-sm dark:border-dark-700 dark:bg-dark-900/60">
          ...
          <div class="mt-2 flex justify-between gap-3 border-t border-gray-100 pt-2 dark:border-dark-700">
            <span class="text-gray-500 dark:text-gray-400">{{ t('withdrawal.totalDeducted') }}</span>
        ...
        <button
          ...
          @click="submit"
        >
          {{ submitting ? t('common.processing') : t('withdrawal.submit') }}
        </button>
      </div>
      ...
      <div class="min-w-0 rounded-lg border border-gray-100 bg-gray-50/70 p-4 dark:border-dark-700 dark:bg-dark-900/30">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('withdrawal.receiptManagement') }}</p>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('withdrawal.receiptHint') }}</p>
          </div>
        ...
        >
          ...
          <div v-else class="flex flex-col items-center gap-2 text-gray-400 dark:text-gray-500">
            ...
            <span class="text-sm">{{ t('withdrawal.notUploaded', { method: methodLabel(selectedMethod) }) }}</span>
          </div>
        </div>

        <div class="mt-4 min-h-12 text-sm text-gray-600 dark:text-gray-300">
          <p>
            {{ currentReceiptCode ? t('withdrawal.savedAt', { date: formatDateTime(currentReceiptCode.updated_at) }) : t('withdrawal.notSaved') }}
          </p>
          <p v-if="draftFile" class="mt-1 text-xs text-primary-600 dark:text-primary-300">
            {{ t('withdrawal.draftHint') }}
          </p>
        ...
        <div class="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          >
            ...
            {{ draftFile ? t('withdrawal.clear') : t('withdrawal.receiptDelete') }}
          </button>
        </div>
      </div>
      ...
      <div class="min-w-0 rounded-lg border border-gray-100 bg-gray-50/70 p-4 dark:border-dark-700 dark:bg-dark-900/30">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('withdrawal.history') }}</p>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('withdrawal.historyHint') }}</p>
          </div>
          <span class="rounded-full bg-white px-2.5 py-1 text-xs text-gray-500 ring-1 ring-gray-100 dark:bg-dark-900 dark:text-gray-400 dark:ring-dark-700">
            {{ t('withdrawal.recent', { count: withdrawals.length }) }}
          </span>
        </div>
        ...
        <div v-if="withdrawals.length" class="mt-4 max-h-[22rem] space-y-3 overflow-y-auto pr-1">
          ...
          >
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('withdrawal.totalDeducted') }}</p>
...
const submitHint = computed(() => {
  if (hasPendingWithdrawal.value) return t('withdrawal.submitPending')
  if (draftFile.value) return t('withdrawal.saveDraftFirst')
  if (!currentReceiptCode.value) return t('withdrawal.uploadFirst')
  if (amountRawText.value && !amountIsValid.value) return t('withdrawal.invalidAmount')
  if (amountIsValid.value && balance.value + 1e-9 < totalDeducted.value) return t('withdrawal.insufficient')
  return ''
})
...
function handleFileChange(event: Event) {
  ...
  if (!['image/png', 'image/jpeg', 'image/gif', 'image/webp'].includes(file.type)) {
    appStore.showError(t('withdrawal.invalidType'))
    return
  }
  if (file.size > 1024 * 1024) {
    appStore.showError(t('withdrawal.tooLarge'))
    return
  }
  revokeDraftPreview()
...
async function handleSave() {
  if (!draftFile.value) {
    appStore.showError(t('withdrawal.selectFile'))
    return
  }
  ...
  try {
    ...
    clearDraft()
    appStore.showSuccess(t('withdrawal.saved'))
  } catch (error: unknown) {
    appStore.showError(extractApiErrorMessage(error, t('withdrawal.saveFailed')))
  } finally {
    saving.value = false
  }
}
...
async function handleDelete() {
  ...
  try {
    ...
    appStore.showSuccess(t('withdrawal.deleted'))
  } catch (error: unknown) {
    appStore.showError(extractApiErrorMessage(error, t('withdrawal.deleteFailed')))
  } finally {
    saving.value = false
  }
}
...
async function submit() {
  if (!canSubmit.value) {
    appStore.showError(submitHint.value || t('withdrawal.confirmInput'))
    return
  }
  ...
  try {
    ...
    appStore.showSuccess(t('withdrawal.submitted'))
  } catch (error: unknown) {
    appStore.showError(extractApiErrorMessage(error, t('withdrawal.submitFailed')))
  } finally {
...
async function cancel(id: number) {
  ...
  try {
    ...
    await Promise.all([load(), authStore.refreshUser()])
    appStore.showSuccess(t('withdrawal.cancelled'))
  } catch (error: unknown) {
