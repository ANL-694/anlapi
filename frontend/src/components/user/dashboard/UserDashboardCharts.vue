  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <div class="card relative overflow-hidden p-4">
        ...
        <ErrorState v-if="error" :retry="true" @retry="$emit('retry')" />
      ...
      <div v-if="error" class="card p-4">
        <ErrorState :retry="true" @retry="$emit('retry')" />
      </div>
      <TokenUsageTrend v-else :trend-data="trend" :loading="loading" />
    </div>
  </div>
</template>
...
defineEmits(['update:startDate', 'update:endDate', 'update:granularity', 'dateRangeChange', 'granularityChange', 'refresh', 'retry'])
const { t } = useI18n()
