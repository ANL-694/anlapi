...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'revert-proxy-fallback', 'create-spark-shadow'])
const { t } = useI18n()
...
const hasProxyFallback = computed(() => !isShadow.value && Boolean(props.account?.proxy_fallback_origin_id))
