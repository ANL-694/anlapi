<template>
  <Teleport to="body">
    <div v-if="show && effectiveAnchorRect">
...
const props = defineProps<{
  ...
  /** Backward-compatible position prop used by the user account view. */
...
const effectiveAnchorRect = computed(() => {
...
const updatePosition = () => {
  if (!menuRef.value || !effectiveAnchorRect.value) return

  if (!props.anchorRect && props.position) {
    menuPosition.value.top = props.position.top
    menuPosition.value.left = props.position.left
    return
  }

  const { width, height } = menuRef.value.getBoundingClientRect()
  const anchor = effectiveAnchorRect.value
  const gap = 4
...
watch([menuRef, effectiveAnchorRect, viewportWidth, viewportHeight], updatePosition, { flush: 'post' })
