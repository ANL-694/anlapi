...
function showSuccess(message: string) {
  ...
  if (pinia) useAppStore(pinia).showSuccess(message)
}

function showError(message: string) {
  const pinia = getActivePinia()
  if (pinia) useAppStore(pinia).showError(message)
}
...
async function handleSend() {
  ...
  try {
    ...
    showSuccess(t('admin.emailBroadcast.notifications.sendQueued', { id: broadcast.id }))
...
async function confirmDeleteHistory() {
  ...
  try {
    ...
    showSuccess(t('admin.emailBroadcast.notifications.deleteSuccess'))
  } catch (err: any) {
    const msg = err?.response?.data?.message || err?.message || t('common.unknownError')
    showError(msg)
  } finally {
