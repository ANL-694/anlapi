...
const hasTextOrChatModality = (modalities: string[]): boolean => {
  return modalities.some((modality) => /(^|[_-])(text|chat|language)([_-]|$)/.test(modality))
}

const hasMediaOnlyModality = (modalities: string[]): boolean => {
  return modalities.some((modality) => /image|audio|video|speech|embedding|moderation|realtime|transcription/.test(modality))
    && !hasTextOrChatModality(modalities)
}

const isNonChatModel = (modelId: string, metadata?: ModelMetadata): boolean => {
  const normalized = modelId.toLowerCase()
  const capabilities = metadata?.capabilities
  if (capabilities && typeof capabilities === 'object' && !Array.isArray(capabilities)) {
    const record = capabilities as Record<string, unknown>
    if (record.chat === false || record.chat_completions === false) {
      return true
    }
  }
  if (metadata?.type && typeof metadata.type === 'string' && /image|audio|video|speech|embedding|moderation/i.test(metadata.type)) {
    return true
  }
  const modalityRecord = asRecord(metadata?.modalities)
  if (modalityRecord && 'output' in modalityRecord) {
    if (hasMediaOnlyModality(collectStringValues(modalityRecord.output))) return true
  } else if (hasMediaOnlyModality(collectStringValues(metadata?.modalities))) {
    return true
  }
  return (
    ...
    normalized.startsWith('grok-imagine-') ||
...
const modelOptionsFromPayload = (payload: unknown): ModelOption[] => {
  ...
  return rawModels
    .map((model): (ModelOption & { metadata?: ModelMetadata }) | null => {
    ...
    .filter((model): model is ModelOption & { metadata?: ModelMetadata } => {
      if (!model || seen.has(model.id)) return false
      seen.add(model.id)
      return !isNonChatModel(model.id, model.metadata)
    })
    .map(({ metadata: _metadata, ...model }) => model)
}
...
const streamChatResponse = async (response: Response): Promise<void> => {
  if (response.headers.get('content-type')?.includes('application/json')) {
    ...
    if (!content) throw new Error(t('keys.testModelModal.emptyResponse'))
  ...
  const processEvent = () => {
    ...
    try {
      ...
      throw new Error(t('keys.testModelModal.invalidResponse'))
    }
    const root = asRecord(payload)
    const detail = responseErrorDetail(payload)
    if (root?.error || root?.type === 'error') {
  ...
  if (eventLines.length > 0) processEvent()
  if (!finished) throw new Error(t('keys.testModelModal.incompleteResponse'))
}
...
const startTest = async () => {
  ...
  try {
    ...
    if (!streamingContent.value) throw new Error(t('keys.testModelModal.emptyResponse'))
    addLine(streamingContent.value, 'text-green-300')
    streamingContent.value = ''
    status.value = 'success'
  } catch (error: unknown) {
    if (isAbortError(error)) {
      // Cancellation is a neutral outcome: leave the modal ready for a retry
      // instead of marooning it in the testing state.
      status.value = 'ready'
      streamingContent.value = ''
      errorMessage.value = ''
      return
