...
import * as userAccountsAPI from '@/api/accounts'
import {
  useAccountOAuth,
  type AddMethod,
...
const updateAccount = (id: number, updates: Parameters<typeof adminAPI.accounts.update>[1]) => {
  return isUserScope.value
    ? userAccountsAPI.update(id, updates)
    : adminAPI.accounts.update(id, updates)
}

const clearAccountError = async (id: number): Promise<void> => {
  // The user API intentionally does not expose an admin-only error reset route.
  // Updating the owned account is sufficient after a successful re-authorization.
  if (!isUserScope.value) {
    await adminAPI.accounts.clearError(id)
  }
}
...
const handleExchangeCode = async () => {
  ...
  if (isOpenAILike.value) {
    ...
    try {
      ...
      await clearAccountError(props.account.id)
...
const handleCookieAuth = async (sessionKey: string) => {
  ...
  try {
    const tokenInfo = await claudeOAuth.cookieAuth(
      addMethod.value,
      sessionKey,
      props.account.proxy_id
    )
