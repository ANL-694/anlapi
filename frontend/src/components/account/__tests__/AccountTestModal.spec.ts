...
describe('AccountTestModal', () => {
  ...
  it('uses the injected user models loader and user test endpoint', async () => {
    const userModelsLoader = vi.fn().mockResolvedValue([
      { id: 'gpt-5.4', display_name: 'GPT-5.4' }
    ])
    const wrapper = mount(AccountTestModal, {
      props: {
        show: false,
        account: buildAccount(),
        modelsLoader: userModelsLoader,
        testEndpointBase: '/api/v1/accounts',
      },
      global: {
        stubs: {
          BaseDialog: BaseDialogStub,
    ...
    expect(userModelsLoader).toHaveBeenCalledWith(1)
    expect(getAvailableModelsMock).not.toHaveBeenCalled()
    expect(global.fetch).toHaveBeenCalledWith(
      '/api/v1/accounts/1/test',
