...
describe('AccountUsageCell', () => {
  ...
  it('uses the injected user usage loader instead of the admin API', async () => {
    const userUsageLoader = vi.fn().mockResolvedValue({
      five_hour: { utilization: 12, resets_at: '2026-08-18T12:00:00Z' },
    })

    mount(AccountUsageCell, {
      props: {
        account: makeAccount({ id: 5100, platform: 'openai', type: 'oauth', extra: {} }),
        usageLoader: userUsageLoader,
        usageCacheScope: 'user',
      },
      global: {
        stubs: {
          UsageProgressBar: true,
    ...
    expect(userUsageLoader).toHaveBeenCalledWith(5100)
    expect(getUsage).not.toHaveBeenCalled()
  })
