  ...
  >
    ...
    >
      ...
      <GroupSelector
        ...
        group-scope="user_private"
        :owner-user-id="account?.owner_user_id"
...
const loadGrokMediaEligibility = async (accountID: number): Promise<GrokMediaEligibilityState | null> => {
  if (!isGrokOAuthAccount.value || typeof adminAPI.accounts.getGrokMediaEligibility !== 'function') return null
  ...
  try {
    ...
    if (requestVersion === grokMediaEligibilityRequestVersion) grokMediaEligibilityLoading.value = false
  }
}
