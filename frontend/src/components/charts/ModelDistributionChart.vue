<template>
  <div class="card p-4">
    ...
    <ErrorState v-else-if="rankingError" :retry="true" @retry="emit('ranking-retry')" />
...
const props = withDefaults(defineProps<{
  ...
  rankingTotalActualCost?: number | null
  rankingTotalRequests?: number | null
  rankingTotalTokens?: number | null
  loading?: boolean
  ...
  rankingItems: () => [],
  rankingTotalActualCost: null,
  rankingTotalRequests: null,
  rankingTotalTokens: null,
  loading: false,
  metric: 'tokens',
...
const otherRankingItem = computed<RankingDisplayItem | null>(() => {
  if (!props.rankingItems?.length) return null
  if (props.rankingTotalActualCost === null || props.rankingTotalRequests === null || props.rankingTotalTokens === null) return null
