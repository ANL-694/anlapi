...
import type { AdminGroup, GroupPlatform, GroupScope } from '@/types'
...
interface Props {
  ...
  /** Restrict account forms to public groups or groups owned by one user. */
  groupScope?: GroupScope | 'all'
...
const filteredGroups = computed(() => {
  let result: AdminGroup[] = authStore.isSimpleMode
    ...
    : props.groups
  if (props.groupScope && props.groupScope !== 'all') {
    result = result.filter((group) => {
      // Keep existing bindings visible so opening and saving an older account
      // cannot silently hide or alter its historical group assignments.
      if (props.modelValue.includes(group.id)) return true
      const scope = group.scope || 'public'
      if (props.groupScope === 'public') return scope === 'public'
      if (props.ownerUserId == null) return scope === 'public'
      return scope === 'public' ||
        ((scope === 'user_private' || scope === 'user_carpool') && group.owner_user_id === props.ownerUserId)
    })
  }
  if (props.platform) {
