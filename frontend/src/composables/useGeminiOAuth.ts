...
export function useGeminiOAuth(scope: 'admin' | 'user' = 'admin') {
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      const tokenInfo = scope === 'user'
        ? (await apiClient.post<Record<string, unknown>>('/account-oauth/gemini/exchange-code', payload)).data
  ...
  const getCapabilities = async (): Promise<GeminiOAuthCapabilities | null> => {
    try {
      if (scope === 'user') {
        const { data } = await apiClient.get<GeminiOAuthCapabilities>('/account-oauth/gemini/capabilities')
        return data
      }
