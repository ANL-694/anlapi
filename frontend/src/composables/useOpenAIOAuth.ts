...
export function useOpenAIOAuth(scope: 'admin' | 'user' = 'admin') {
  ...
  const generateAuthUrl = async (
    ...
    try {
      ...
      const endpoint = `${endpointPrefix}/auth-url`
      const response = scope === 'user'
        ? (await apiClient.post<{ auth_url: string; session_id: string }>(endpoint, payload)).data
  ...
  const exchangeAuthCode = async (
    ...
    try {
      ...
      const tokenInfo = scope === 'user'
        ? (await apiClient.post<Record<string, unknown>>(`${endpointPrefix}/exchange-code`, payload)).data
        : await adminAPI.accounts.exchangeCode(`${endpointPrefix}/exchange-code`, payload)
  ...
  const validateRefreshToken = async (
    ...
    try {
      ...
      const tokenInfo = scope === 'user'
        ? (await apiClient.post<Record<string, unknown>>(`${endpointPrefix}/refresh-token`, payload)).data
