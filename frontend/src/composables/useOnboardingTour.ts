...
export function useOnboardingTour(options: OnboardingOptions) {
  ...
  const getStorageKey = () => {
    const baseKey = typeof options.storageKey === 'function'
      ? options.storageKey()
      : options.storageKey ?? 'onboarding_tour'
  ...
  const startTour = async (startIndex = 0) => {
    // Administrators can enter the real user workspace, so the active
    // workspace decides which guide to show instead of the account role alone.
    const workspace = typeof options.workspace === 'function'
      ? options.workspace()
      : options.workspace
    const isAdmin = workspace ? workspace === 'admin' : userStore.user?.role === 'admin'
