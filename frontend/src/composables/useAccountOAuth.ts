...
export function useAccountOAuth(scope: 'admin' | 'user' = 'admin') {
  ...
  const generateAuthUrl = async (
    ...
    try {
      ...
      const response = scope === 'user'
        ? (await apiClient.post<{ auth_url: string; session_id: string }>(endpoint, proxyConfig)).data
        : await adminAPI.accounts.generateAuthUrl(endpoint, proxyConfig)
  ...
  const exchangeAuthCode = async (
    ...
    try {
      ...
      const tokenInfo = scope === 'user'
        ? (await apiClient.post<Record<string, unknown>>(endpoint, payload)).data
        : await adminAPI.accounts.exchangeCode(endpoint, payload)
