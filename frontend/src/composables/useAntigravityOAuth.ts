...
export function useAntigravityOAuth(scope: 'admin' | 'user' = 'admin') {
  ...
  const generateAuthUrl = async (proxyId: number | null | undefined): Promise<boolean> => {
    ...
    try {
      ...
      const response = scope === 'user'
        ? (await apiClient.post<{ auth_url: string; session_id: string; state: string }>(
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      const tokenInfo = scope === 'user'
        ? (await apiClient.post<Record<string, unknown>>('/account-oauth/antigravity/exchange-code', payload)).data
  ...
  const validateRefreshToken = async (
    ...
    try {
      const tokenInfo = scope === 'user'
        ? (await apiClient.post<Record<string, unknown>>('/account-oauth/antigravity/refresh-token', {
          refresh_token: refreshToken.trim(),
          ...(proxyId ? { proxy_id: proxyId } : {})
        })).data
        : await adminAPI.antigravity.refreshAntigravityToken(refreshToken.trim(), proxyId)
      return tokenInfo as AntigravityTokenInfo
    } catch (err: any) {
      error.value =
