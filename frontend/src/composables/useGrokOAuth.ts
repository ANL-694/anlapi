...
export function useGrokOAuth(scope: 'admin' | 'user' = 'admin') {
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      return scope === 'user'
        ? (await apiClient.post<GrokTokenInfo>('/account-oauth/grok/exchange-code', payload)).data
  ...
  const validateRefreshToken = async (
    ...
    try {
      return scope === 'user'
        ? (await apiClient.post<GrokTokenInfo>('/account-oauth/grok/refresh-token', {
  ...
  const authorizePassword = async (
    ...
    if (scope === 'user') {
      error.value = t('common.adminOnly')
      return null
    }
