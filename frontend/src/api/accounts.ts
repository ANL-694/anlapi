...
import type { Account, AccountUsageInfo, AccountUsageStatsResponse, AdminDataPayload, ClaudeModel, CreateProxyRequest, PaginatedResponse, Proxy, ProxyQualityCheckResult, UpdateAccountRequest, UpdateProxyRequest, UserAccountQuotaPoolDashboard, WindowStats } from '@/types'
...
export async function create(accountData: UserAccountCreateRequest): Promise<Account> {
  const { data } = await apiClient.post<Account>('/accounts', accountData)
  return data
}

export async function importAccount(accountData: UserAccountCreateRequest): Promise<Account> {
