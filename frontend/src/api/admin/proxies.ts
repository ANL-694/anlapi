...
function invalidProxyListResponse(): never {
  throw new Error('Invalid proxy list response')
}

function parsePaginatedProxyList(data: unknown): PaginatedResponse<Proxy> {
  if (!data || typeof data !== 'object') return invalidProxyListResponse()
  const payload = data as Partial<PaginatedResponse<Proxy>>
  if (!Array.isArray(payload.items)) return invalidProxyListResponse()
  return data as PaginatedResponse<Proxy>
}

function parseProxyList(data: unknown): Proxy[] {
  if (!Array.isArray(data)) return invalidProxyListResponse()
...
export async function list(
  ...
  return parsePaginatedProxyList(data)
}
