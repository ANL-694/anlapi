...
const TOKEN_REFRESH_LOCK_NAME = 'anlapi-auth-token-refresh'
...
export function refreshAuthTokens(
  ...
  if (
    inFlightRefresh &&
    inFlightRefresh.snapshot.refreshToken === snapshot.refreshToken &&
    inFlightRefresh.snapshot.userID === snapshot.userID
  ) {
    return inFlightRefresh.promise
  }

  const pending = runRefresh(snapshot, options)
  const entry = { snapshot, promise: pending }
  inFlightRefresh = entry
  const clearPending = (): void => {
    if (inFlightRefresh === entry) {
