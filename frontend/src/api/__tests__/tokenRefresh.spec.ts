...
describe('refreshAuthTokens', () => {
  ...
  it('does not share an old session refresh with a newly signed-in session', async () => {
    vi.useFakeTimers()
    seedSession({
      auth_token: 'user-a-access',
      refresh_token: 'user-a-refresh'
    })
    let resolveOldRequest!: (value: ReturnType<typeof refreshedResponse>) => void
    mockedPost.mockImplementationOnce(
      () => new Promise((resolve) => {
    ...
    const oldSessionRefresh = refreshAuthTokens({ failedAccessToken: 'user-a-access' })
    ...
    await expect(
      refreshAuthTokens({ failedAccessToken: 'user-b-access' })
    ).resolves.toMatchObject({
      access_token: 'user-b-new-access',
      refresh_token: 'user-b-new-refresh'
    })
    ...
    resolveOldRequest(refreshedResponse())
    const rejection = expect(oldSessionRefresh).rejects.toThrow(
