export default {
    accounts: {
      ...
      usageWindowsHint: '"5h / 7d" are the upstream account\'s official rolling usage windows (e.g. OpenAI ChatGPT, Claude). They are imposed by the upstream provider on the account itself — not configured by ANLAPI, and unrelated to the models you map. Usage resets automatically once each window rolls over, and the limit cannot be lifted from within ANLAPI.',
      ollamaCloud: {
      ...
      upstreamBilling: {
        trustWarning: 'This rate is declared by the upstream site for the current API key. ANLAPI cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.',
      ...
      poolModeInfo:
        'When enabled, upstream 429/403/401 errors will auto-retry without marking the account as rate-limited or errored. Suitable for upstream pointing to another ANLAPI instance.',
      ...
      grokClientToolCache: {
        title: 'Client Tool Cache',
        hint: 'Applies only to recognized Grok Free OAuth accounts. Codex, Trae, and similar client-tool requests use the upstream prompt-cache route by default; turn this off to disable that automatic routing.'
      },
