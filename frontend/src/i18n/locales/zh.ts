...
const anlZhOverrides = {
  ...
  httpStatusCodes: {
    title: 'HTTP 状态码',
    description: '按类别查看常用 HTTP 状态码及其简要含义。',
    codes: '个状态码',
    note: '状态码的具体语义可能受 HTTP 方法、响应头和应用协议约定影响。',
  },
  ...
  keys: {
    ...
    deleteSystemConfirmMessage: "确定要重新生成 '{name}' 吗？删除后系统会立即创建一个新的生图专线 Key，原 Key 将失效。",
    systemManaged: '自动绑定',
    imageGenerationRoute: 'GPT 生图专线',
    apiKey: 'API 密钥',
    group: '分组',
    ...
    keyCreatedRevealTitle: '请立即保存 API 密钥',
    keyCreatedRevealWarning: '完整密钥只在当前会话中显示。关闭页面或重新登录后无法再次恢复，请立即复制并妥善保存。',
    keyCreatedRevealHint: '列表和后续读取接口只显示脱敏值；只有本次会话中的新密钥可以继续使用、测试或导入。',
    keyValueUnavailable: '完整 API 密钥不可用，请创建或刷新密钥后再操作。',
    ...
    systemGroupLocked: 'GPT 生图专线由系统自动绑定到管理员指定的生图分组，可直接使用',
    ...
    testModelModal: {
      title: '测试 API 密钥',
      description: '先读取此密钥实际可用的模型，再发送一条最小对话请求验证模型是否正常。',
      modelLabel: '选择模型',
      modelPlaceholder: '选择要测试的模型',
      loadingModels: '正在读取可用模型...',
      modelsLoaded: '已读取 {count} 个可测试模型',
      noModels: '此 API 密钥没有可用于对话测试的模型。',
      missingKey: '当前 API 密钥内容不可用，无法测试。',
      loadFailed: '读取可用模型失败',
      ready: '已准备好测试',
      testing: '测试中...',
      starting: '正在使用 {model} 发送测试请求',
      promptHint: '测试消息：Reply with exactly OK.',
      usageHint: '测试会产生一次最小模型调用，并计入此 API 密钥的用量。',
      requestFailed: '请求失败',
      noResponseBody: '服务器没有返回响应内容',
      emptyResponse: '模型返回了空响应，但请求已成功。',
      testFailed: '模型测试失败',
      completed: '模型响应正常',
      start: '开始测试',
      retry: '重新测试',
  ...
  usage: {
    ...
    cacheCreation5mHint: '5 分钟缓存创建 Token',
    cacheCreation1hHint: '1 小时缓存创建 Token',
    totalRequests: '总请求数',
    totalTokens: '总 Token',
    totalCost: '总消费',
    standardCost: '标准',
  ...
  admin: {
    ...
    backup: {
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2: '点击「创建存储桶」，输入名称（如 anlapi-backups），选择区域',
          line3: '点击创建完成'
        },
        step2: {
          title: '创建 API 令牌',
    ...
    accounts: {
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号本身设定，并非 anlapi 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 anlapi 端解除该限制。',
      ...
      openai: {
        ...
        imageApiKeyTitle: 'GPT 生图 Key/上游',
        imageApiKeyDesc: 'OpenAI API Key 或兼容生图上游',
        imageApiKeyBaseUrlHint: '已预填 OpenAI 官方地址；如使用 OpenAI 兼容生图上游，可修改此地址。',
        imageApiKeyGroupRequirement: '该账号必须绑定 allow_image_generation=true 的 OpenAI 生图分组；快捷模式仅显示符合条件的分组。',
        imageApiKeyNoEligibleGroup: '当前没有符合条件的 OpenAI 生图分组，请先在分组管理中开启图片生成。',
        imageApiKeyGroupRequired: '请先选择一个已开启图片生成的 OpenAI 分组。',
        imageApiKeyModelRestriction: 'GPT Image 模型白名单',
        imageApiKeyModelsLocked: '快捷模式固定使用项目当前支持的 GPT Image 模型，提交时不可扩展为其他模型。',
      ...
      poolModeInfo: '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 anlapi 实例的场景。',
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 anlapi 用户登录',
  ...
  onboarding: {
    ...
    user: {
      welcome: {
        title: '欢迎使用 anlapi',
        description: '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 anlapi AI 服务平台。</p><p style="margin-bottom: 12px;"><b>快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>创建 API 密钥</li><li>复制密钥到您的应用</li><li>开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始',
        prevBtn: '跳过'
      },
      keyManage: {
...
const parityZhSupplement = {
  ...
  admin: {
    accounts: {
      ...
      accountUpdatedSuccess: '账号更新成功',
      cookieRefreshedSuccess: 'Cookie 刷新成功',
      deleteConfirmMessage: '确定要删除此账号吗？',
      failedToSave: '保存账号失败',
      filters: {
        allPlatforms: '全部平台',
        allStatuses: '全部状态',
      ...
      form: {
        credentialsLabel: '凭据',
        credentialsPlaceholder: '输入凭据',
        nameLabel: '名称',
        namePlaceholder: '输入账号名称',
        platformLabel: '平台',
        ...
        statusLabel: '状态',
        typeLabel: '类型',
        weightHint: '数值越大分配的流量越多。',
        weightLabel: '权重'
      },
      noAccounts: '暂无账号',
      noAccountsDescription: '添加账号后即可开始转发请求。',
      refreshCookie: '刷新 Cookie',
      refreshing: '刷新中',
      saving: '保存中',
      testAccount: '测试账号',
...
export default mergeLocaleMessages(mergeLocaleMessages(sub2Zh, anlZhOverrides), parityZhSupplement)
