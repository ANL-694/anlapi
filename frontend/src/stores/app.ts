...
export const useAppStore = defineStore('app', () => {
  ...
  const SIDEBAR_COLLAPSED_STORAGE_KEY = 'sub2api_sidebar_collapsed'
  ...
  const sidebarCollapsed = ref<boolean>(readSidebarCollapsedPreference())
  ...
  const siteName = ref<string>('ANL Gateway')
  ...
  function reset(): void {
    ...
    try {
      ...
      // Ignore unavailable storage during test teardown or private browsing.
    }
    loading.value = false
  ...
  function applySettings(config: PublicSettings): void {
    ...
    // 176 的历史默认值曾写成 anlapi；仅修正这个遗留值，保留用户自定义站点名。
    siteName.value = config.site_name === 'anlapi' || config.site_name === 'anl-api'
      ? 'ANL Gateway'
      : (config.site_name || 'ANL Gateway')
