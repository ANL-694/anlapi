...
func TestEveryGatewayPOSTRouteIsClassifiedForPromptAuditCoverage(t *testing.T) {
	...
	audited := map[string][]string{
		...
		"/realtime/sessions":        {"openai_live.go"},
	...
	for route, files := range audited {
		...
		for _, filename := range files {
			...
			require.Truef(t,
				strings.Contains(handlerSource, "checkSecurityAudit") || strings.Contains(handlerSource, "runPreFlightHooks"),
				"%s route handler %s bypasses Coordinator", route, filename,
			)
		}
	}
	preFlightSource, err := os.ReadFile(filepath.Join("..", "..", "handler", "gateway_preflight.go"))
	require.NoError(t, err)
	require.Contains(t, string(preFlightSource), "checkSecurityAudit", "HTTP preflight must delegate to Prompt Audit Coordinator")
