...
func RegisterGatewayRoutes(
	...
	cfg *config.Config,
	privateGateway ...*middleware.ANLPrivateGateway,
) {
	...
	var privateGatewayMiddleware *middleware.ANLPrivateGateway
	if len(privateGateway) > 0 {
		privateGatewayMiddleware = privateGateway[0]
	}
	if privateGatewayMiddleware == nil {
		privateGatewayMiddleware = middleware.NewANLPrivateGateway(cfg, apiKeyService, nil)
	}
	...
	gateway.Use(privateGatewayMiddleware.Authentication())
	gateway.Use(gin.HandlerFunc(apiKeyAuth))
	gateway.GET("/sub2api/billing", h.Gateway.KeyBillingInfo)
	gateway.GET("/sub2api/idempotency/:key", privateGatewayMiddleware.Status())
	gateway.Use(privateGatewayMiddleware.Idempotency())
