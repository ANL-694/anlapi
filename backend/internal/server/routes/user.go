...
func RegisterUserRoutes(
	...
	authenticated.Use(gin.HandlerFunc(auditLog))
	{
		...
		accounts.POST("/import-credentials", panelRateLimiter.Heavy(), h.UserAccount.ImportCredentials)
		accounts.POST("/bulk-update", h.UserAccount.BulkUpdate)
		accounts.POST("/bulk-delete", h.UserAccount.BulkDelete)
		accounts.POST("/batch-refresh/async", panelRateLimiter.Heavy(), h.UserAccount.CreateBatchRefreshTask)
		accounts.POST("/batch-revalidate-public-share/async", panelRateLimiter.Heavy(), h.UserAccount.CreateBatchRevalidatePublicShareTask)
		accounts.GET("/batch-tasks/:id", h.UserAccount.GetBatchTask)
		accounts.POST("/today-stats/batch", panelRateLimiter.Heavy(), h.UserAccount.GetBatchTodayStats)
		accounts.GET("/:id/usage", panelRateLimiter.Heavy(), h.UserAccount.GetUsage)
		accounts.GET("/:id/stats", panelRateLimiter.Heavy(), h.UserAccount.GetStats)
		accounts.GET("/:id/today-stats", panelRateLimiter.Heavy(), h.UserAccount.GetTodayStats)
		accounts.GET("/:id/models", h.UserAccount.GetAvailableModels)
		accounts.POST("/:id/test", panelRateLimiter.Heavy(), h.UserAccount.Test)
		accounts.POST("/:id/refresh", panelRateLimiter.Heavy(), h.UserAccount.Refresh)
		...
		accountProxies.POST("/:id/test", panelRateLimiter.Heavy(), h.UserAccount.TestProxy)
		accountProxies.POST("/:id/quality-check", panelRateLimiter.Heavy(), h.UserAccount.CheckProxyQuality)
