...
func TestAdminAuthSeparatesUserAndAdminRequests(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router.GET("/api/v1/admin/users", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"workspace": "admin"})
	})
	...
	tests := []struct {
		...
		{
			name:       "regular user cannot call admin API",
			tokenUser:  1,
			wantStatus: http.StatusForbidden,
			wantBody:   "FORBIDDEN",
		},
		{
			name:       "administrator can call admin API",
			tokenUser:  2,
			wantStatus: http.StatusOK,
			wantBody:   `"workspace":"admin"`,
		},
	}
