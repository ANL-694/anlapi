...
func accountCredentialImportSourceFromMap(item map[string]any, opts AccountCredentialImportOptions) (AccountCredentialImportSource, error) {
	...
	if opts.ClaudeWebImport && hasClaudeWebImportPayload(item) {
		return AccountCredentialImportSource{}, fmt.Errorf("Claude Web credential import is not supported by the Sub2API 0.1.176 base")
	}
...
func hasClaudeWebImportPayload(item map[string]any) bool {
	if len(item) == 0 {
		return false
	}
	_, found := importAnyField(item, "cookies", "cookie", "browser_cookie", "browserCookie")
	return found
}
