...
func (s *RevenueService) fillRevenueWalletBreakdownStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	hasPointsLedger, err := s.pointsLedgerTableAvailable(ctx)
	if err != nil {
		return fmt.Errorf("check revenue points ledger availability: %w", err)
	}
	if hasPointsLedger {
...
func (s *RevenueService) pointsLedgerTableAvailable(ctx context.Context) (bool, error) {
	query := `
		SELECT EXISTS (
			SELECT 1
	...
	if s.entClient.Driver().Dialect() != "postgres" {
		query = "SELECT COUNT(*) > 0 FROM sqlite_master WHERE type = 'table' AND name = 'points_ledger'"
	}
	var exists bool
	if err := s.querySingle(ctx, query, nil, &exists); err != nil {
		return false, err
	}
	return exists, nil
}
