...
type UpstreamFailoverError struct {
	...
	RequestScopedTransient   bool        // 请求级瞬时故障（如容量降载），不得据此临时封禁账号
...
func NewGatewayService(deps ...any) *GatewayService {
	...
	for _, dependency := range deps {
		...
		if value, ok := dependency.(AccountRepository); ok {
			accountRepo = value
		}
		if value, ok := dependency.(GroupRepository); ok {
			groupRepo = value
		}
		if value, ok := dependency.(UsageLogRepository); ok {
			usageLogRepo = value
		}
		if value, ok := dependency.(UsageBillingRepository); ok {
			usageBillingRepo = value
		}
		if value, ok := dependency.(UserRepository); ok {
			userRepo = value
		}
		if value, ok := dependency.(UserSubscriptionRepository); ok {
			userSubRepo = value
		}
		if value, ok := dependency.(UserGroupRateRepository); ok {
			userGroupRateRepo = value
		}
		if value, ok := dependency.(GatewayCache); ok {
			cache = value
		}
		if value, ok := dependency.(HTTPUpstream); ok {
			httpUpstream = value
		}
		if value, ok := dependency.(SessionLimitCache); ok {
			sessionLimitCache = value
		}
		if value, ok := dependency.(RPMCache); ok {
			rpmCache = value
		}
...
func (s *GatewayService) BindStickySessionForGroup(ctx context.Context, groupID *int64, sessionHash string, accountID int64, group *Group) error {
	...
	return s.cache.SetSessionAccountID(ctx, derefGroupID(groupID), sessionHash, accountID, group.EffectiveKiroStickySessionTTL())
}
