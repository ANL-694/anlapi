...
// account snapshot used by media scheduling. Deterministic states such as
// forbidden or Free are returned as normal ineligibility decisions.
...
func buildGrokQuotaProbeBody(model string) ([]byte, error) {
	return buildGrokResponsesProbeBody(model, grokQuotaProbeInput)
}

func buildGrokResponsesProbeBody(model, input string) ([]byte, error) {
