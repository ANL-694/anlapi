...
type OpsIngressRejectHealth struct {
	...
	InvalidAuthAbuse InvalidAuthAbuseHealth `json:"invalid_auth_abuse"`
}

type invalidAuthAbuseHealthReporter interface {
	InvalidAuthAbuseHealth() InvalidAuthAbuseHealth
}
...
type OpsIngressRejectAggregator struct {
	...
	invalidAuthAbuseHealth invalidAuthAbuseHealthReporter
...
func NewOpsIngressRejectAggregator(repo OpsIngressRejectRepository, healthReporters ...invalidAuthAbuseHealthReporter) *OpsIngressRejectAggregator {
	ctx, cancel := context.WithCancel(context.Background())
	a := &OpsIngressRejectAggregator{
		repo: repo, ctx: ctx, cancel: cancel, flushCh: make(chan struct{}, 1),
	}
	if len(healthReporters) > 0 {
		a.invalidAuthAbuseHealth = healthReporters[0]
	}
...
func (a *OpsIngressRejectAggregator) Health() OpsIngressRejectHealth {
	...
	if a.invalidAuthAbuseHealth != nil {
		h.InvalidAuthAbuse = a.invalidAuthAbuseHealth.InvalidAuthAbuseHealth()
	}
	return h
}
