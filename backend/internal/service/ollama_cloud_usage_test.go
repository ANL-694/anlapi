...
func sameOllamaUsageTestIdentity(left, right *Account) bool {
	return left != nil && right != nil && left.Platform == right.Platform && left.Type == right.Type &&
		...
		reflect.DeepEqual(left.OwnerUserID, right.OwnerUserID)
}
...
func TestOllamaCloudUsageGroupFingerprintScopesByOwnerAndKeepsANLAccountSemantics(t *testing.T) {
	ownerUserID := int64(101)
	account := ollamaUsageAccount(1)
	account.Credentials["api_key"] = "shared-key"
	...
	sameOwner := *account
	sameOwner.Platform = PlatformAnthropic
	sameOwner.ShareMode = AccountShareModePrivate
	sameOwner.ShareStatus = AccountShareStatusSuspended
	sameOwner.Concurrency = 99
	sameOwnerFingerprint, ok := ollamaCloudUsageGroupFingerprint(&sameOwner)
	require.True(t, ok)
	require.Equal(t, fingerprint, sameOwnerFingerprint)

	otherOwnerUserID := int64(202)
	otherOwner := sameOwner
	otherOwner.OwnerUserID = &otherOwnerUserID
	otherOwnerFingerprint, ok := ollamaCloudUsageGroupFingerprint(&otherOwner)
	require.True(t, ok)
	require.NotEqual(t, fingerprint, otherOwnerFingerprint)

	systemAccount := sameOwner
	systemAccount.OwnerUserID = nil
	systemFingerprint, ok := ollamaCloudUsageGroupFingerprint(&systemAccount)
	require.True(t, ok)
	require.NotEqual(t, fingerprint, systemFingerprint)

	oauthAccount := sameOwner
	oauthAccount.Type = AccountTypeOAuth
	_, ok = ollamaCloudUsageGroupFingerprint(&oauthAccount)
	require.False(t, ok)
}
...
func TestOllamaCloudUsageManagedExtraCRSImportUsesLocalState(t *testing.T) {
