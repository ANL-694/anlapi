...
func buildSMTPMessageWithContentType(config *SMTPConfig, to, subject, body, contentType string) (smtpMessage, error) {
	...
	contentType = strings.TrimSpace(sanitizeEmailHeader(contentType))
	if contentType == "" {
		contentType = "text/html; charset=UTF-8"
	}
	if !strings.EqualFold(contentType, "text/html; charset=UTF-8") && !strings.EqualFold(contentType, "text/plain; charset=UTF-8") {
		return smtpMessage{}, fmt.Errorf("unsupported email content type: %s", contentType)
	}
