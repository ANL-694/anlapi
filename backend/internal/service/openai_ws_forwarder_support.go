...
func (s *OpenAIGatewayService) performOpenAIWSGeneratePrewarm(
	...
	for {
		...
		if eventType == "response.failed" {
			...
			if errMsg == "" {
				errMsg = "OpenAI websocket prewarm failed"
			}
			lease.MarkBroken()
			if s.persistOpenAIModelCapacitySignal(ctx, account, lease.HandshakeHeaders(), message, errMsg) {
				return wrapOpenAIWSFallback("prewarm_upstream_capacity", errors.New(errMsg))
			}
...
func (s *OpenAIGatewayService) persistOpenAIWSRateLimitSignal(ctx context.Context, account *Account, headers http.Header, responseBody []byte, codeRaw, errTypeRaw, msgRaw string) {
	...
	if s.persistOpenAIModelCapacitySignal(ctx, account, headers, responseBody, msgRaw) {
		return
	}
...
func classifyOpenAIWSErrorEventFromRaw(codeRaw, errTypeRaw, msgRaw string) (string, bool) {
	...
	if isOpenAIModelCapacityText(codeRaw + " " + errTypeRaw + " " + msgRaw) {
...
func openAIWSErrorHTTPStatusFromRaw(codeRaw, errTypeRaw string, msgRaw ...string) int {
	code := strings.ToLower(strings.TrimSpace(codeRaw))
	errType := strings.ToLower(strings.TrimSpace(errTypeRaw))
	message := ""
	if len(msgRaw) > 0 {
	...
	switch {
	case isOpenAIModelCapacityText(codeRaw + " " + errTypeRaw + " " + message):
		return http.StatusServiceUnavailable
...
func openAIWSErrorHTTPStatus(message []byte) int {
	...
	return openAIWSErrorHTTPStatusFromRaw(codeRaw, errTypeRaw, msgRaw)
}
