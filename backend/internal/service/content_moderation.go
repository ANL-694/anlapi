...
func (s *ContentModerationService) RecordCyberPolicyEvent(ctx context.Context, input CyberPolicyRecordInput) {
	...
	errorBody := strings.TrimSpace(input.UpstreamMessage)
	if body := strings.TrimSpace(input.UpstreamBody); body != "" {
		errorBody = strings.TrimSpace(errorBody + "\n" + body)
	}
	if input.UpstreamInTok > 0 || input.UpstreamOutTok > 0 {
		errorBody = fmt.Sprintf("%s\nupstream_usage=in:%d,out:%d", errorBody, input.UpstreamInTok, input.UpstreamOutTok)
	}
	log := &ContentModerationLog{
		RequestID:       input.RequestID,
		UserID:          userID,
		...
		HighestScore:    1,
		Error:           trimRunes(redactContentModerationSecrets(errorBody), maxModerationExcerptRunes*4),
		CreatedAt:       time.Now(),
	}
	autoBanned := s.applyFlaggedAccountSideEffects(ctx, cfg, log)
	if s.emailService != nil && strings.TrimSpace(log.UserEmail) != "" {
		if err := s.sendCyberPolicyEmail(ctx, log); err != nil {
			slog.Warn("content_moderation.cyber_email_failed", "user_id", input.UserID, "error", err)
		} else {
			log.EmailSent = true
		}
		if autoBanned {
			if err := s.sendAccountDisabledEmail(ctx, cfg, log); err != nil {
				slog.Warn("content_moderation.cyber_ban_email_failed", "user_id", input.UserID, "error", err)
			} else {
				log.EmailSent = true
			}
		}
	...
	if err := s.repo.CreateLog(ctx, log); err != nil {
		slog.Warn("content_moderation.cyber_create_log_failed", "user_id", input.UserID, "error", err)
	}
}
