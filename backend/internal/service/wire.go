...
func ProvideUpdateService(cache UpdateCache, githubClient GitHubReleaseClient, buildInfo BuildInfo, cfg *config.Config) *UpdateService {
	...
	return NewUpdateService(cache, githubClient, buildInfo.Version, buildInfo.BuildType, repository, dockerImage)
}
...
func ProvideEmailBroadcastService(repo EmailBroadcastRepository, userRepo UserRepository, emailService *EmailService, settingRepo SettingRepository) *EmailBroadcastService {
...
func ProvideCarpoolService(repo CarpoolRepository, groupRepo GroupRepository, accountRepo AccountRepository, proxyRepo ProxyRepository, userRepo UserRepository, userSubRepo UserSubscriptionRepository, subscriptionService *SubscriptionService, settingService *SettingService, billingCacheService *BillingCacheService, authCacheInvalidator APIKeyAuthCacheInvalidator, accountUsageService *AccountUsageService, emailService *EmailService, rateLimitService *RateLimitService) *CarpoolService {
	return NewCarpoolService(repo, groupRepo, accountRepo, proxyRepo, userRepo, userSubRepo, subscriptionService, settingService, billingCacheService, authCacheInvalidator, accountUsageService, emailService, rateLimitService)
}

func ProvideOpenAIGatewayService(accountRepo AccountRepository, usageLogRepo UsageLogRepository, usageBillingRepo UsageBillingRepository, userRepo UserRepository, userSubRepo UserSubscriptionRepository, userGroupRateRepo UserGroupRateRepository, cache GatewayCache, cfg *config.Config, schedulerSnapshot *SchedulerSnapshotService, concurrencyService *ConcurrencyService, billingService *BillingService, rateLimitService *RateLimitService, billingCacheService *BillingCacheService, httpUpstream HTTPUpstream, deferredService *DeferredService, openAITokenProvider *OpenAITokenProvider, grokTokenProvider *GrokTokenProvider, resolver *ModelPricingResolver, channelService *ChannelService, balanceNotifyService *BalanceNotifyService, settingService *SettingService, userPlatformQuotaRepo UserPlatformQuotaRepository, carpoolRepo CarpoolRepository) *OpenAIGatewayService {
...
func ProvideAccountService(accountRepo AccountRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, proxyProber ProxyExitInfoProber) *AccountService {
...
var ProviderSet = wire.NewSet(
	...
	NewAccountShareSettlementService,
	NewProxyService,
	NewRedeemService,
	NewPromoService,
	NewUsageService,
	...
	ProvideGroupRateScheduleServiceList,
	NewChannelService,
...
func ProvideGroupRateScheduleService(repo GroupRateScheduleRepository, groupRepo GroupRepository, authCacheInvalidator APIKeyAuthCacheInvalidator) *GroupRateScheduleService {
	svc := NewGroupRateScheduleService(repo, groupRepo, authCacheInvalidator, defaultGroupRateScheduleInterval)
	svc.Start()
	return svc
}

func ProvideGroupRateScheduleServiceList(svc *GroupRateScheduleService) []*GroupRateScheduleService {
	if svc == nil {
		return nil
	}
	return []*GroupRateScheduleService{svc}
}
...
func ProvideShopService(entClient *dbent.Client, paymentService *PaymentService, authCacheInvalidator APIKeyAuthCacheInvalidator, billingCacheService *BillingCacheService, settingRepo SettingRepository, key payment.EncryptionKey, fileCardStoreFactory ShopFileCardObjectStoreFactory) *ShopService {
	svc := NewShopService(entClient, paymentService, authCacheInvalidator, billingCacheService, WithShopSettingRepository(settingRepo), WithShopEncryptionKey([]byte(key)), WithShopFileCardObjectStoreFactory(fileCardStoreFactory))
	return svc
}
