...
// IsFreeModelsEnabled is the server-side source of truth for the user-facing
// free-model account surface. A missing setting or repository failure fails
// closed so the feature cannot be enabled accidentally.
func (s *SettingService) IsFreeModelsEnabled(ctx context.Context) bool {
	if s == nil || s.settingRepo == nil {
		return false
	}
	value, err := s.settingRepo.GetValue(ctx, SettingKeyFreeModelsEnabled)
