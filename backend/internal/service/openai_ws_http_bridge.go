...
const (
	...
	openAIWSHTTPBridgeDrainTimeoutDefault         = 30 * time.Second
)
...
// HTTP bridge requests deliberately outlive a client disconnect long enough to
// drain usage and terminal events, but they must still have a hard upper bound.
// Reuse the existing WS read timeout so operators have one familiar control.
func (s *OpenAIGatewayService) openAIWSHTTPBridgeTurnTimeout() time.Duration {
...
func prepareOpenAIWSHTTPBridgeBody(payload []byte) ([]byte, error) {
	// RawMessage preserves large numeric fields (for example max_output_tokens)
	// exactly. Decoding into map[string]any would round them through float64.
...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	upstreamBaseCtx := context.Background()
	if ctx != nil {
		// Keep draining after a client disconnect, while the explicit timeout
		// below prevents an abandoned upstream stream from living forever.
		upstreamBaseCtx = context.WithoutCancel(ctx)
	}
	upstreamCtx, cancelUpstream := context.WithTimeout(upstreamBaseCtx, s.openAIWSHTTPBridgeTurnTimeout())
	...
	startDrainTimer := func() {
		if drainTimer == nil {
			drainTimer = time.AfterFunc(openAIWSHTTPBridgeDrainTimeoutDefault, cancelUpstream)
		}
	}
	defer func() {
		if drainTimer != nil {
