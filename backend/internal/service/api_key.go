...
// System-managed API key purposes. Empty ManagedType means a user-created key.
const (
...
type APIKey struct {
	...
	// ManagedType is populated from the durable system_api_key_bindings table.
	// It is intentionally not stored on api_keys so ordinary key CRUD remains
