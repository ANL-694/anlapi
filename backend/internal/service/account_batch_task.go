...
type AccountBatchTaskRepository interface {
	CreateTask(context.Context, CreateAccountBatchTaskInput) (*AccountBatchTask, error)
	GetTask(context.Context, int64) (*AccountBatchTask, error)
	ClaimNextPendingTask(context.Context, int64) (*AccountBatchTask, error)
	ListPendingItems(context.Context, int64) ([]AccountBatchTaskItem, error)
	MarkItemRunning(context.Context, int64) error
	MarkItemSucceeded(context.Context, int64, map[string]any) error
	MarkItemFailed(context.Context, int64, string) error
	RefreshTaskProgress(context.Context, int64) (*AccountBatchTask, error)
	MarkTaskSucceeded(context.Context, int64) error
	MarkTaskFailed(context.Context, int64, string) error
}

type AccountBatchTaskExecutor func(context.Context, *AccountBatchTask, AccountBatchTaskItem) (map[string]any, error)
...
func NewAccountBatchTaskService(repo AccountBatchTaskRepository, timingWheel *TimingWheelService) *AccountBatchTaskService {
	return &AccountBatchTaskService{repo: repo, timingWheel: timingWheel, executors: map[string]AccountBatchTaskExecutor{}}
}

// Start is intentionally called after all handlers register their executors.
...
func (s *AccountBatchTaskService) CreateTask(ctx context.Context, input CreateAccountBatchTaskInput) (*AccountBatchTask, error) {
	...
	if input.Scope == "" || input.Operation == "" || input.CreatedBy <= 0 || len(input.AccountIDs) == 0 {
		return nil, fmt.Errorf("invalid account batch task input")
	}
...
func (s *AccountBatchTaskService) runOnce() {
	if s == nil || s.repo == nil || !atomic.CompareAndSwapInt32(&s.running, 0, 1) {
		return
	}
...
func (s *AccountBatchTaskService) executeItem(ctx context.Context, task *AccountBatchTask, item AccountBatchTaskItem, executor AccountBatchTaskExecutor) {
	if err := s.repo.MarkItemRunning(ctx, item.ID); err != nil {
		...
		// A task must never be reported as succeeded while an item remains
		// pending. Persist the claim failure as an item failure when possible;
		// finishTaskByProgress also fails closed on an incomplete task.
		_ = s.repo.MarkItemFailed(context.Background(), item.ID, "failed to claim task item: "+trimAccountBatchError(err.Error()))
		return
	}
...
func (s *AccountBatchTaskService) finishTaskByProgress(taskID int64) error {
	...
	if task == nil {
		return fmt.Errorf("account batch task %d disappeared while finishing", taskID)
	}
	if task.Processed < task.Total {
		message := fmt.Sprintf("account batch task incomplete: processed %d of %d", task.Processed, task.Total)
		_ = s.repo.MarkTaskFailed(context.Background(), taskID, message)
