...
const (
	...
	codexImageGenerationBridgeMarker     = "<anlapi-codex-image-generation>"
	codexImageGenerationBridgeText       = codexImageGenerationBridgeMarker + "\nWhen the user asks for raster image generation or editing, use the OpenAI Responses native `image_generation` tool attached to this request. The local Codex client may not expose an `image_gen` namespace, but that does not mean image generation is unavailable. Do not ask the user to switch to CLI fallback solely because `image_gen` is absent.\n</anlapi-codex-image-generation>"
	codexSparkImageUnsupportedMarker     = "<anlapi-codex-spark-image-unsupported>"
	codexSparkImageUnsupportedText       = codexSparkImageUnsupportedMarker + "\nThe current model is gpt-5.3-codex-spark, which does not support image generation, image editing, image input, the `image_generation` tool, or Codex `image_gen`/`$imagegen` workflows. If the user asks for image generation or image editing, clearly explain this model limitation and ask them to switch to a non-Spark Codex model such as gpt-5.3-codex or gpt-5.4. Do not claim that the local environment merely lacks image_gen tooling, and do not suggest CLI fallback as the primary fix while the model remains Spark.\n</anlapi-codex-spark-image-unsupported>"
)
...
func applyCodexOAuthTransformWithOptions(reqBody map[string]any, opts codexOAuthTransformOptions) codexTransformResult {
	...
	// ChatGPT internal Codex endpoint does not accept role:"system". Mirror its
	// text into instructions; callers may omit text-only input messages after a
	// lossless promotion to avoid duplicate OAuth system prompts.
...
func hasCodexImageGenerationFunctionTool(reqBody map[string]any) bool {
	return len(reqBody) > 0 && codexToolsContainFunctionName(reqBody["tools"], codexImageGenerationFunctionToolName)
}
...
// developer. When omitPromoted is true, text-only items are removed after
// their content is promoted without loss.
...
func filterCodexInputWithOptions(input []any, opts codexInputFilterOptions) []any {
	...
	for _, item := range input {
		...
		// Under store=false, replaying an rs_* id causes an upstream lookup and
		// 404. The encrypted_content payload itself is the supported cross-turn
		// carrier, so keep the reasoning item and remove only its id.
		if typ == "reasoning" {
			newItem := make(map[string]any, len(m))
			for key, value := range m {
				if key == "id" {
...
// isCodexToolCallInputType excludes output items whose ids are not constrained
// by the upstream fc-prefix validation.
