...
func parseOllamaCloudUsageHTML(body []byte) (*OllamaCloudUsageData, error) {
	...
	if ollamaUsageContainsAny(lowerPage, "sign in to ollama", "log in to ollama", "continue to sign in") {
...
func ollamaModelWindow(node *html.Node) OllamaCloudUsageModelWindow {
	for block := parentNode(node); block != nil; block = block.Parent {
		if value, ok := htmlAttribute(block, "data-usage-window"); ok {
			...
			if ollamaUsageContainsAny(normalized, "five hour", "5 hour", "5h", "session") {
				return OllamaCloudUsageModelWindowFiveHour
			}
			if ollamaUsageContainsAny(normalized, "seven day", "7 day", "7d", "weekly") {
				return OllamaCloudUsageModelWindowSevenDay
			}
		}
		text := strings.ToLower(normalizedNodeText(block))
		fiveHour := ollamaUsageContainsAny(text, ollamaFiveHourUsageAliases...)
		sevenDay := ollamaUsageContainsAny(text, ollamaSevenDayUsageAliases...)
...
func ollamaUsageContainsAny(value string, candidates ...string) bool {
