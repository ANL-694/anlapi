...
func (s *defaultOpenAIAccountScheduler) trySelectByLoadBalancePool(
	...
	if overflowResult, overflowCompactBlocked, overflowErr := s.tryProbeOpenAILoadBalanceOverflow(ctx, req, overflowPlan, budget); overflowErr != nil {
...
func (s *defaultOpenAIAccountScheduler) tryProbeOpenAILoadBalanceOverflow(
	ctx context.Context,
	req OpenAIAccountScheduleRequest,
	plan openAIAccountLoadPlan,
	budget *openAISelectionProbeBudget,
) (*AccountSelectionResult, bool, error) {
	if plan.includeOverflowFallback || plan.topK <= 0 || len(plan.candidates) <= plan.topK {
		return nil, false, nil
	}
	overflowLimit := openAIHybridOverflowProbeCount(plan.topK, len(plan.candidates))
	overflow := selectOpenAIOverflowProbeCandidates(plan.candidates, plan.selectionOrder, overflowLimit, req)
	if len(overflow) == 0 {
		return nil, false, nil
	}
	budget.enableLimit()
	return s.tryAcquireOpenAISelectionOrderWithBudget(ctx, req, overflow, budget)
}
