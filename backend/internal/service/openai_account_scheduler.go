...
func (s *defaultOpenAIAccountScheduler) SnapshotMetrics() OpenAIAccountSchedulerMetricsSnapshot {
	...
	if selectTotal > 0 {
		...
		stickyHitTotal := s.metrics.stickyHitTotal.Load()
		snapshot.StickyHitRatio = float64(stickyHitTotal) / float64(selectTotal)
...
func openAIQuotaHeadroomFactor(account *Account, now time.Time) float64 {
	...
	primaryUsedPercent, ok := resolveCanonicalCodexQuotaPercent(account.Extra, "7d")
	if !ok || openAICanonicalCodexQuotaWindowReset(account.Extra, "7d", now) {
		return openAIQuotaHeadroomNeutralFactor
	}

	factor := 1 - clamp01(primaryUsedPercent/100)
	if secondaryUsedPercent, ok := resolveCanonicalCodexQuotaPercent(account.Extra, "5h"); ok &&
		!openAICanonicalCodexQuotaWindowReset(account.Extra, "5h", now) {
...
// resolveCanonicalCodexQuotaPercent prefers the normalized 5h/7d fields. When
// only the raw primary/secondary headers exist, map them by their advertised
// window length (short <= 6h, long > 6h), retaining the historical primary=7d,
// secondary=5h fallback when no lengths are available.
func resolveCanonicalCodexQuotaPercent(extra map[string]any, window string) (float64, bool) {
	canonicalKey := "codex_" + window + "_used_percent"
	if value, ok := resolveAccountExtraNumber(extra, canonicalKey); ok {
		return value, true
	}
	primary, primaryOK := resolveAccountExtraNumber(extra, "codex_primary_used_percent")
	secondary, secondaryOK := resolveAccountExtraNumber(extra, "codex_secondary_used_percent")
	primaryMinutes, primaryMinutesOK := resolveAccountExtraNumber(extra, "codex_primary_window_minutes")
	secondaryMinutes, secondaryMinutesOK := resolveAccountExtraNumber(extra, "codex_secondary_window_minutes")
	short := window == "5h"
	if primaryMinutesOK && secondaryMinutesOK {
		if (primaryMinutes <= 360) == short && primaryOK {
			return primary, true
		}
		if (secondaryMinutes <= 360) == short && secondaryOK {
			return secondary, true
		}
	} else if primaryMinutesOK {
		if (primaryMinutes <= 360) == short && primaryOK {
			return primary, true
		}
		if primaryMinutes <= 360 && !short && secondaryOK {
			return secondary, true
		}
		if primaryMinutes > 360 && short && secondaryOK {
			return secondary, true
		}
	} else if secondaryMinutesOK {
		if (secondaryMinutes <= 360) == short && secondaryOK {
			return secondary, true
		}
	}
...
func openAICanonicalCodexQuotaWindowReset(extra map[string]any, window string, now time.Time) bool {
	// A canonical value takes precedence over raw aliases, including its reset
	// state. This prevents an unrelated raw window from invalidating it.
	if _, ok := resolveAccountExtraNumber(extra, "codex_"+window+"_used_percent"); ok {
		return openAIQuotaWindowReset(extra, window, now)
	}
	short := window == "5h"
	primaryMinutes, primaryMinutesOK := resolveAccountExtraNumber(extra, "codex_primary_window_minutes")
	secondaryMinutes, secondaryMinutesOK := resolveAccountExtraNumber(extra, "codex_secondary_window_minutes")
	rawWindow := ""
	if primaryMinutesOK && (primaryMinutes <= 360) == short {
		rawWindow = "primary"
	}
	if rawWindow == "" && secondaryMinutesOK && (secondaryMinutes <= 360) == short {
		rawWindow = "secondary"
	}
	if rawWindow == "" && primaryMinutesOK {
		if (primaryMinutes <= 360) == short {
			rawWindow = "primary"
		} else if secondaryMinutesOK || short || !short {
			rawWindow = "secondary"
		}
	}
	if rawWindow == "" {
	...
	return openAIQuotaWindowReset(extra, rawWindow, now)
}
...
func openAISchedulingResetWindowEnd(account *Account, now time.Time) (time.Time, bool) {
	...
	resetAtRaw, _ := account.Extra["codex_5h_reset_at"].(string)
	if raw := strings.TrimSpace(resetAtRaw); raw != "" {
		if resetAt, err := time.Parse(time.RFC3339, raw); err == nil && resetAt.After(now) {
			return resetAt.UTC(), true
		}
	}
	updatedRawValue, _ := account.Extra["codex_usage_updated_at"].(string)
	updatedRaw := strings.TrimSpace(updatedRawValue)
	if updatedRaw == "" {
		return time.Time{}, false
	}
	...
	var seconds int64
	switch value := account.Extra["codex_5h_reset_after_seconds"].(type) {
	case int:
		seconds = int64(value)
	case int64:
		seconds = value
	...
	end := updatedAt.Add(time.Duration(seconds) * time.Second)
	if !end.After(now) {
		return time.Time{}, false
	}
	return end.UTC(), true
