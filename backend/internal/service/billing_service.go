...
type CostBreakdown struct {
	...
	// PricingVersion is the stable identifier of the effective pricing card
	// used for this calculation. Empty means the caller used a legacy path that
	// has not yet supplied a pricing snapshot.
...
const (
	...
	deepseekFlashLegacyInputPrice   = 2.2e-7  // $0.22 per MTok before 2026-09-10
	deepseekFlashLegacyOutputPrice  = 6.6e-7  // $0.66 per MTok before 2026-09-10
	deepseekFlashLegacyCacheRead    = 7e-9    // $0.007 per MTok before 2026-09-10
...
// deepseekFlashPriceCutoverAt is the UTC start of the 2026-09-10 official
// Flash price card. Historical usage before this point keeps the prior card.
var deepseekFlashPriceCutoverAt = time.Date(2026, 9, 10, 0, 0, 0, 0, time.UTC)
...
func (s *BillingService) GetModelPricing(model string) (*ModelPricing, error) {
	...
	return s.getModelPricingAt(model, s.billingNow())
}

func (s *BillingService) billingNow() time.Time {
	if s != nil && s.now != nil {
		return s.now().UTC()
	}
	return time.Now().UTC()
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	if input.Resolver == nil {
		...
		if err == nil {
			...
			if breakdown != nil && breakdown.PricingVersion == "" {
				breakdown.PricingVersion = BuildLegacyPricingSnapshotVersion(input.Model, nil)
			}
		}
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	if breakdown != nil {
		breakdown.PricingVersion = BuildPricingSnapshotVersion(
			input.Model,
			resolved.Source,
			resolved.Mode,
			pricing,
			resolved,
			resolvedChannelTimeMultiplier(resolved, input.PricingAt),
		)
	}
...
func (s *BillingService) calculatePerRequestCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	return &CostBreakdown{
		...
		PricingVersion: BuildPricingSnapshotVersion(
			input.Model,
			resolved.Source,
			resolved.Mode,
			nil,
...
func (s *BillingService) calculateCostInternalWithPolicy(
	...
	if breakdown != nil {
		breakdown.PricingVersion = BuildLegacyPricingSnapshotVersion(model, pricing)
	}
	return breakdown, nil
}
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	if forceDeepSeekRates && isDeepSeekModel(model) {
		...
		if isDeepSeekProModel(model) && !deepseekProBilledAsFlash(pricingAt) {
			...
			if !pricingAt.IsZero() && pricingAt.Before(deepseekFlashPriceCutoverAt) {
				cloned.InputPricePerToken = deepseekFlashLegacyInputPrice
				cloned.OutputPricePerToken = deepseekFlashLegacyOutputPrice
				cloned.CacheReadPricePerToken = deepseekFlashLegacyCacheRead
			} else {
...
func (s *BillingService) CalculateWebSearchCost(callCount int, groupPrice *float64, rateMultiplier float64) *CostBreakdown {
	...
	return &CostBreakdown{
		...
		PricingVersion: BuildAuxiliaryPricingSnapshotVersion("", BillingModePerRequest, source, "web_search", unitPrice),
	}
}
...
func (s *BillingService) CalculateSearchCost(numCalls int, groupPricePer1k *float64, rateMultiplier float64) *CostBreakdown {
	...
	if pricePer1k == 0 {
		return &CostBreakdown{PricingVersion: BuildAuxiliaryPricingSnapshotVersion("", BillingModePerRequest, source, "search_per_1k", pricePer1k)}
	}
	if rateMultiplier < 0 {
		rateMultiplier = 0
	}
	...
	return &CostBreakdown{
		...
		BillingMode:    string(BillingModePerRequest),
		PricingVersion: BuildAuxiliaryPricingSnapshotVersion("", BillingModePerRequest, source, "search_per_1k", pricePer1k),
	}
}
...
func (s *BillingService) CalculateAudioCost(mode string, durationOrUnits float64, groupConfig *audioPriceConfig, rateMultiplier float64) *CostBreakdown {
	...
	return &CostBreakdown{
		...
		ActualCost:     total * rateMultiplier,
		BillingMode:    string(BillingModePerRequest),
		PricingVersion: BuildAuxiliaryPricingSnapshotVersion("", BillingModePerRequest, source, strings.ToLower(mode), unitPrice),
	}
}
...
func (s *BillingService) CalculateImageCost(model string, imageSize string, imageCount int, groupConfig *ImagePriceConfig, rateMultiplier float64) *CostBreakdown {
	...
	source := PricingSourceFallback
	if imagePriceConfiguredForTier(groupConfig, imageSize) {
		source = PricingSourceGroup
	}
	...
	return &CostBreakdown{
		...
		PricingVersion: BuildAuxiliaryPricingSnapshotVersion(model, BillingModeImage, source, imageSize, unitPrice),
	}
}
...
func (s *BillingService) CalculateVideoCost(model string, resolution string, videoCount int, durationSeconds int, groupConfig *VideoPriceConfig, rateMultiplier float64) *CostBreakdown {
	...
	source := PricingSourceFallback
	if videoPriceConfiguredForResolution(groupConfig, resolution) {
		source = PricingSourceGroup
	}
	...
	return &CostBreakdown{
		...
		PricingVersion: BuildAuxiliaryPricingSnapshotVersion(model, BillingModeVideo, source, resolution, perSecondPrice),
	}
}

func imagePriceConfiguredForTier(config *ImagePriceConfig, tier string) bool {
	if config == nil {
		return false
	}
	switch NormalizeImageBillingTierOrDefault(tier) {
	case "1K":
		return config.Price1K != nil
	case "2K":
		return config.Price2K != nil
	case "4K":
		return config.Price4K != nil
	default:
		return false
	}
}

func videoPriceConfiguredForResolution(config *VideoPriceConfig, resolution string) bool {
	if config == nil {
		return false
	}
	switch NormalizeVideoBillingResolutionOrDefault(resolution) {
	case "480p":
		return config.Price480P != nil
	case "720p":
		return config.Price720P != nil
	case "1080p":
		return config.Price1080P != nil
	default:
		return false
	}
}
