...
// AdminGroupScopeService is an optional extension for installations that
// expose user-private and user-carpool groups in the admin group browser.
type AdminGroupScopeService interface {
	ListGroupsWithScope(ctx context.Context, page, pageSize int, platform, status, search string, isExclusive *bool, scope, sortBy, sortOrder string) ([]Group, int64, error)
	GetAllGroupsWithScope(ctx context.Context, scope string) ([]Group, error)
	GetAllGroupsByPlatformWithScope(ctx context.Context, platform, scope string) ([]Group, error)
	GetAllGroupsIncludingInactiveWithScope(ctx context.Context, scope string) ([]Group, error)
}

// AdminAccountQuotaDashboardService is an optional extension implemented by
// installations that expose the ANL account quota overview.
type AdminAccountQuotaDashboardService interface {
