...
func (s *OpenAIGatewayService) handleOpenAIImagesErrorResponse(
	...
	// A retired/configured Responses driver is not an image-model quota
	// failure. Surface the actionable upstream error instead of cooling every
	// image account and hiding the configuration problem behind a generic 503.
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	directImages := usesCodexDirectImages(upstreamModel) && !isOpenAIImagesForceResponses(ctx)
	logger.LegacyPrintf(
	...
	requestBody, targetURL, err := buildOpenAIImagesOAuthPayload(parsed, upstreamModel)
	if err != nil {
		return nil, err
	}
	upstreamCtx = withOpenAIImagesSelfBuiltRequest(upstreamCtx)
	upstreamReq, err := s.buildUpstreamRequest(upstreamCtx, c, account, requestBody, token, true, parsed.StickySessionSeed(), false)
	if err != nil {
		return nil, err
	}
	if directImages {
		if parsedURL, parseErr := url.Parse(targetURL); parseErr != nil {
			return nil, fmt.Errorf("parse direct images target URL: %w", parseErr)
		} else {
			upstreamReq.URL = parsedURL
		}
	...
	if resp.StatusCode >= 400 {
		...
		if directImages && (resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusMethodNotAllowed) {
			_ = resp.Body.Close()
	...
	writerSizeBeforeResponse := OpenAIImagesJSONKeepaliveAdjustedWrittenSize(c)
	var directUpstreamResponseModel string
	if directImages && parsed.Stream {
		usage, imageCount, imageOutputSizes, directUpstreamResponseModel, firstTokenMs, err = s.handleCodexDirectImagesStreamingResponse(resp, c, startTime, parsed)
		if err != nil {
			if imageCount > 0 {
				return &OpenAIForwardResult{RequestID: resp.Header.Get("x-request-id"), UpstreamHeaders: resp.Header, Usage: usage, Model: requestModel, UpstreamModel: upstreamModel, UpstreamResponseModel: directUpstreamResponseModel, UpstreamEndpoint: upstreamReq.URL.Path, Stream: parsed.Stream, ResponseHeaders: resp.Header.Clone(), Duration: time.Since(startTime), FirstTokenMs: firstTokenMs, ImageCount: imageCount, ImageSize: parsed.SizeTier, ImageInputSize: parsed.Size, ImageOutputSizes: imageOutputSizes}, err
			}
			return nil, err
		}
	} else if directImages {
	...
	return &OpenAIForwardResult{
		...
		Model:                 requestModel,
		UpstreamModel:         upstreamModel,
		UpstreamResponseModel: directUpstreamResponseModel,
