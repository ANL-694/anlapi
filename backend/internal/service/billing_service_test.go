...
func TestCalculateImageCost(t *testing.T) {
	...
	require.Equal(t, cost.PricingVersion, svc.CalculateImageCost("gpt-image-1", "1K", 1, cfg, 1.0).PricingVersion)
}
...
func TestCalculateVideoCostUsesSeparateConfig(t *testing.T) {
	...
	require.NotEmpty(t, videoCost.PricingVersion)
	require.Equal(t, videoCost.PricingVersion, svc.CalculateVideoCost("grok-imagine-video", "480p", 1, 1, &VideoPriceConfig{Price480P: &videoPrice}, 0.5).PricingVersion)
	require.NotEqual(t, videoCost.PricingVersion, svc.CalculateVideoCost("grok-imagine-video", "480p", 1, 10, &VideoPriceConfig{Price480P: func() *float64 { v := 0.09; return &v }()}, 0.5).PricingVersion)
}
...
func TestCalculateVideoCostBillsPerSecond(t *testing.T) {
	...
	require.NotEmpty(t, oneSecond.PricingVersion)
	require.Equal(t, oneSecond.PricingVersion, fifteenSeconds.PricingVersion)
	require.Equal(t, oneSecond.PricingVersion, defaultDuration.PricingVersion)
	require.Equal(t, oneSecond.PricingVersion, clampedDuration.PricingVersion)
}
