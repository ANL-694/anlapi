...
func expandAnthropicCompatResponsesInputToolCallStart(items []apicompat.ResponsesInputItem, start int) int {
	...
	for index := start; index < len(items); index++ {
		...
		callID := strings.TrimSpace(items[index].CallID)
		if callID != "" {
	...
	expandedStart := start
	for index := start - 1; index >= 0 && len(needed) > 0; index-- {
...
func isOpenAICompatPreviousResponseNotFound(statusCode int, upstreamMessage string, upstreamBody []byte) bool {
	...
	if check(upstreamMessage) || check(string(upstreamBody)) {
		return true
	}
...
func isOpenAICompatPreviousResponseUnsupported(statusCode int, upstreamMessage string, upstreamBody []byte) bool {
...
func (s *OpenAIGatewayService) bindOpenAICompatSessionResponseID(_ context.Context, c *gin.Context, account *Account, promptCacheKey, responseID string) {
	...
	binding := openAICompatSessionResponseBinding{ResponseID: id, ExpiresAt: time.Now().Add(s.openAIWSResponseStickyTTL())}
...
func (s *OpenAIGatewayService) disableOpenAICompatSessionContinuation(_ context.Context, c *gin.Context, account *Account, promptCacheKey string) {
	...
	binding := openAICompatSessionResponseBinding{ContinuationDisabled: true, ExpiresAt: time.Now().Add(s.openAIWSResponseStickyTTL())}
	if raw, ok := s.openaiCompatSessionResponses.Load(key); ok {
		if existing, ok := raw.(openAICompatSessionResponseBinding); ok {
			binding.TurnState = existing.TurnState
		}
...
func (s *OpenAIGatewayService) bindOpenAICompatSessionTurnState(_ context.Context, c *gin.Context, account *Account, promptCacheKey, turnState string) {
	...
	binding := openAICompatSessionResponseBinding{TurnState: state, ExpiresAt: time.Now().Add(s.openAIWSResponseStickyTTL())}
	if raw, ok := s.openaiCompatSessionResponses.Load(key); ok {
		if existing, ok := raw.(openAICompatSessionResponseBinding); ok {
