...
type UsageBillingCommand struct {
	...
	// PricingVersion identifies the effective pricing snapshot used for this
	// request. It is persisted in the idempotency ledger and may be empty for
	// historical or legacy callers.
	PricingVersion string

	// Share fields are immutable request-time snapshots. They let settlement
	// use the sharing state that was observed when the request ran instead of
	// re-evaluating a later account configuration.
...
func (c *UsageBillingCommand) Normalize() {
	...
	c.PricingVersion = strings.TrimSpace(c.PricingVersion)
