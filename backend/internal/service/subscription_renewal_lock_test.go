...
func (r *lockingRenewalRepo) GetByID(context.Context, int64) (*UserSubscription, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	copy := r.current
	return &copy, nil
...
func (r *lockingRenewalRepo) GetByIDForUpdate(context.Context, int64) (*UserSubscription, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.lockReads++
	copy := r.current
