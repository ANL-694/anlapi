...
func NewAuthService(
	...
	for _, dependency := range dependencies {
		if value, ok := dependency.(UserPlatformQuotaRepository); ok {
			svc.userPlatformQuotaRepo = value
		}
	}
	return svc
}
...
func (s *AuthService) canBypassRegistrationDisabledForOAuth(ctx context.Context, signupSource string) bool {
	if signupSource != "dingtalk" || s == nil || s.settingService == nil {
		return false
	}
...
func (s *AuthService) LoginOrRegisterOAuthWithTokenPair(ctx context.Context, email, username, invitationCode, affiliateCode string, signupSource ...string) (*TokenPair, *User, error) {
	source := ""
	if len(signupSource) > 0 {
		source = signupSource[0]
	}
	return s.loginOrRegisterOAuthWithTokenPair(ctx, email, username, invitationCode, affiliateCode, "", source)
}
...
func (s *AuthService) LoginOrRegisterOAuthWithTokenPairAndPromoCode(ctx context.Context, email, username, invitationCode, affiliateCode, promoCode string, signupSource ...string) (*TokenPair, *User, error) {
	source := ""
	if len(signupSource) > 0 {
		source = signupSource[0]
	}
	return s.loginOrRegisterOAuthWithTokenPair(ctx, email, username, invitationCode, affiliateCode, promoCode, source)
}
...
func (s *AuthService) resolveSignupGrantPlan(ctx context.Context, signupSource string) signupGrantPlan {
	...
	for platform, patch := range s.settingService.GetAuthSourcePlatformQuotas(ctx, signupSource) {
		if current := plan.PlatformQuotas[platform]; current != nil {
			mergePlatformQuotaDefaults(current, patch)
		}
	}
	return plan
}
...
// RevokeAllUserTokens revokes all refresh sessions for a user.
//
// The users table has no token_version column. Incrementing the in-memory
// service model would not invalidate stateless access tokens across requests,
// and writing that stale model back could overwrite unrelated user updates.
// Password changes invalidate access tokens through the password fingerprint.
