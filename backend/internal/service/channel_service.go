...
func channelLookupPlatform(ctx context.Context, groupID int64, groupPlatform string) string {
	if ctx != nil {
		...
		if groupPlatform == PlatformComposite {
			if platform, ok := ResolvedTargetPlatformForGroup(ctx, groupID); ok {
				return platform
			}
		}
	}
...
func (s *ChannelService) lookupGroupChannel(ctx context.Context, groupID int64) (*channelLookup, error) {
	...
	return &channelLookup{
		cache:    cache,
		channel:  ch,
		platform: channelLookupPlatform(ctx, groupID, cache.groupPlatform[groupID]),
	}, nil
}
