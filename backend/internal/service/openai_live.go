...
const (
	...
	liveControllerLeaseTTL        = time.Minute
	liveRedisOperationTimeout     = 3 * time.Second
	liveClosedRecordTTL           = 24 * time.Hour
	liveObserverPollInterval      = 250 * time.Millisecond
	liveObserverRetryInterval     = time.Second
	liveSidebandDialTimeout       = liveLeaseRefreshInterval
...
type liveRuntimeTiming struct {
	leaseRefreshInterval   time.Duration
	controllerPollInterval time.Duration
	observerRetryInterval  time.Duration
	sidebandDialTimeout    time.Duration
}

func (s *OpenAIGatewayService) liveTimings() liveRuntimeTiming {
	timing := liveRuntimeTiming{
		leaseRefreshInterval:   liveLeaseRefreshInterval,
		controllerPollInterval: liveObserverPollInterval,
		observerRetryInterval:  liveObserverRetryInterval,
		sidebandDialTimeout:    liveSidebandDialTimeout,
	}
	if s != nil {
		if s.liveTiming.leaseRefreshInterval > 0 {
			timing.leaseRefreshInterval = s.liveTiming.leaseRefreshInterval
		}
		if s.liveTiming.controllerPollInterval > 0 {
			timing.controllerPollInterval = s.liveTiming.controllerPollInterval
		}
		if s.liveTiming.observerRetryInterval > 0 {
			timing.observerRetryInterval = s.liveTiming.observerRetryInterval
		}
		if s.liveTiming.sidebandDialTimeout > 0 {
			timing.sidebandDialTimeout = s.liveTiming.sidebandDialTimeout
		}
	}
	if timing.sidebandDialTimeout > timing.leaseRefreshInterval {
		timing.sidebandDialTimeout = timing.leaseRefreshInterval
	}
	return timing
}
...
// 账号调度槽在服务层是 no-op，Live 租约只接替调用方持有的普通用户槽位。
...
func (s *OpenAIGatewayService) dialLiveSideband(ctx context.Context, record *LiveCallRecord) (liveFrameConn, error) {
	...
	dialCtx, cancel := context.WithTimeout(ctx, s.liveTimings().sidebandDialTimeout)
	defer cancel()
	account, err := s.accountRepo.GetByID(dialCtx, record.AccountID)
	if err != nil {
		return nil, err
	}
	...
	headers, err := s.liveSidebandHeaders(dialCtx, account, record)
	if err != nil {
		return nil, err
	}
	target := strings.TrimRight(chatGPTLiveSidebandBaseURL, "/") + "/" + url.PathEscape(record.CallID)
	conn, status, _, err := s.getOpenAIWSPassthroughDialer().Dial(dialCtx, target, headers, resolveAccountProxyURL(account))
	if err != nil {
...
func (s *OpenAIGatewayService) ProxyLiveSideband(
	...
	runErr := s.runLiveController(proxyCtx, record, upstream, owner, errCh)
	cancel()
	_, _ = store.ReleaseLiveController(context.Background(), record.CallHash, owner)
...
func (s *OpenAIGatewayService) runLiveController(
	...
	refreshTicker := time.NewTicker(s.liveTimings().leaseRefreshInterval)
	...
	for {
		...
		case <-refreshTicker.C:
			if !s.refreshLiveController(record.CallHash, owner) {
				return ErrLiveControllerChanged
			}
...
func (s *OpenAIGatewayService) observeLiveCall(callHash string) {
	...
	for {
		...
		if dialErr != nil {
			retryErr := s.waitForLiveObserverRetry(record, owner)
			if retryErr != nil {
				if liveSessionEnded(retryErr) {
					s.finalizeLiveCall(record)
				}
				return
			}
		...
		runErr := s.runLiveObserverConnection(record, upstream, owner)
		_ = upstream.Close()
...
func (s *OpenAIGatewayService) runLiveObserverConnection(record *LiveCallRecord, upstream liveFrameConn, owner string) error {
	...
	timing := s.liveTimings()
	refreshTicker := time.NewTicker(timing.leaseRefreshInterval)
	defer refreshTicker.Stop()
	controllerTicker := time.NewTicker(timing.controllerPollInterval)
...
func (s *OpenAIGatewayService) waitForLiveObserverRetry(record *LiveCallRecord, owner string) error {
	timing := s.liveTimings()
	retryAfter := timing.observerRetryInterval
	...
	defer timer.Stop()
	controllerTicker := time.NewTicker(min(timing.controllerPollInterval, retryAfter))
	defer controllerTicker.Stop()
	leaseTicker := time.NewTicker(min(timing.leaseRefreshInterval, retryAfter))
...
func (s *OpenAIGatewayService) refreshLiveController(callHash, owner string) bool {
	store, err := s.liveStore()
	if err != nil {
		return false
	}
	...
	defer cancel()
	refreshed, err := store.RefreshLiveController(ctx, callHash, owner, liveControllerLeaseTTL)
