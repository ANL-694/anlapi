...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	for {
		...
		_, currentPreviousResponseIDSeen := seenResponseIDs[currentPreviousResponseID]
		if turn > 1 && currentPreviousResponseIDSeen && expectedPrev != "" && currentPreviousResponseID != expectedPrev {
			updatedPayload, setPrevErr := setPreviousResponseIDToRawPayload(currentPayload, expectedPrev)
			if setPrevErr != nil {
				logOpenAIWSModeInfo(
					"ingress_ws_prev_response_repair_skip account_id=%d turn=%d conn_id=%s previous_response_id=%s expected_previous_response_id=%s cause=%s",
					account.ID,
					turn,
					truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
				...
				logOpenAIWSModeInfo(
					"ingress_ws_prev_response_repair account_id=%d turn=%d conn_id=%s previous_response_id=%s repaired_previous_response_id=%s",
					account.ID,
					turn,
					truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
					truncateOpenAIWSLogValue(currentPreviousResponseID, openAIWSIDValueMaxLen),
