...
func TestOpenAIGatewayServiceRecordUsage_DeepSeekUsesOfficialPricingAcrossRequestTimes(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, userRepo, subRepo, nil)
	// The request timestamp is still propagated for ANL billing policies, but
	// official provider cost cards remain fixed until the official pricing data
	// is updated.
	svc.billingService.now = func() time.Time {
		return time.Date(2026, time.August, 16, 16, 0, 1, 0, time.UTC)
	}
	requestPricingAt := time.Date(2026, time.August, 16, 15, 59, 59, 0, time.UTC)

	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "deepseek-pricing-at-boundary",
	...
	require.NoError(t, err)
	require.NotNil(t, usageRepo.lastLog)
	require.InDelta(t, 2.2e-7*1000+6.6e-7*1000, usageRepo.lastLog.TotalCost, 1e-12)
	require.InDelta(t, (2.2e-7*1000+6.6e-7*1000)*1.1, usageRepo.lastLog.ActualCost, 1e-12)
}

func TestOpenAIGatewayServiceRecordUsage_DeepSeekPricingAtFlowsThroughResolver(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, userRepo, subRepo, nil)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "deepseek-pricing-at-resolver",
			Model:     "deepseek-v4-flash",
			Usage: OpenAIUsage{
				InputTokens:  1000,
				OutputTokens: 1000,
