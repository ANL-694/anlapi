...
func TestParseAccountCredentialImportContentsRejectsClaudeWebOn176Base(t *testing.T) {
	content := `{"cookies":{"sessionKey":"sk-ant-sid02-first"}}`
	sources, errs := ParseAccountCredentialImportContentsWithOptions([]string{content}, AccountCredentialImportOptions{
		ClaudeWebImport: true,
	})
	require.Empty(t, sources)
	require.Len(t, errs, 1)
	require.Contains(t, errs[0].Message, "Claude Web credential import is not supported")
}
