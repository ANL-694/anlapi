...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result.UserPrivateGroupRateMultiplier = parseNonNegativeSettingFloat(settings[SettingKeyUserPrivateGroupRateMultiplier], 1)
	if result.UserPrivateGroupRateMultiplier <= 0 {
	...
	result.UserPrivateGroupCommissionRate = parseNonNegativeSettingFloat(settings[SettingKeyUserPrivateGroupCommissionRate], 0.005)
	...
	if effort := strings.TrimSpace(settings[SettingKeyOpenAIImagesResponsesReasoningEffort]); IsValidOpenAIImagesResponsesReasoningEffort(effort) {
		result.OpenAIImagesResponsesReasoningEffort = NormalizeOpenAIImagesResponsesReasoningEffort(effort)
	}
	...
	if value, err := strconv.ParseFloat(strings.TrimSpace(settings[SettingKeyOpenAIFreeAccountRepairWeeklyThresholdUSD]), 64); err == nil && value > 0 && !math.IsNaN(value) && !math.IsInf(value, 0) {
		result.OpenAIFreeAccountRepairWeeklyThresholdUSD = value
	} else {
...
func mergeProviderDefaultGrantSettings(globalDefaults ProviderDefaultGrantSettings, providerDefaults ProviderDefaultGrantSettings) ProviderDefaultGrantSettings {
	...
	// auth source 的持久化默认 concurrency 为 5；它表示该来源未覆盖全局默认值。
	// 只有偏离来源默认值时才覆盖 globalDefaults，否则新用户会被无意间固定为 5。
