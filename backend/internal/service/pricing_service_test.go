...
func TestPricingService_OfficialCatalogWinsOverFallbackPrice(t *testing.T) {
	dir := t.TempDir()
	fallbackFile := filepath.Join(dir, "fallback.json")
	require.NoError(t, os.WriteFile(fallbackFile, []byte(`{
		"deepseek-v4-flash": {
			...
			"litellm_provider": "legacy-anl-fallback",
			"mode": "chat"
		},
		"fallback-only-model": {
	...
	merged, _, err := svc.buildPricingData([]byte(`{
		"deepseek-v4-flash": {
			...
			"litellm_provider": "official-catalog",
			"mode": "chat"
		}
	}`))
	require.NoError(t, err)

	// A fallback file may add missing cards, but it must never replace the
	// official card when the upstream catalog already contains the model.
	require.InDelta(t, 0.0000022, merged["deepseek-v4-flash"].InputCostPerToken, 1e-12)
	require.InDelta(t, 0.0000066, merged["deepseek-v4-flash"].OutputCostPerToken, 1e-12)
	require.Equal(t, "official-catalog", merged["deepseek-v4-flash"].LiteLLMProvider)
	require.InDelta(t, 0.000003, merged["fallback-only-model"].InputCostPerToken, 1e-12)
}
...
func TestGetModelPricing_GPTImage25UsesOfficialFallbackOnlyWhenCatalogMissing(t *testing.T) {
	const model = "gpt-image-2.5-flare"

	missing := &PricingService{pricingData: map[string]*LiteLLMModelPricing{}}
	got := missing.GetModelPricing(model)
	require.NotNil(t, got)
	...
	precise := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		model: {InputCostPerToken: 1.1e-6, OutputCostPerToken: 2.2e-6},
	}}
	got = precise.GetModelPricing(model)
	require.NotNil(t, got)
	require.InDelta(t, 1.1e-6, got.InputCostPerToken, 1e-12)
	require.InDelta(t, 2.2e-6, got.OutputCostPerToken, 1e-12)
}
