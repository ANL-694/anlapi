...
func GroupAccessAllowedForUser(group *Group, userID int64) bool {
	if group == nil || userID <= 0 {
		return false
	}
	switch NormalizeGroupScope(group.Scope) {
	case GroupScopeUserPrivate, GroupScopeUserCarpool:
		return group.OwnerUserID != nil && *group.OwnerUserID == userID
	case GroupScopePublic:
		return group.OwnerUserID == nil
	default:
		return false
	}
}
...
func (g *Group) IsUserPrivateScope() bool {
	return g != nil && NormalizeGroupScope(g.Scope) == GroupScopeUserPrivate
}

func (g *Group) IsUserCarpoolScope() bool {
	return g != nil && NormalizeGroupScope(g.Scope) == GroupScopeUserCarpool
}
