...
func (s *adminServiceImpl) ListGroupsWithScope(ctx context.Context, page, pageSize int, platform, status, search string, isExclusive *bool, scope, sortBy, sortOrder string) ([]Group, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	...
	if scope != "" && !strings.EqualFold(strings.TrimSpace(scope), "all") {
...
func (s *adminServiceImpl) GetAllGroupsWithScope(ctx context.Context, scope string) ([]Group, error) {
...
func (s *adminServiceImpl) GetAllGroupsByPlatformWithScope(ctx context.Context, platform, scope string) ([]Group, error) {
...
func (s *adminServiceImpl) GetAllGroupsIncludingInactiveWithScope(ctx context.Context, scope string) ([]Group, error) {
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	var accountIDsToCopy []int64
	if len(input.CopyAccountsFromGroupIDs) > 0 {
		// Resolve and validate all source data before mutating the destination.
		// The production repository then applies the group update and binding
	...
	if len(input.CopyAccountsFromGroupIDs) > 0 {
		atomicRepo, ok := s.groupRepo.(AtomicGroupBindingRepository)
		if ok {
			if err := atomicRepo.UpdateWithAccountBindings(ctx, group, accountIDsToCopy); err != nil {
				return nil, err
			}
		} else {
			// Narrow in-memory test doubles do not expose transaction plumbing. A
			// real service must always be wired with the atomic repository.
			if s.entClient != nil {
				return nil, errors.New("atomic group binding repository is not configured")
			}
