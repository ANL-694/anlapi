...
const (
	...
	BillingModeUnmetered  BillingMode = "unmetered"
)
...
func (m BillingMode) IsValidUsageFilter() bool {
	switch m {
	case BillingModeToken, BillingModePerRequest, BillingModeImage, BillingModeVideo, BillingModeUnmetered, "":
		return true
	}
	return false
}

func ParseBillingMode(raw string) (string, error) {
	mode := BillingMode(strings.ToLower(strings.TrimSpace(raw)))
	if mode.IsValidUsageFilter() {
		return string(mode), nil
	}
	return "", fmt.Errorf("invalid billing_mode, allowed values: token, per_request, image, video, unmetered")
}
