...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	if settings.UserPrivateGroupCommissionRate < 0 || math.IsNaN(settings.UserPrivateGroupCommissionRate) || math.IsInf(settings.UserPrivateGroupCommissionRate, 0) {
		settings.UserPrivateGroupCommissionRate = 0.005
	}
...
func (s *SettingService) validateHomeStatsGroup(ctx context.Context, groupID int64) error {
	...
	if err != nil {
		if errors.Is(err, ErrGroupNotFound) {
			return infraerrors.BadRequest("HOME_STATS_GROUP_INVALID", "home stats group must be an administrator public group").WithMetadata(map[string]string{
...
func (s *SettingService) validateSystemImageGenerationGroup(ctx context.Context, groupID int64) error {
	...
	if err != nil {
		if errors.Is(err, ErrGroupNotFound) {
			return infraerrors.BadRequest("SYSTEM_IMAGE_GENERATION_GROUP_INVALID", "system image generation group must be an active public OpenAI image group").WithMetadata(map[string]string{
				"group_id": strconv.FormatInt(groupID, 10),
			})
		}
	...
	if !isAdministratorPublicGroup(group) || !group.IsActive() ||
		!strings.EqualFold(strings.TrimSpace(group.Platform), PlatformOpenAI) || !group.AllowImageGeneration {
		return infraerrors.BadRequest("SYSTEM_IMAGE_GENERATION_GROUP_INVALID", "system image generation group must be an active public OpenAI image group").WithMetadata(map[string]string{
			"group_id": strconv.FormatInt(groupID, 10),
		})
	}
