...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if account.OwnerUserID != nil {
		if len(groupIDs) == 0 {
			return nil, errors.New("owned accounts must be created with a user-private group")
		}
		if s.accountDuplicateRepo == nil {
			return nil, errors.New("account group transaction repository is not configured")
		}
		...
		// Keep the official public-account creation path unchanged.
		if len(groupIDs) > 0 {
