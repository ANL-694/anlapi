...
// UserPrivateGroupRepository is intentionally a narrow optional capability.
// It keeps the official GroupRepository test doubles/source contract stable
// while exposing only B07b ownership queries to the private-group flow.
type UserPrivateGroupRepository interface {
...
// AtomicGroupBindingRepository updates a group and replaces its account
// bindings in one durable transaction. The capability is optional so narrow
// unit-test doubles do not need to model database transaction plumbing; the
// production repository implements it for admin copy-account updates.
type AtomicGroupBindingRepository interface {
	UpdateWithAccountBindings(ctx context.Context, group *Group, accountIDs []int64) error
}
...
// EnsureUserPrivateGroup returns the durable per-user/per-platform private
// group, creating it through the repository's unique constraint when absent.
// It is an internal provisioning primitive; no public self-service route is
// implied by this method.
func (s *GroupService) EnsureUserPrivateGroup(ctx context.Context, userID int64, platform string) (*Group, error) {
	if userID <= 0 {
	...
	repo, ok := s.groupRepo.(UserPrivateGroupRepository)
	if !ok {
		return nil, fmt.Errorf("private group repository is not configured")
	}
	if existing, err := repo.FindUserPrivateByOwnerAndPlatform(ctx, userID, platform); err == nil {
		return existing, nil
	} else if !errors.Is(err, ErrGroupNotFound) {
		return nil, err
	}
	private := &Group{
		Name:             fmt.Sprintf("user-private-%s-%d", platform, userID),
		Description:      "",
		Platform:         platform,
		RateMultiplier:   1,
		Status:           StatusActive,
	...
	if err := s.groupRepo.Create(ctx, private); err != nil {
		if errors.Is(err, ErrGroupExists) {
			return repo.FindUserPrivateByOwnerAndPlatform(ctx, userID, platform)
		}
		return nil, err
	}
	return private, nil
...
// ListActiveVisibleToUser exposes only public groups allowed by the official
// subscription surface plus the caller's own private groups.
func (s *GroupService) ListActiveVisibleToUser(ctx context.Context, userID int64, subscribedGroupIDs []int64) ([]Group, error) {
	repo, ok := s.groupRepo.(UserPrivateGroupRepository)
	if !ok {
		return nil, fmt.Errorf("private group repository is not configured")
	}
	return repo.ListActiveVisibleToUser(ctx, userID, subscribedGroupIDs)
}
