...
// Some OpenAI-compatible upstreams (including other compatible gateway instances)
...
func (s *OpenAIGatewayService) handleSSEToJSON(args ...any) (*openaiNonStreamingResult, error) {
	...
	case 7:
		resp, _ = args[1].(*http.Response)
		c, _ = args[2].(*gin.Context)
		body, _ = args[4].([]byte)
		originalModel, _ = args[5].(string)
		mappedModel, _ = args[6].(string)
	default:
		return nil, fmt.Errorf("invalid OpenAI SSE-to-JSON arguments")
	}
	if resp == nil || c == nil {
		return nil, fmt.Errorf("invalid OpenAI SSE-to-JSON response context")
	}
