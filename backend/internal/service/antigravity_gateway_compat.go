...
func (s *AntigravityGatewayService) buildAntigravityCompatGeminiBody(
	...
	// The compatibility endpoint must prefer client functions when a request
	// mixes them with server-side built-ins. The generic transformer keeps both
	// for native Gemini callers, so apply the endpoint-specific policy here.
...
func enableMixedGeminiToolInvocations(body []byte) ([]byte, error) {
	...
	toolContainer := request
	if nested, ok := request["request"].(map[string]any); ok {
		toolContainer = nested
	}
	tools, ok := toolContainer["tools"].([]any)
	if !ok || len(tools) == 0 {
		return body, nil
	}
	...
	toolContainer["tools"] = filtered
	if toolConfig, ok := toolContainer["toolConfig"].(map[string]any); ok {
		delete(toolConfig, "includeServerSideToolInvocations")
		delete(toolConfig, "include_server_side_tool_invocations")
		if len(toolConfig) == 0 {
			delete(toolContainer, "toolConfig")
		}
	}
