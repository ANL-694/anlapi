...
func (s *OpenAIGatewayService) handleCodexDirectImagesStreamingResponse(resp *http.Response, c *gin.Context, startTime time.Time, parsed *OpenAIImagesRequest) (OpenAIUsage, int, []string, string, *int, error) {
	responseheaders.WriteFilteredHeaders(c.Writer.Header(), resp.Header, s.responseHeaderFilter)
	...
	format := strings.TrimSpace(parsed.ResponseFormat)
	if format == "" {
		format = "b64_json"
	}
	usage := OpenAIUsage{}
	...
	for reader.Scan() {
		...
		if err := openAIImagesUpstreamErrorFromSSEPayload(data); err != nil {
			if !clientDisconnected {
				s.tryWriteOpenAIImagesStreamEvent(c, flusher, &clientDisconnected, nil, "error", buildOpenAIImagesStreamErrorBodyFromUpstream(err))
			}
			return usage, len(seen), sizes, upstreamModel, firstTokenMs, err
		}
		...
		if typ == "image_generation.partial_image" || typ == "image_edit.partial_image" {
			b64 := strings.TrimSpace(gjson.GetBytes(data, "b64_json").String())
			if b64 == "" {
				continue
			}
			meta := openAIResponsesImageResult{Model: gjson.GetBytes(data, "model").String(), Size: gjson.GetBytes(data, "size").String(), OutputFormat: gjson.GetBytes(data, "output_format").String()}
			payload := buildOpenAIImagesStreamPartialPayload(prefix+".partial_image", b64, gjson.GetBytes(data, "partial_image_index").Int(), format, time.Now().Unix(), meta)
			s.tryWriteOpenAIImagesStreamEvent(c, flusher, &clientDisconnected, nil, prefix+".partial_image", payload)
			continue
		}
		...
		if _, exists := seen[b64]; exists {
			continue
		}
		seen[b64] = struct{}{}
		img := openAIResponsesImageResult{Result: b64, Model: gjson.GetBytes(data, "model").String(), Size: gjson.GetBytes(data, "size").String(), OutputFormat: gjson.GetBytes(data, "output_format").String(), Quality: gjson.GetBytes(data, "quality").String(), Background: gjson.GetBytes(data, "background").String()}
		usageRaw := []byte(gjson.GetBytes(data, "usage").Raw)
		payload := buildOpenAIImagesStreamCompletedPayload(prefix+".completed", img, format, time.Now().Unix(), usageRaw)
		s.tryWriteOpenAIImagesStreamEvent(c, flusher, &clientDisconnected, nil, prefix+".completed", payload)
		if img.Size != "" {
	...
	if !completed {
		...
		if !clientDisconnected {
			s.tryWriteOpenAIImagesStreamEvent(c, flusher, &clientDisconnected, nil, "error", buildOpenAIImagesStreamErrorBody(err.Error()))
		}
		return usage, len(seen), sizes, upstreamModel, firstTokenMs, err
	}
	return usage, len(seen), sizes, upstreamModel, firstTokenMs, nil
}
