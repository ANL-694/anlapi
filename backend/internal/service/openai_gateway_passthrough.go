...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	// 终态收口与非透传路径一致：统一当前 Codex 版本和官方身份。
	if account.Type == AccountTypeOAuth {
...
func shouldFailoverOpenAIPassthroughResponse(account *Account, statusCode int, responseBody []byte) bool {
	...
	if isOpenAIModelCapacityError(statusCode, "", responseBody) {
		return true
	}
...
// sanitizeOpenAICapacityShedErrorCodeForClient maps capacity-shed codes that
// Codex treats as fatal to a retryable server error after gateway failover is
// no longer possible. Monitoring and account state continue using the original payload.
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" {
				...
				s.persistOpenAIModelCapacitySignal(ctx, account, resp.Header, dataBytes, failedMessage, mappedModel)
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthrough(
	...
	default:
		return nil, fmt.Errorf("invalid OpenAI passthrough response arguments")
	}
...
// Some upstreams (e.g. other compatible gateway instances) may return SSE even when
