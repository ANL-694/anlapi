...
func TestGrokFreeClientToolCacheRequestOptInDoesNotOverridePaidTier(t *testing.T) {
...
func TestGrokFreeRequestClientSearchFunctionUsesExplicitRequestOptIn(t *testing.T) {
...
func TestGrokFreeMessagesFunctionToolCacheRouteRequiresKnownFreeTier(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.wantMix {
				if tt.account.Extra == nil {
					tt.account.Extra = make(map[string]any)
				}
				tt.account.Extra[grokClientToolCacheOptInExtraKey] = true
			}
