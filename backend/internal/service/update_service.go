...
const (
	...
	defaultDockerImage = "weishaw/sub2api"
...
// repository is optional for compatibility with existing callers. An empty
// value deliberately keeps the official Sub2API release source.
func NewUpdateService(cache UpdateCache, githubClient GitHubReleaseClient, version, buildType string, repository ...string) *UpdateService {
	repo := defaultGitHubRepo
	if len(repository) > 0 && strings.TrimSpace(repository[0]) != "" {
		repo = strings.TrimSpace(repository[0])
	}
	image := defaultDockerImage
	if len(repository) > 1 && strings.TrimSpace(repository[1]) != "" {
		image = strings.TrimSpace(repository[1])
	}
