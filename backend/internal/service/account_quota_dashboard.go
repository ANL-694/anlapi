...
// UserAccountQuotaPoolDashboard separates a user's own pool from accounts
// explicitly approved for the platform-wide shared pool.
...
// GetQuotaPoolDashboard returns only the authenticated user's owned accounts
// plus explicitly public-share-approved platform accounts.
func (s *AccountService) GetQuotaPoolDashboard(ctx context.Context, ownerUserID int64) (*UserAccountQuotaPoolDashboard, error) {
	...
	now := time.Now().UTC()
	mine := newAccountQuotaDashboardBuilder(now)
	mineGroups := newAccountQuotaGroupDashboardBuilder(now)
	platform := newAccountQuotaDashboardBuilder(now)
	platformGroups := newAccountQuotaGroupDashboardBuilder(now)

	visit := func(account Account) {
	...
	if err := visitAccountQuotaDashboardAccounts(ctx, s.accountRepo, visit); err != nil {
		return nil, err
	}
...
func (b *accountQuotaGroupDashboardBuilder) addAccountToGroup(account Account, groupID *int64, groupName, groupStatus, platform string, requireOAuthOnly, requirePrivacySet bool) {
	...
	acc.addAccount(account, b.generatedAt, accountSchedulableInQuotaGroup(account, b.generatedAt, groupStatus, platform, requireOAuthOnly, requirePrivacySet))
}
...
func accountSchedulableInQuotaGroup(account Account, now time.Time, groupStatus, groupPlatform string, requireOAuthOnly, requirePrivacySet bool) bool {
	...
	if requireOAuthOnly && account.Type != AccountTypeOAuth && account.Type != AccountTypeSetupToken {
		return false
	}
