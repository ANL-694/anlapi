...
func TransformClaudeToGeminiWithOptions(claudeReq *ClaudeRequest, projectID, mappedModel string, opts TransformOptions) ([]byte, error) {
	...
	hasClientFunctionTool := hasClientFunctionTool(claudeReq.Tools)
	requestType := "agent"
	targetModel := mappedModel
	if hasWebSearchTool && !hasClientFunctionTool {
...
// stripClaudeAttribution removes the leading Claude Code attribution metadata
// line from Antigravity system text. It is prompt metadata rather than an HTTP
// header and must not be forwarded to the Google upstream.
...
func hasClientFunctionTool(tools []ClaudeTool) bool {
	for _, tool := range tools {
