...
func buildChatMessagesFromItems(messages []ChatMessage, rawItems []json.RawMessage, opts *ResponsesToChatOptions) ([]ChatMessage, toolOutputMediaByCallID, error) {
	...
	for _, raw := range rawItems {
		...
		rawRole := strings.TrimSpace(rawString(item["role"]))
		...
		if strings.EqualFold(rawRole, "developer") || strings.EqualFold(rawRole, "system") {
			// Responses permits developer/system items in the input history, but
			// Chat Completions only has a reliable system slot before the
			// conversation starts. Merge leading policy text into the existing
			// system message; once a non-system message exists, preserve order by
			// representing the item as a user turn.
			leadingSystem := true
			...
			if leadingSystem {
				if len(messages) > 0 && messages[len(messages)-1].Role == "system" {
					messages[len(messages)-1].Content = mergeLeadingSystemContent(messages[len(messages)-1].Content, chatContent)
				} else {
					messages = append(messages, ChatMessage{Role: "system", Content: chatContent})
				}
				pendingReasoning = ""
				continue
			}
...
func mergeLeadingSystemContent(first, second json.RawMessage) json.RawMessage {
	var firstText, secondText string
	if err := json.Unmarshal(first, &firstText); err != nil {
		firstText = strings.TrimSpace(string(first))
	}
	if err := json.Unmarshal(second, &secondText); err != nil {
		secondText = strings.TrimSpace(string(second))
	}
	merged, _ := json.Marshal(strings.TrimSpace(firstText) + "\n\n" + strings.TrimSpace(secondText))
	return merged
}
