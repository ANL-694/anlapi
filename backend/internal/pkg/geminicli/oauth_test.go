...
func TestBuildAuthorizationURL_RequiresBuiltinSecretFallbackConfiguration(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "GEMINI_CLI_OAUTH_CLIENT_SECRET_MISSING") {
		t.Fatalf("缺少内置 client_secret 时应明确报错，实际错误: %v", err)
	}
}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "GEMINI_CLI_OAUTH_CLIENT_SECRET_MISSING") {
		t.Fatalf("不设置环境变量时应明确提示缺少内置 secret，实际错误: %v", err)
	}
}
