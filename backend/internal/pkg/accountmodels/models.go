// Package accountmodels derives the testable model set from an account.
...
// ForAccount is shared by the administrative and owner-scoped account APIs so
// both surfaces always use the same official model-resolution rules.
func ForAccount(account *service.Account) any {
	if account == nil {
		return []any{}
	}
