...
// GetQuotaDashboard returns the administrator's account quota overview.
// GET /api/v1/admin/accounts/quota-dashboard
func (h *AccountHandler) GetQuotaDashboard(c *gin.Context) {
	quotaService, ok := h.adminService.(service.AdminAccountQuotaDashboardService)
	if !ok {
		response.Error(c, http.StatusServiceUnavailable, "Account quota dashboard is not available")
		return
	}

	dashboard, err := quotaService.GetAccountQuotaDashboard(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
