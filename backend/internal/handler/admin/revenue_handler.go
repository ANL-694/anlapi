...
func (h *RevenueHandler) GetSummary(c *gin.Context) {
	startTime, endTime, err := parseRevenueTimeRange(c)
	if err != nil {
...
func parseRevenueTimeRange(c *gin.Context) (time.Time, time.Time, error) {
	...
	if startDate := strings.TrimSpace(c.Query("start_date")); startDate != "" {
		parsed, err := tzpkg.ParseInUserLocation("2006-01-02", startDate, userTZ)
		if err != nil {
	...
	if endDate := strings.TrimSpace(c.Query("end_date")); endDate != "" {
		parsed, err := tzpkg.ParseInUserLocation("2006-01-02", endDate, userTZ)
		if err != nil {
