...
func (repo *ollamaCloudUsageHandlerTestRepo) GetByID(_ context.Context, id int64) (*service.Account, error) {
	if repo.account != nil && repo.account.ID == id {
...
func (repo *ollamaCloudUsageHandlerTestRepo) ListOllamaCloudUsageGroupAccounts(_ context.Context, _ []*service.Account) ([]service.Account, error) {
	repo.groupResolveCalls++
	result := make([]service.Account, 0, len(repo.accounts)+1)
	if repo.account != nil {
		result = append(result, *repo.account)
	}
	for _, account := range repo.accounts {
...
func (repo *ollamaCloudUsageHandlerTestRepo) SaveOllamaCloudUsageSession(context.Context, *service.Account, string, bool) error {
	return nil
}

func (repo *ollamaCloudUsageHandlerTestRepo) DeleteOllamaCloudUsageSession(context.Context, *service.Account) error {
	return nil
}

func (repo *ollamaCloudUsageHandlerTestRepo) SetOllamaCloudUsageAutoRefresh(context.Context, *service.Account, bool) error {
	return nil
}

func (repo *ollamaCloudUsageHandlerTestRepo) UpdateOllamaCloudUsageSnapshot(context.Context, *service.Account, *service.OllamaCloudUsageSnapshot) error {
	return nil
}
...
func newOllamaCloudUsageHandlerTestService(t *testing.T) *service.OllamaCloudUsageService {
	t.Helper()
	usageService := service.NewOllamaCloudUsageService(nil, nil, nil, nil, false)
...
func TestOllamaCloudUsageHandlersValidateRequestsAndDependencies(t *testing.T) {
	...
	usageService := newOllamaCloudUsageHandlerTestService(t)
	...
	t.Run("invalid account id", func(t *testing.T) {
		...
		(&AccountHandler{ollamaCloudUsage: usageService}).GetOllamaCloudUsage(ctx)
	...
	t.Run("empty session", func(t *testing.T) {
		...
		(&AccountHandler{ollamaCloudUsage: usageService}).SaveOllamaCloudUsageSession(ctx)
		require.Equal(t, http.StatusBadRequest, recorder.Code)
	})
	...
	t.Run("missing enabled", func(t *testing.T) {
		...
		(&AccountHandler{ollamaCloudUsage: usageService}).SetOllamaCloudUsageAutoRefresh(ctx)
		require.Equal(t, http.StatusBadRequest, recorder.Code)
	})
...
func TestOllamaCloudUsageEncryptionKeyStateConsistentAcrossAccountResponses(t *testing.T) {
	...
	for _, configured := range []bool{false, true} {
		t.Run("configured="+strconv.FormatBool(configured), func(t *testing.T) {
			...
			handler := NewAccountHandler(adminService)
