...
// GetRollbackVersions lists the releases currently eligible for rollback.
...
// Rollback restores the local backup or a specific eligible release.
...
func (h *SystemHandler) Rollback(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.system.rollback", payload, service.DefaultSystemOperationIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		if targetVersion != "" {
			...
			defer cancel()
			rollbackErr = h.updateSvc.RollbackToVersion(rollbackCtx, targetVersion)
		} else {
