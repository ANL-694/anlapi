...
func TestLiveSidebandLocationMatchesCreateRoute(t *testing.T) {
	...
	require.Equal(
		t,
		"/v1/realtime?call_id=call_123%2Fchild",
		liveSidebandLocation("/v1/realtime/sessions", "call_123/child"),
	)
	require.Equal(
		t,
...
func TestLiveSidebandCallID(t *testing.T) {
	gin.SetMode(gin.TestMode)

	t.Run("path parameter takes precedence", func(t *testing.T) {
		context, _ := gin.CreateTestContext(httptest.NewRecorder())
		context.Request = httptest.NewRequest("GET", "/v1/live/path-call?call_id=query-call", nil)
		context.Params = gin.Params{{Key: "call_id", Value: " path-call "}}
		require.Equal(t, "path-call", liveSidebandCallID(context))
	})

	t.Run("query parameter is supported", func(t *testing.T) {
		context, _ := gin.CreateTestContext(httptest.NewRecorder())
		context.Request = httptest.NewRequest("GET", "/v1/realtime?call_id=%20query-call%20", nil)
		require.Equal(t, "query-call", liveSidebandCallID(context))
	})

	t.Run("missing call id is empty", func(t *testing.T) {
		context, _ := gin.CreateTestContext(httptest.NewRecorder())
		context.Request = httptest.NewRequest("GET", "/v1/realtime", nil)
		require.Empty(t, liveSidebandCallID(context))
	})
}
...
func TestLiveAttestationErrorIsExplicit(t *testing.T) {
	...
	(&OpenAIGatewayHandler{}).writeLiveCreateError(context, &service.LiveAttestationUnavailableError{
		Reason: "Live attestation is only supported when ANLAPI runs on macOS",
	})

	require.Equal(t, http.StatusServiceUnavailable, recorder.Code)
	require.Contains(t, recorder.Body.String(), "ANLAPI runs on macOS")
}
