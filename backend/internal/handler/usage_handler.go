...
func (h *UsageHandler) parseUserUsageFilters(c *gin.Context, requireRange bool) (*userUsageFilters, bool) {
	...
	if startPtr != nil && endPtr != nil && !endTime.After(startTime) {
...
// DashboardAccountSharing returns owner-scoped sharing statistics.
...
func (h *UsageHandler) DashboardAccountSharing(c *gin.Context) {
	...
	result, err := h.usageService.GetAccountSharingDashboard(c.Request.Context(), service.AccountSharingQuery{
		UserID: subject.UserID, StartTime: startTime, EndTime: endTime,
		Granularity: granularity, Page: page, PageSize: pageSize,
	})
	if err != nil {
		response.ErrorFrom(c, err)
		return
