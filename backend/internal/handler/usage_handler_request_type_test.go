...
type userErrorOpsRepoCapture struct {
	service.OpsRepository
	listCalls int
	filter    *service.OpsErrorLogFilter
}

func (s *userErrorOpsRepoCapture) ListErrorLogs(ctx context.Context, filter *service.OpsErrorLogFilter) (*service.OpsErrorLogList, error) {
	s.listCalls++
	s.filter = filter
...
type userErrorSettingRepoCapture struct {
	service.SettingRepository
}

func (s *userErrorSettingRepoCapture) GetMultiple(ctx context.Context, keys []string) (map[string]string, error) {
	return map[string]string{
		service.SettingKeyAllowUserViewErrorRequests: "true",
	}, nil
}
...
func newUserErrorListTestRouter(opsRepo *userErrorOpsRepoCapture) *gin.Engine {
	gin.SetMode(gin.TestMode)
	opsService := service.NewOpsService(opsRepo, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	settingService := service.NewSettingService(&userErrorSettingRepoCapture{}, nil)
	...
	router.GET("/usage/errors", handler.ListErrors)
	return router
}

func TestUserErrorListRejectsReversedDateRange(t *testing.T) {
	opsRepo := &userErrorOpsRepoCapture{}
	router := newUserErrorListTestRouter(opsRepo)

	req := httptest.NewRequest(http.MethodGet, "/usage/errors?start_date=2026-03-08&end_date=2026-03-01", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)

	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Contains(t, rec.Body.String(), "end_date must be greater than or equal to start_date")
	require.Zero(t, opsRepo.listCalls, "repository must not be called for an invalid date range")
}

func TestUserErrorListScopesValidDateRangeToCurrentUser(t *testing.T) {
	opsRepo := &userErrorOpsRepoCapture{}
	router := newUserErrorListTestRouter(opsRepo)

	req := httptest.NewRequest(http.MethodGet, "/usage/errors?start_date=2026-03-01&end_date=2026-03-02&page=2&page_size=15", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)

	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, 1, opsRepo.listCalls)
	require.NotNil(t, opsRepo.filter)
	require.NotNil(t, opsRepo.filter.UserID)
	require.Equal(t, int64(42), *opsRepo.filter.UserID)
	require.Equal(t, 2, opsRepo.filter.Page)
	require.Equal(t, 15, opsRepo.filter.PageSize)
	require.NotNil(t, opsRepo.filter.StartTime)
	require.NotNil(t, opsRepo.filter.EndTime)
	require.Equal(t, "2026-03-01", opsRepo.filter.StartTime.Format("2006-01-02"))
	require.Equal(t, "2026-03-03", opsRepo.filter.EndTime.Format("2006-01-02"))
}
...
func TestUserUsageListRejectsReversedDateRange(t *testing.T) {
	repo := &userUsageRepoCapture{}
	router := newUserUsageRequestTypeTestRouter(repo)

	req := httptest.NewRequest(http.MethodGet, "/usage?start_date=2026-03-08&end_date=2026-03-01", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)

	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Contains(t, rec.Body.String(), "end_date must be greater than or equal to start_date")
	require.Zero(t, repo.listFilters.UserID, "repository must not be called for an invalid date range")
}
