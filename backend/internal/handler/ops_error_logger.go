	...
	entry := &service.OpsInsertErrorLogInput{
		...
		RequestType: func() *int16 {
			if value, ok := c.Get(opsRequestTypeKey); ok {
				switch requestType := value.(type) {
				case int16:
					return &requestType
				case int:
					converted := int16(requestType)
					return &converted
				}
			}
			return nil
...
// shouldSkipOpsErrorLogForCyber keeps the middleware from writing a duplicate
// entry when recordCyberPolicyIfMarked has already recorded the cyber-policy event.
