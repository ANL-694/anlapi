...
func TestUsageLogFromService_PricingVersionIsAdminAuditOnly(t *testing.T) {
	t.Parallel()

	log := &service.UsageLog{
		RequestID:      "req_pv_audit",
		Model:          "gpt-5.4",
		PricingVersion: "pv1-deadbeef",
	}
	...
	userJSON, err := json.Marshal(userDTO)
	require.NoError(t, err)
	require.NotContains(t, string(userJSON), "pricing_version")

	require.NotNil(t, adminDTO.PricingVersion)
	require.Equal(t, "pv1-deadbeef", *adminDTO.PricingVersion)
	adminJSON, err := json.Marshal(adminDTO)
	require.NoError(t, err)
	require.Contains(t, string(adminJSON), `"pricing_version":"pv1-deadbeef"`)
}
