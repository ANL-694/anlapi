...
func UserFromService(u *service.User) *User {
	...
	if len(u.APIKeys) > 0 {
		...
		for i := range u.APIKeys {
			k := u.APIKeys[i]
			out.APIKeys = append(out.APIKeys, *APIKeyFromServicePublic(&k))
		}
	}
...
// APIKeyFromServicePublic is used by list/detail/usage DTOs. API keys are
// bearer credentials: after creation they must remain masked and cannot be
// recovered through a normal read endpoint.
...
func maskAPIKey(key string) string {
	...
	// Public DTOs may be passed through more than one mapper. Keep masking
	// idempotent so an already masked value is never altered again.
...
func usageLogFromServiceUser(l *service.UsageLog) UsageLog {
	...
	return UsageLog{
		...
		APIKey:                    APIKeyFromServicePublic(l.APIKey),
...
func UsageLogFromServiceAdmin(l *service.UsageLog) *AdminUsageLog {
	...
	return &AdminUsageLog{
		...
		PricingVersion:          nonEmptyStringPtr(l.PricingVersion),
