...
// writeModelsListResponse writes an OpenAI-compatible model catalogue. When
// the route carries /models/:model, return the exact catalogue entry instead
// of the collection envelope so clients can probe a single visible model.
func writeModelsListResponse(c *gin.Context, models any) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"type": "error", "error": gin.H{
			"type": "api_error", "message": "Failed to encode model list",
		}})
		return
	}
	...
	if err := json.Unmarshal(body, &envelope); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"type": "error", "error": gin.H{
			"type": "api_error", "message": "Failed to decode model list",
		}})
		return
	}
	for _, raw := range envelope.Data {
		...
		if err := json.Unmarshal(raw, &entry); err == nil && entry.ID == requested {
			c.Data(http.StatusOK, "application/json", raw)
			return
		}
	}
	writeModelNotFoundResponse(c, requested)
}

func writeModelNotFoundResponse(c *gin.Context, model string) {
	c.JSON(http.StatusNotFound, gin.H{"type": "error", "error": gin.H{
		"type":    "not_found_error",
		"code":    "model_not_found",
		"message": fmt.Sprintf("Model %q is not available", model),
	}})
}
