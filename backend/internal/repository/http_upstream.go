...
const (
	...
	// DNS/TCP 建连与 TLS 握手必须有独立上限；响应头超时覆盖不到这两个阶段。
...
func isOpenAIHTTP2CompatibilityError(err error) bool {
	if err == nil || isUpstreamTimeoutError(err) {
		return false
	}
...
func (s *httpUpstreamService) recordOpenAIHTTP2Success(profile service.HTTPUpstreamProfile, protocolMode, proxyKey string) {
	if profile != service.HTTPUpstreamProfileOpenAI || protocolMode != upstreamProtocolModeOpenAIH2 || !isHTTPProxyKey(proxyKey) {
		return
	}
	raw, ok := s.openAIHTTP2Fallbacks.Load(proxyKey)
	if !ok {
