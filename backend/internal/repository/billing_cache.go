...
type billingCache struct {
	rdb                                  *redis.Client
	userPlatformQuotaSnapshotPersistence bool
}

// NewBillingCache keeps the historical unconfigured constructor for repository
// fixtures and callers that do not own the quota-flusher policy.
func NewBillingCache(rdb *redis.Client) service.BillingCache {
	return newBillingCache(rdb, false)
}

// ProvideBillingCache is the production Wire provider. It passes the flusher
// setting into the cache so admin mutation paths know whether a Redis snapshot
// must be persisted before invalidation.
func ProvideBillingCache(rdb *redis.Client, cfg *config.Config) service.BillingCache {
	persistQuotaSnapshots := cfg != nil && cfg.Database.UserPlatformQuotaFlusherEnabled
	return newBillingCache(rdb, persistQuotaSnapshots)
}

func newBillingCache(rdb *redis.Client, persistQuotaSnapshots bool) service.BillingCache {
	return &billingCache{
		rdb:                                  rdb,
		userPlatformQuotaSnapshotPersistence: persistQuotaSnapshots,
	}
}

// UserPlatformQuotaCacheSnapshotEnabled tells admin invalidation whether the
// Redis projection is the flusher-backed authority that must be persisted
// before a destructive cache invalidation. It is deliberately an optional
// capability rather than part of service.BillingCache, so test doubles and
// non-Redis adapters keep the historical contract.
func (c *billingCache) UserPlatformQuotaCacheSnapshotEnabled() bool {
	return c != nil && c.userPlatformQuotaSnapshotPersistence
}
...
// IncrUserPlatformQuotaUsageCacheResult reports whether the Redis Lua script
// actually incremented the cache entry. A false result is an explicit miss
// (missing key or stale schema), not a Redis transport error.
func (c *billingCache) IncrUserPlatformQuotaUsageCacheResult(ctx context.Context, userID int64, platform string, cost float64, ttl time.Duration, markDirty bool) (bool, error) {
	member := ""
	if markDirty {
	...
	if !ok {
		return false, fmt.Errorf("unexpected user platform quota cache increment result type %T", result)
	}
	return value == 1, nil
}
...
func (c *billingCache) IncrUserPlatformQuotaUsageCache(ctx context.Context, userID int64, platform string, cost float64, ttl time.Duration, markDirty bool) error {
	_, err := c.IncrUserPlatformQuotaUsageCacheResult(ctx, userID, platform, cost, ttl, markDirty)
	return err
}
