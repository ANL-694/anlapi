...
func TestLockAndMergeAccountProbeExtraCoalescesNullableOllamaGroupIdentity(t *testing.T) {
	...
	merged, err := lockAndMergeAccountProbeExtra(ctx, tx.Client(), loaded, loaded.Credentials, nil)
...
func TestOllamaCloudUsageGroupWritesStayWithinOwnerAndPreserveANLSemantics(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	repo := newAccountRepositoryWithSQL(tx.Client(), tx, nil)
	firstOwner := mustCreateUser(t, tx.Client(), &service.User{Concurrency: 2})
	secondOwner := mustCreateUser(t, tx.Client(), &service.User{Concurrency: 9})

	create := func(name, platform, accountType string, ownerUserID int64, shareMode string, concurrency int) *service.Account {
		t.Helper()
		account := mustCreateAccount(t, tx.Client(), &service.Account{
			Name: name, Platform: platform, Type: accountType, Concurrency: concurrency,
			Credentials: map[string]any{"api_key": "owner-scoped-key", "base_url": "https://ollama.com"},
			Extra:       map[string]any{"anl_marker": name},
		})
	...
	first := create("owner-a-openai", service.PlatformOpenAI, service.AccountTypeAPIKey, firstOwner.ID, service.AccountShareModePublic, 7)
	sameOwner := create("owner-a-anthropic", service.PlatformAnthropic, service.AccountTypeAPIKey, firstOwner.ID, service.AccountShareModePrivate, 11)
	otherOwner := create("owner-b-openai", service.PlatformOpenAI, service.AccountTypeAPIKey, secondOwner.ID, service.AccountShareModePublic, 13)
	oauthAccount := create("owner-a-oauth", service.PlatformOpenAI, service.AccountTypeOAuth, firstOwner.ID, service.AccountShareModePublic, 17)

	anchor, err := repo.GetByID(ctx, first.ID)
	require.NoError(t, err)
	require.NoError(t, repo.SaveOllamaCloudUsageSession(ctx, anchor, "cipher:owner-a", true))

	firstLoaded, err := repo.GetByID(ctx, first.ID)
	require.NoError(t, err)
	sameOwnerLoaded, err := repo.GetByID(ctx, sameOwner.ID)
	require.NoError(t, err)
	otherOwnerLoaded, err := repo.GetByID(ctx, otherOwner.ID)
	require.NoError(t, err)
	oauthLoaded, err := repo.GetByID(ctx, oauthAccount.ID)
	require.NoError(t, err)

	for _, account := range []*service.Account{firstLoaded, sameOwnerLoaded} {
		require.Equal(t, "cipher:owner-a", account.Extra[service.OllamaCloudUsageSessionExtraKey])
		require.Equal(t, true, account.Extra[service.OllamaCloudUsageAutoRefreshExtraKey])
	}
	require.NotContains(t, otherOwnerLoaded.Extra, service.OllamaCloudUsageSessionExtraKey)
	require.NotContains(t, oauthLoaded.Extra, service.OllamaCloudUsageSessionExtraKey)
	require.Equal(t, "owner-b-openai", otherOwnerLoaded.Extra["anl_marker"])
	require.Equal(t, "owner-a-oauth", oauthLoaded.Extra["anl_marker"])
	require.Equal(t, service.AccountShareModePublic, firstLoaded.ShareMode)
	require.Equal(t, service.AccountShareStatusApproved, firstLoaded.ShareStatus)
	require.Equal(t, 7, firstLoaded.Concurrency)
	require.Equal(t, service.AccountShareModePrivate, sameOwnerLoaded.ShareMode)
	require.Equal(t, 11, sameOwnerLoaded.Concurrency)

	firstOwnerLoaded, err := tx.Client().User.Get(ctx, firstOwner.ID)
	require.NoError(t, err)
	secondOwnerLoaded, err := tx.Client().User.Get(ctx, secondOwner.ID)
	require.NoError(t, err)
	require.Equal(t, 2, firstOwnerLoaded.Concurrency)
	require.Equal(t, 9, secondOwnerLoaded.Concurrency)
}
