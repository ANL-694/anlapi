...
func TestApplyMigrationsFS_NonTransactionalMigration_UsersEmailAliasDedupIndexDropsInvalidIndexBeforeRetry(t *testing.T) {
	...
	mock.ExpectQuery("SELECT checksum FROM schema_migrations WHERE filename = \\$1").
		...
		WillReturnError(sql.ErrNoRows)
	mock.ExpectQuery("SELECT EXISTS \\(").
		WithArgs(usersEmailDotStrippedIndex).
		WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(true))
	mock.ExpectExec("DROP INDEX CONCURRENTLY IF EXISTS idx_users_email_dot_stripped").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_users_email_dot_stripped").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\$1, \\$2\\)").
	...
	fsys := fstest.MapFS{
		usersEmailAliasDedupIndexMigration: &fstest.MapFile{
			Data: []byte(`
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_users_email_dot_stripped
    ON users ((replace(lower(email), '.', '')));
`),
		},
	}
...
func TestApplyMigrationsFS_DefersOpsRetryReplayCleanupByDefault(t *testing.T) {
	t.Setenv(applyOpsRetryReplayCleanupEnv, "")
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	prepareMigrationsBootstrapExpectations(mock)
	mock.ExpectQuery("SELECT checksum FROM schema_migrations WHERE filename = \\$1").
		WithArgs(opsRetryReplayCleanupMigration).
		WillReturnError(sql.ErrNoRows)
	mock.ExpectExec("SELECT pg_advisory_unlock\\(\\$1\\)").
		WithArgs(migrationsAdvisoryLockID).
		WillReturnResult(sqlmock.NewResult(0, 1))
	...
	fsys := fstest.MapFS{
		opsRetryReplayCleanupMigration: &fstest.MapFile{
			Data: []byte("DROP TABLE IF EXISTS ops_retry_attempts CASCADE;"),
		},
	}
...
func TestApplyMigrationsFS_AppliesOpsRetryReplayCleanupWhenEnabled(t *testing.T) {
	t.Setenv(applyOpsRetryReplayCleanupEnv, "true")
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	mock.ExpectQuery("SELECT checksum FROM schema_migrations WHERE filename = \\$1").
		...
		WillReturnError(sql.ErrNoRows)
	mock.ExpectBegin()
	mock.ExpectExec("DROP TABLE IF EXISTS ops_retry_attempts CASCADE").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\$1, \\$2\\)").
		WithArgs(opsRetryReplayCleanupMigration, sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
	mock.ExpectExec("SELECT pg_advisory_unlock\\(\\$1\\)").
		WithArgs(migrationsAdvisoryLockID).
