...
func TestUserRepositoryExistsByEmailAliasChecksCandidatesAfterFirstPage(t *testing.T) {
	repo, _ := newUserEntRepo(t)
	ctx := context.Background()
	const baseLocal = "abcdefghijkl"

	// Outside the Gmail family dots remain significant. These addresses therefore
	// over-match the dot-stripped SQL probe but are different inbox identities.
	for mask := 1; mask <= emailAliasCandidatePageSize+1; mask++ {
		local := baseLocal[:1]
		for i := 1; i < len(baseLocal); i++ {
			if mask&(1<<(i-1)) != 0 {
				local += "."
			}
			local += baseLocal[i : i+1]
		}
		seedUserForAliasTest(t, repo, local+"@qq.com")
	}

	exists, err := repo.ExistsByEmailAlias(ctx, baseLocal+"@qq.com")
	require.NoError(t, err)
	require.False(t, exists)

	seedUserForAliasTest(t, repo, baseLocal+"+existing@qq.com")
	exists, err = repo.ExistsByEmailAlias(ctx, baseLocal+"@qq.com")
	require.NoError(t, err)
