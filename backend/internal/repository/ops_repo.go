...
func buildOpsErrorLogsWhere(filter *service.OpsErrorLogFilter) (string, []any) {
	...
	// endpoints never set the opt-in and retain this guard. Streaming cyber policy
	// failures are client-visible even though the HTTP stream was opened with 200.
	...
	if filter != nil && filter.UserID != nil && *filter.UserID > 0 {
		args = append(args, *filter.UserID)
		clauses = append(clauses, "e.user_id = $"+itoa(len(args)))
	}
	if filter != nil && filter.APIKeyID != nil && *filter.APIKeyID > 0 {
	...
	if filter != nil {
		if model := strings.TrimSpace(filter.Model); model != "" {
			if filter.ModelFuzzy {
				args = append(args, "%"+escapeLikePattern(model)+"%")
