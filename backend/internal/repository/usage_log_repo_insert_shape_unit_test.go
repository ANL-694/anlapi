...
func TestPrepareUsageLogInsert_PricingVersionArgWiring(t *testing.T) {
	pricingVersion := "pv1-0123456789abcdef"
	prepared := prepareUsageLogInsert(&service.UsageLog{
		UserID:         1,
		APIKeyID:       2,
		RequestID:      "client:pricing-version",
		Model:          "gpt-5",
	...
	for i, typ := range usageLogInsertArgTypes {
		if typ == "text" && i > 0 && i+1 < len(usageLogInsertArgTypes) && usageLogInsertArgTypes[i+1] == "numeric" {
			idx = i
			break
		}
	}
	require.NotEqual(t, -1, idx, "pricing_version text slot should be present before rate_multiplier")
	arg, ok := prepared.args[idx].(sql.NullString)
	require.True(t, ok, "pricing_version arg should be sql.NullString, got %T", prepared.args[idx])
	require.True(t, arg.Valid)
	require.Equal(t, pricingVersion, arg.String)
	require.Contains(t, usageLogSelectColumns, "pricing_version")

	absent := prepareUsageLogInsert(&service.UsageLog{
		UserID:    1,
		APIKeyID:  2,
		RequestID: "client:pricing-version-absent",
		Model:     "gpt-5",
		CreatedAt: time.Now().UTC(),
	})
	nullArg, ok := absent.args[idx].(sql.NullString)
	require.True(t, ok)
	require.False(t, nullArg.Valid, "missing pricing version must be NULL")
}
