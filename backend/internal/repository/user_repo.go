...
func (r *userRepository) Update(ctx context.Context, userIn *service.User, fields service.UserUpdateFields) error {
	...
	if fields.PreferPointsBilling {
		updateOp = updateOp.SetPreferPointsBilling(userIn.PreferPointsBilling)
	}
	if fields.RPMLimit {
...
func (r *userRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters service.UserListFilters) ([]service.User, *pagination.PaginationResult, error) {
	// Only the user query bypasses soft deletion. Related entities retain their normal filtering.
	userCtx := ctx
...
// AdjustBalance atomically applies a delta and returns the before/after values.
// A negative result is rejected without writing anything.
...
// SetBalance atomically replaces a user's balance and returns the before/after values.
...
// emailAliasCandidatePageSize 限制别名查重每页取回的候选行数。探针都以去点后的
// 本地部分为前缀锚定（见 dotStrippedEmailExpr），正常收件箱的变体只有个位数；
// 游标分页避免公开未鉴权的注册/发码端点一次把大量候选读进内存，同时不能截断
// 最终归一化检查，否则第 51 行之后的真实别名会漏过注册查重。
...
func existsByEmailAliasWithClient(ctx context.Context, client *dbent.Client, email string) (bool, error) {
	...
	for {
		candidates, err := client.User.Query().
			Where(dbuser.Or(preds...), dbuser.IDGT(lastID)).
			Order(dbent.Asc(dbuser.FieldID)).
