...
func NewUserRepository(client *dbent.Client, sqlDB *sql.DB, vault service.OAuthCredentialVault) service.UserRepository {
	return newUserRepositoryWithSQL(client, sqlDB, vault)
}

func newUserRepositoryWithSQL(client *dbent.Client, sqlq sqlExecutor, vaults ...service.OAuthCredentialVault) *userRepository {
	var vault service.OAuthCredentialVault
	if len(vaults) > 0 {
		vault = vaults[0]
	}
	return &userRepository{client: client, sql: sqlq, oauthVault: vault}
}
...
func (r *userRepository) Delete(ctx context.Context, id int64) error {
	...
	if existingTx := dbent.TxFromContext(ctx); existingTx != nil {
		if err := lockUserForDeletion(ctx, existingTx.Client(), id); err != nil {
			return err
		}
		// The caller owns this commit. Keep durable tombstones until reconciliation
		// observes whether the business delete committed or rolled back.
		if _, err := r.beginOwnedOAuthVaultDeletes(ctx, existingTx.Client(), id); err != nil {
			return err
		}
	...
	if err := lockUserForDeletion(ctx, exec, id); err != nil {
		return err
	}
	vaultDeletes, err := r.beginOwnedOAuthVaultDeletes(ctx, exec, id)
	if err != nil {
		return err
	}
	...
	defer func() {
		if !commitAttempted {
			r.cancelOwnedOAuthVaultDeletes(ctx, vaultDeletes)
		}
	}()
	...
	if err := r.completeOwnedOAuthVaultDeletes(ctx, vaultDeletes); err != nil {
		return err
	}
	return nil
}

func lockUserForDeletion(ctx context.Context, exec *dbent.Client, userID int64) error {
	query := `
...
type ownedOAuthVaultDelete struct {
	key         service.OAuthCredentialVaultKey
	coordinator service.OAuthCredentialVaultDeleteCoordinator
}

func (r *userRepository) beginOwnedOAuthVaultDeletes(ctx context.Context, exec *dbent.Client, userID int64) ([]ownedOAuthVaultDelete, error) {
	if r.oauthVault == nil || r.oauthVault.Mode() != service.OAuthCredentialVaultModeExternal {
		return nil, nil
	}
	coordinator, ok := r.oauthVault.(service.OAuthCredentialVaultDeleteCoordinator)
	if !ok {
		return nil, errors.New("external OAuth credential Vault does not support durable deletion")
	}
	rows, err := exec.QueryContext(ctx, `
		SELECT id FROM accounts
		WHERE owner_user_id = $1 AND deleted_at IS NULL AND type IN ('oauth', 'setup-token')
		ORDER BY id ASC
	`, userID)
	if err != nil {
		return nil, err
	...
	defer func() { _ = rows.Close() }()
	deletes := make([]ownedOAuthVaultDelete, 0)
	for rows.Next() {
		var accountID int64
		if err := rows.Scan(&accountID); err != nil {
			return nil, err
		}
		entry := ownedOAuthVaultDelete{
			key:         service.OAuthCredentialVaultKey{AccountID: accountID},
			coordinator: coordinator,
		}
		if err := coordinator.BeginDelete(ctx, entry.key); err != nil {
			r.cancelOwnedOAuthVaultDeletes(ctx, deletes)
			return nil, fmt.Errorf("record OAuth credential deletion for owned account %d: %w", accountID, err)
		}
...
func (r *userRepository) cancelOwnedOAuthVaultDeletes(ctx context.Context, deletes []ownedOAuthVaultDelete) {
	for _, entry := range deletes {
		recoveryCtx, cancel := oauthVaultRecoveryContext(ctx)
		_ = entry.coordinator.CancelDelete(recoveryCtx, entry.key)
		cancel()
	}
}

func (r *userRepository) completeOwnedOAuthVaultDeletes(ctx context.Context, deletes []ownedOAuthVaultDelete) error {
	for _, entry := range deletes {
		postCommitCtx, cancel := oauthVaultRecoveryContext(ctx)
		err := entry.coordinator.CompleteDelete(postCommitCtx, entry.key)
		cancel()
		if err != nil {
			return fmt.Errorf("complete OAuth credential deletion for owned account %d: %w", entry.key.AccountID, err)
		}
	}
	return nil
}
...
func (r *userRepository) deleteUser(ctx context.Context, exec *dbent.Client, id int64) error {
	if err := r.clearOwnedAccountAndPrivateGroupState(ctx, exec, id); err != nil {
		return err
	}
...
// clearOwnedAccountAndPrivateGroupState runs inside the same business
// transaction as user deletion. It removes every routing-visible association
// before the user row is deleted, then neutralizes private groups before the
// owner FK is evaluated. This is deliberately SQL-level so stale joins cannot
// escape through an alternate repository path. Delete orchestrates OAuth Vault
// tombstones around this business transaction; this helper only changes the
// business rows after the durable precondition has been established.
func (r *userRepository) clearOwnedAccountAndPrivateGroupState(ctx context.Context, exec *dbent.Client, userID int64) error {
	rows, err := exec.QueryContext(ctx, `SELECT id FROM accounts WHERE owner_user_id = $1 AND deleted_at IS NULL`, userID)
	if err != nil {
		return err
	}
	...
	if _, err := exec.ExecContext(ctx, `
		...
		WHERE account_id IN (SELECT id FROM accounts WHERE owner_user_id = $1 AND deleted_at IS NULL)`, userID); err != nil {
		return err
	}
	if err := execOptionalContext(ctx, exec, `
	...
	if _, err := exec.ExecContext(ctx, `
		UPDATE accounts
		SET deleted_at = CURRENT_TIMESTAMP, owner_user_id = NULL, schedulable = FALSE, updated_at = CURRENT_TIMESTAMP
		WHERE owner_user_id = $1 AND deleted_at IS NULL`, userID); err != nil {
		return err
	}

	groupRows, err := exec.QueryContext(ctx, `SELECT id FROM groups WHERE owner_user_id = $1 AND scope = 'user_private' AND deleted_at IS NULL`, userID)
	if err != nil {
		return err
	}
	...
	if _, err := exec.ExecContext(ctx, `UPDATE user_subscriptions SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE deleted_at IS NULL AND (user_id = $1 OR group_id IN (SELECT id FROM groups WHERE owner_user_id = $1 AND scope = 'user_private' AND deleted_at IS NULL))`, userID); err != nil {
		return err
	}
	if _, err := exec.ExecContext(ctx, `DELETE FROM user_allowed_groups WHERE user_id = $1 OR group_id IN (SELECT id FROM groups WHERE owner_user_id = $1 AND scope = 'user_private' AND deleted_at IS NULL)`, userID); err != nil {
		return err
	}
	if err := execOptionalContext(ctx, exec, `DELETE FROM user_group_rate_multipliers WHERE user_id = $1 OR group_id IN (SELECT id FROM groups WHERE owner_user_id = $1 AND scope = 'user_private' AND deleted_at IS NULL)`, userID); err != nil {
		return err
	}
	if err := execOptionalContext(ctx, exec, `UPDATE user_platform_quotas SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE deleted_at IS NULL AND user_id = $1`, userID); err != nil {
		return err
	}
	if _, err := exec.ExecContext(ctx, `
		UPDATE groups
		SET deleted_at = CURRENT_TIMESTAMP, owner_user_id = NULL, scope = 'public', updated_at = CURRENT_TIMESTAMP
		WHERE owner_user_id = $1 AND scope = 'user_private' AND deleted_at IS NULL`, userID); err != nil {
		return err
	}
...
// Some supported installations predate optional scheduler/quota tables. Their
// absence must not make the core user deletion transaction fail, while all
// other SQL errors remain fail-closed.
func execOptionalContext(ctx context.Context, exec *dbent.Client, query string, args ...any) error {
	if _, err := exec.ExecContext(ctx, query, args...); err != nil {
		message := strings.ToLower(err.Error())
		if strings.Contains(message, "no such table") || (strings.Contains(message, "relation") && strings.Contains(message, "does not exist")) {
			return nil
		}
		return err
	}
