...
import (
	...
	dbaccountgroup "anlapi/ent/accountgroup"
	dbgroup "anlapi/ent/group"
	dbpredicate "anlapi/ent/predicate"
	dbproxy "anlapi/ent/proxy"
...
func (r *accountRepository) UpdateCredentials(ctx context.Context, id int64, credentials map[string]any) error {
	...
	result, err := client.ExecContext(ctx, `
		...
		SET
			credentials = $1::jsonb,
			extra = CASE
				-- 凭证整体未变化时不得进入 Ollama 清理分支，避免非 Ollama 账号
				-- 的幂等保存误清探测快照或把 NULL extra 改写为空对象。
...
// pool used to decide whether a model is supported. It intentionally ignores
// transient rate limits, overload, expiry, and temporary unschedulability.
...
// ListShadowsByParent returns active spark shadows linked to the parent account.
