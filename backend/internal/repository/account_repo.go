...
func NewAccountRepository(client *dbent.Client, sqlDB *sql.DB, schedulerCache service.SchedulerCache, vault ...service.OAuthCredentialVault) service.AccountRepository {
...
func NewAdminAccountRepository(client *dbent.Client, sqlDB *sql.DB, schedulerCache service.SchedulerCache, vault ...service.OAuthCredentialVault) service.AdminAccountRepository {
	return newAccountRepositoryWithSQL(client, sqlDB, schedulerCache, vault...)
}

// newAccountRepositoryWithSQL 是内部构造函数，支持依赖注入 SQL 执行器。
// 这种设计便于单元测试时注入 mock 对象。
func newAccountRepositoryWithSQL(client *dbent.Client, sqlq sqlExecutor, schedulerCache service.SchedulerCache, vaults ...service.OAuthCredentialVault) *accountRepository {
	var vault service.OAuthCredentialVault
	if len(vaults) > 0 {
		vault = vaults[0]
	}
	return &accountRepository{client: client, sql: sqlq, schedulerCache: schedulerCache, oauthVault: vault}
}
...
func (r *accountRepository) CreateWithAccountGroups(ctx context.Context, account *service.Account, groups []service.AccountGroup) error {
	...
	if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeDisabled && service.IsOAuthCredentialAccount(account.Type) {
...
func (r *accountRepository) updateAccount(
	...
	if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeExternal &&
		!service.IsOAuthCredentialAccount(account.Type) && service.OAuthCredentialVaultVersion(account.Credentials) != "" {
		return errors.New("changing an externally vaulted OAuth account to a non-OAuth type is not supported")
	}
	if r.client == nil {
		return errors.New("account repository client is not configured")
	}
...
func (r *accountRepository) updateLockedAccount(
	...
	builder := client.Account.UpdateOneID(account.ID).
		...
		SetAutoPauseOnExpired(account.AutoPauseOnExpired)
	if len(ownerUserIDs) > 0 && ownerUserIDs[0] != nil {
		builder.Where(dbaccount.OwnerUserIDEQ(*ownerUserIDs[0]))
	}
...
func (r *accountRepository) ListOAuthRefreshCandidatePage(ctx context.Context, options service.OAuthRefreshPageOptions) (*service.OAuthRefreshCandidatePage, error) {
	...
	//
	// Active refresh/reconciliation scans exclude permanently unschedulable
	// accounts. Callers that intentionally need paused accounts can omit
	// ActiveOnly and retain the broader refresh-candidate behavior.
	query := `
		SELECT id
		FROM accounts
		WHERE deleted_at IS NULL
...
func (r *accountRepository) SetGrokCredentialErrorIfMatch(
	...
	snapshotCredentialsJSON := snapshot.CredentialsJSON
	if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeExternal {
		var err error
		snapshotCredentialsJSON, err = r.grokCredentialSnapshotJSONForCAS(snapshot.CredentialsJSON)
		if err != nil {
			return false, err
		}
	}
	...
	`, service.StatusError, errorMsg, id, service.StatusActive, service.PlatformGrok, service.AccountTypeOAuth,
		snapshotCredentialsJSON, snapshot.ProxyID, string(service.GrokCredentialReasonProxyInvalid),
...
func (r *accountRepository) SetGrokOAuthErrorIfCredentialsUnchanged(
	...
	expectedRaw, err := json.Marshal(normalizeJSONMap(expectedCredentials))
	if err != nil {
		return false, err
	}
	expectedJSON, err := r.grokCredentialSnapshotJSONForCAS(string(expectedRaw))
	if err != nil {
		return false, err
	}
	result, err := r.sql.ExecContext(ctx, `
...
func (r *accountRepository) ListModelAvailabilityCandidates(
	...
	if groupID != nil {
		targetGroup, err := r.client.Group.Query().
			Where(dbgroup.IDEQ(*groupID)).
			Select(dbgroup.FieldScope, dbgroup.FieldOwnerUserID).
			Only(ctx)
		if err != nil {
		...
		switch service.NormalizeGroupScope(string(targetGroup.Scope)) {
		case service.GroupScopeUserPrivate, service.GroupScopeUserCarpool:
			if targetGroup.OwnerUserID == nil {
				return nil, fmt.Errorf("owned group %d has no owner", *groupID)
			}
			preds = append(preds, dbaccount.OwnerUserIDEQ(*targetGroup.OwnerUserID))
		default:
			preds = append(preds, dbaccount.OwnerUserIDIsNil())
		}
		accounts, err := r.client.Account.Query().Where(preds...).
			Order(dbent.Asc(dbaccount.FieldPriority)).
			All(ctx)
		if err != nil {
			return nil, err
...
// SetRateLimitedIfUnchanged applies a new account rate-limit generation only
// when the caller's observed row version and rate-limit timestamps still match.
// This prevents a stale asynchronous usage probe from overwriting a newer 429,
// an administrative clear, or a re-armed generation.
func (r *accountRepository) SetRateLimitedIfUnchanged(
	ctx context.Context,
	id int64,
	expectedUpdatedAt time.Time,
	expectedLimitedAt *time.Time,
	expectedResetAt *time.Time,
	resetAt time.Time,
) (bool, error) {
	predicates := []dbpredicate.Account{
		dbaccount.IDEQ(id),
		dbaccount.UpdatedAtEQ(expectedUpdatedAt),
	}
	if expectedLimitedAt == nil {
		predicates = append(predicates, dbaccount.RateLimitedAtIsNil())
	} else {
		predicates = append(predicates, dbaccount.RateLimitedAtEQ(*expectedLimitedAt))
	}
	if expectedResetAt == nil {
		predicates = append(predicates, dbaccount.RateLimitResetAtIsNil())
	} else {
		predicates = append(predicates, dbaccount.RateLimitResetAtEQ(*expectedResetAt))
	}
...
func (r *accountRepository) SetGrokCredentialTempUnschedulableIfMatch(
	...
	`, until, reason, id, service.StatusActive, service.PlatformGrok, service.AccountTypeOAuth,
		snapshotCredentialsJSON, snapshot.ProxyID, service.SchedulerOutboxEventAccountChanged)
	if err != nil {
		return false, err
	}
	affected, err := result.RowsAffected()
...
func (r *accountRepository) createAccountRecordWithVault(ctx context.Context, client *dbent.Client, account *service.Account) (oauthVaultWrite, error) {
	if account == nil {
		return oauthVaultWrite{}, service.ErrAccountNilInput
	}
	if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeDisabled && service.IsOAuthCredentialAccount(account.Type) {
		return oauthVaultWrite{}, service.ErrOAuthCredentialVaultDisabled
	}
	persisted, sensitive, hasSensitive := service.SplitOAuthCredentials(account.Credentials)
	if !service.IsOAuthCredentialAccount(account.Type) || !hasSensitive {
		return oauthVaultWrite{}, createAccountRecord(ctx, client, account)
	}
	original := account.Credentials
	account.Credentials = persisted
	defer func() { account.Credentials = original }()
	if err := createAccountRecord(ctx, client, account); err != nil {
		return oauthVaultWrite{}, err
	}
	write := oauthVaultWrite{key: service.OAuthCredentialVaultKey{AccountID: account.ID}}
	version, err := r.oauthVault.Put(ctx, write.key, sensitive)
	if err != nil {
		return oauthVaultWrite{}, fmt.Errorf("store OAuth credentials for account %d: %w", account.ID, err)
	}
	write.newVersion = version
	...
	if _, err := client.Account.UpdateOneID(account.ID).SetCredentials(normalizeJSONMap(persisted)).Save(ctx); err != nil {
		_ = r.discardOAuthVaultWrite(context.Background(), write)
		return oauthVaultWrite{}, fmt.Errorf("mark OAuth credentials for account %d: %w", account.ID, err)
	}
	return write, nil
}

func (r *accountRepository) GetOwnedByID(ctx context.Context, ownerUserID, accountID int64) (*service.Account, error) {
	m, err := r.client.Account.Query().
		Where(dbaccount.IDEQ(accountID), dbaccount.OwnerUserIDEQ(ownerUserID)).
		Only(ctx)
	if err != nil {
		return nil, translatePersistenceError(err, service.ErrAccountNotFound, nil)
	}
...
func (r *accountRepository) UpdateOwned(ctx context.Context, ownerUserID int64, account *service.Account) error {
	if ownerUserID <= 0 {
		return service.ErrAccountNotFound
	}
	...
	return r.updateAccount(ctx, account, nil, nil, account.RateMultiplier, &ownerUserID)
}

func (r *accountRepository) discardOAuthVaultWrite(ctx context.Context, write oauthVaultWrite) error {
	if write.newVersion == "" || r.oauthVault == nil {
		return nil
	}
	janitor, ok := r.oauthVault.(service.OAuthCredentialVaultVersionJanitor)
	if !ok {
		return errors.New("external OAuth credential Vault does not support version cleanup")
	}
	recoveryCtx, cancel := oauthVaultRecoveryContext(ctx)
	defer cancel()
	return janitor.DeleteVersion(recoveryCtx, write.key, write.newVersion)
}

func shouldDiscardOAuthVaultWrite(committed, commitAttempted bool) bool {
	return !committed && !commitAttempted
}

func shouldCancelOAuthVaultDelete(businessDeletePending, commitAttempted bool) bool {
	return businessDeletePending && !commitAttempted
}

func oauthVaultRecoveryContext(ctx context.Context) (context.Context, context.CancelFunc) {
	base := context.Background()
	if ctx != nil {
		base = context.WithoutCancel(ctx)
	}
...
func (r *accountRepository) activateOAuthVaultWrite(ctx context.Context, write oauthVaultWrite) {
	if write.newVersion == "" || r.oauthVault == nil {
		return
	}
	coordinator, ok := r.oauthVault.(service.OAuthCredentialVaultWriteCoordinator)
	if !ok {
		logger.LegacyPrintf("repository.account", "[OAuthVault] activate unsupported: account=%d version=%s", write.key.AccountID, write.newVersion)
		return
	}
	base := context.Background()
	if ctx != nil {
	...
	activationCtx, cancel := context.WithTimeout(base, 2*time.Second)
	defer cancel()
	if err := coordinator.ActivateVersion(activationCtx, write.key, write.newVersion); err != nil {
		// The marker already committed and pending versions remain readable and
		// immune to GC. Reconciliation will activate the referenced version.
		logger.LegacyPrintf("repository.account", "[OAuthVault] activate pending version failed: account=%d version=%s err=%v", write.key.AccountID, write.newVersion, err)
	}
}

func (r *accountRepository) prepareOAuthCredentialsForWrite(ctx context.Context, account *service.Account) (func(), oauthVaultWrite, error) {
	if r.oauthVault == nil || account == nil || !service.IsOAuthCredentialAccount(account.Type) {
		return func() {}, oauthVaultWrite{}, nil
	}
	if r.oauthVault.Mode() == service.OAuthCredentialVaultModeDisabled {
		return func() {}, oauthVaultWrite{}, service.ErrOAuthCredentialVaultDisabled
	}
	if r.oauthVault.Mode() != service.OAuthCredentialVaultModeExternal {
		return func() {}, oauthVaultWrite{}, nil
	}
	original := account.Credentials
	persisted, sensitive, hasSensitive := service.SplitOAuthCredentials(original)
	write := oauthVaultWrite{key: service.OAuthCredentialVaultKey{AccountID: account.ID}}
	if hasSensitive {
		version, err := r.oauthVault.Put(ctx, write.key, sensitive)
		if err != nil {
			return func() {}, oauthVaultWrite{}, fmt.Errorf("store OAuth credentials for account %d: %w", account.ID, err)
		}
		write.newVersion = version
		persisted = service.SetOAuthCredentialVaultVersion(persisted, version)
	} else if service.OAuthCredentialVaultVersion(persisted) == "" {
		return func() {}, oauthVaultWrite{}, fmt.Errorf("OAuth credential Vault version missing for account %d", account.ID)
	}
	account.Credentials = persisted
	return func() { account.Credentials = original }, write, nil
}

func (r *accountRepository) prepareOAuthCredentialsForDirectWrite(ctx context.Context, id int64, credentials map[string]any) (map[string]any, oauthVaultWrite, error) {
	if r.oauthVault == nil || r.oauthVault.Mode() == service.OAuthCredentialVaultModeLegacy {
		return credentials, oauthVaultWrite{}, nil
	}
	if r.client == nil {
		return nil, oauthVaultWrite{}, errors.New("account repository client is not configured")
	}
	account, err := r.client.Account.Query().Where(dbaccount.IDEQ(id)).Only(ctx)
	if err != nil {
		return nil, oauthVaultWrite{}, translatePersistenceError(err, service.ErrAccountNotFound, nil)
	}
	if !service.IsOAuthCredentialAccount(account.Type) {
		return credentials, oauthVaultWrite{}, nil
	}
	if r.oauthVault.Mode() == service.OAuthCredentialVaultModeDisabled {
		return nil, oauthVaultWrite{}, service.ErrOAuthCredentialVaultDisabled
	}
	persisted, _, storedSensitive := service.SplitOAuthCredentials(copyJSONMap(account.Credentials))
	if storedSensitive {
		return nil, oauthVaultWrite{}, fmt.Errorf("sensitive OAuth credentials remain in business database for account %d", id)
	}
	write := oauthVaultWrite{key: service.OAuthCredentialVaultKey{AccountID: id}}
	incoming, sensitive, hasSensitive := service.SplitOAuthCredentials(credentials)
	for key, value := range incoming {
		if key != service.OAuthCredentialVaultMarkerKey {
			persisted[key] = value
		}
	}
	if hasSensitive {
		version, err := r.oauthVault.Put(ctx, write.key, sensitive)
		if err != nil {
			return nil, oauthVaultWrite{}, fmt.Errorf("store OAuth credentials for account %d: %w", id, err)
		}
		write.newVersion = version
		return service.SetOAuthCredentialVaultVersion(persisted, version), write, nil
	}
	if service.OAuthCredentialVaultVersion(persisted) == "" {
		return nil, oauthVaultWrite{}, fmt.Errorf("OAuth credential Vault version missing for account %d", id)
	}
	return persisted, write, nil
}

func (r *accountRepository) oauthCredentialsForCAS(credentials map[string]any) (map[string]any, error) {
	if r.oauthVault == nil {
	...
	if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeDisabled {
		return nil, service.ErrOAuthCredentialVaultDisabled
	}
	persisted, _, _ := service.SplitOAuthCredentials(credentials)
	if service.OAuthCredentialVaultVersion(persisted) == "" {
		return nil, errors.New("OAuth credential Vault version missing for compare-and-set")
	}
	return persisted, nil
}

func (r *accountRepository) grokCredentialSnapshotJSONForCAS(credentialsJSON string) (string, error) {
	var credentials map[string]any
	if err := json.Unmarshal([]byte(credentialsJSON), &credentials); err != nil {
		return "", fmt.Errorf("decode Grok credential snapshot: %w", err)
	}
	persisted, err := r.oauthCredentialsForCAS(credentials)
	if err != nil {
		return "", err
	}
	encoded, err := json.Marshal(normalizeJSONMap(persisted))
	if err != nil {
		return "", fmt.Errorf("encode Grok credential snapshot for compare-and-set: %w", err)
	}
	return string(encoded), nil
}

func validateOAuthVaultTypeTransition(
	ctx context.Context,
	exec sqlExecutor,
	...
	if vault == nil || vault.Mode() == service.OAuthCredentialVaultModeLegacy {
		return nil
	}
	...
	if err := rows.Scan(&currentType); err != nil {
		return err
	}
	if service.IsOAuthCredentialAccount(currentType) != service.IsOAuthCredentialAccount(newType) {
		return errors.New("changing account type across the OAuth Vault boundary is not supported")
	}
	return nil
}

func (r *accountRepository) DeleteOwned(ctx context.Context, ownerUserID, id int64) error {
	if ownerUserID <= 0 {
		return service.ErrAccountNotFound
	}
	return r.deleteAccount(ctx, id, &ownerUserID)
}
...
func lockAccountOwnerUser(ctx context.Context, client *dbent.Client, ownerUserID *int64) error {
	if ownerUserID == nil {
		return nil
	}
	if *ownerUserID <= 0 {
	...
	rows, err := client.QueryContext(ctx, query, *ownerUserID)
	if err != nil {
		return err
	}
	defer func() { _ = rows.Close() }()
...
func accountGroupIDs(groups []service.AccountGroup) []int64 {
...
func validateAccountGroupBindings(ctx context.Context, client *dbent.Client, accountID int64, ownerUserID *int64, groupIDs []int64) error {
	groupIDs = uniquePositiveInt64s(groupIDs)
	if len(groupIDs) == 0 {
		return nil
	}
	if err := lockGroupRows(ctx, client, groupIDs); err != nil {
		return err
	}
	groups, err := client.Group.Query().Where(dbgroup.IDIn(groupIDs...)).All(ctx)
	if err != nil {
		return err
	}
	...
	accountPlatform := ""
	if accountID > 0 {
		account, accountErr := client.Account.Query().Where(dbaccount.IDEQ(accountID)).Only(ctx)
		if accountErr != nil {
			return translatePersistenceError(accountErr, service.ErrAccountNotFound, nil)
		}
		accountPlatform = strings.ToLower(strings.TrimSpace(account.Platform))
	}
	for _, target := range groups {
		if accountPlatform != "" && strings.ToLower(strings.TrimSpace(target.Platform)) != accountPlatform {
			return fmt.Errorf("account %d platform %q does not match group %d platform %q", accountID, accountPlatform, target.ID, target.Platform)
		}
		switch service.NormalizeGroupScope(string(target.Scope)) {
		case service.GroupScopeUserPrivate, service.GroupScopeUserCarpool:
			if target.OwnerUserID == nil || ownerUserID == nil || *target.OwnerUserID != *ownerUserID {
				return fmt.Errorf("account %d owner does not match private group %d", accountID, target.ID)
			}
		default:
			if ownerUserID != nil {
				return fmt.Errorf("owned account %d cannot bind public group %d", accountID, target.ID)
			}
		}
	}
	return nil
...
func lockGroupRows(ctx context.Context, client *dbent.Client, groupIDs []int64) error {
	groupIDs = uniquePositiveInt64s(groupIDs)
	if len(groupIDs) == 0 {
		return nil
	}
	query := `SELECT id FROM groups WHERE id = ANY($1) AND deleted_at IS NULL`
	if client.Driver().Dialect() == dialect.Postgres {
		query += " FOR UPDATE"
	}
	rows, err := client.QueryContext(ctx, query, pq.Array(groupIDs))
	if err != nil {
		return err
	}
	defer func() { _ = rows.Close() }()
	...
	if len(seen) != len(groupIDs) {
		return service.ErrGroupNotFound
	}
	return nil
}

func (r *accountRepository) hydrateOAuthCredentials(ctx context.Context, account *service.Account) error {
	if account == nil || !service.IsOAuthCredentialAccount(account.Type) || r.oauthVault == nil {
		return nil
	}
	switch r.oauthVault.Mode() {
	case service.OAuthCredentialVaultModeLegacy:
		return nil
	case service.OAuthCredentialVaultModeDisabled:
		return service.ErrOAuthCredentialVaultDisabled
	case service.OAuthCredentialVaultModeExternal:
		persisted, _, hasSensitive := service.SplitOAuthCredentials(account.Credentials)
		version := service.OAuthCredentialVaultVersion(persisted)
		if version == "" {
			if hasSensitive && r.oauthVault.LegacyFallbackEnabled() {
				return nil
			}
			return fmt.Errorf("OAuth credential Vault version missing for account %d", account.ID)
		}
		if hasSensitive {
			return fmt.Errorf("sensitive OAuth credentials remain in business database for account %d", account.ID)
		}
		sensitive, err := r.oauthVault.Get(ctx, service.OAuthCredentialVaultKey{AccountID: account.ID}, version)
		if err != nil {
			return fmt.Errorf("hydrate OAuth credentials for account %d: %w", account.ID, err)
		}
		account.Credentials = service.MergeOAuthCredentials(persisted, sensitive)
		return nil
	default:
		return fmt.Errorf("unsupported OAuth credential Vault mode %q", r.oauthVault.Mode())
	}
}

func accountGroupBindingVisible(accountOwner *int64, group *service.Group) bool {
	if group == nil {
		return false
	}
	if scope := service.NormalizeGroupScope(group.Scope); scope == service.GroupScopeUserPrivate || scope == service.GroupScopeUserCarpool {
		return accountOwner != nil && group.OwnerUserID != nil && *accountOwner == *group.OwnerUserID
	}
	return accountOwner == nil && group.OwnerUserID == nil
}
...
func (r *accountRepository) deleteAccount(ctx context.Context, id int64, ownerUserID *int64) error {
	groupIDs, err := r.loadAccountGroupIDs(ctx, id)
	if err != nil {
		return err
	}
	accountQuery := r.client.Account.Query().Where(dbaccount.IDEQ(id))
	if ownerUserID != nil {
		accountQuery = accountQuery.Where(dbaccount.OwnerUserIDEQ(*ownerUserID))
	}
	account, err := accountQuery.Only(ctx)
	if err != nil {
		return translatePersistenceError(err, service.ErrAccountNotFound, nil)
	}
	key := service.OAuthCredentialVaultKey{AccountID: id}
	var deleteCoordinator service.OAuthCredentialVaultDeleteCoordinator
	businessDeletePending := false
	commitAttempted := false
	if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeExternal && service.IsOAuthCredentialAccount(account.Type) {
		var ok bool
		deleteCoordinator, ok = r.oauthVault.(service.OAuthCredentialVaultDeleteCoordinator)
		if !ok {
			return errors.New("external OAuth credential Vault does not support durable deletion")
		}
		if err := deleteCoordinator.BeginDelete(ctx, key); err != nil {
			return fmt.Errorf("record OAuth credential deletion for account %d: %w", id, err)
		}
		businessDeletePending = true
		defer func() {
			if shouldCancelOAuthVaultDelete(businessDeletePending, commitAttempted) {
				// A failed cancellation remains safe: reconciliation sees the business
				// account and removes the stale pending marker before any cleanup.
				cancelCtx, cancel := oauthVaultRecoveryContext(context.Background())
				defer cancel()
				_ = deleteCoordinator.CancelDelete(cancelCtx, key)
			}
		}()
	}
	// 使用事务保证账号与关联分组的删除原子性
	...
	if err == nil {
		...
		if r.oauthVault != nil && r.oauthVault.Mode() == service.OAuthCredentialVaultModeExternal {
			return errors.New("cannot delete an externally vaulted OAuth account inside a caller-owned transaction")
		}
		// 已处于外部事务中（ErrTxStarted），复用当前 client
		txClient = r.client
	}
	...
	var deleted int
	if ownerUserID != nil {
		deleted, err = txClient.Account.Delete().Where(dbaccount.IDEQ(id), dbaccount.OwnerUserIDEQ(*ownerUserID)).Exec(ctx)
	} else {
		deleted, err = txClient.Account.Delete().Where(dbaccount.IDEQ(id)).Exec(ctx)
	}
	if err != nil {
		return err
	}
	...
	var vaultCleanupErr error
	postCommitCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), 2*time.Second)
	defer cancel()
	if deleteCoordinator != nil {
		businessDeletePending = false
		// The tombstone was written before the business transaction. Once the
		// delete commits it must remain retryable if Vault cleanup is unavailable.
		if err := deleteCoordinator.CompleteDelete(postCommitCtx, key); err != nil {
			vaultCleanupErr = fmt.Errorf("complete OAuth credential deletion for account %d: %w", id, err)
		}
	}
	var cacheCleanupErr error
	if r.schedulerCache != nil {
		if err := r.schedulerCache.DeleteAccount(postCommitCtx, id); err != nil {
			cacheCleanupErr = fmt.Errorf("delete scheduler account snapshot %d: %w", id, err)
		}
	}
	if vaultCleanupErr != nil {
		return vaultCleanupErr
	}
	if cacheCleanupErr != nil {
		return cacheCleanupErr
	}
	return nil
}
