...
type ollamaCloudUsageGroupIdentity struct {
...
func (r *accountRepository) ListOllamaCloudUsageGroupAccounts(ctx context.Context, accounts []*service.Account) ([]service.Account, error) {
	...
	identities := make([]ollamaCloudUsageGroupIdentity, 0, len(accounts))
	...
	for _, account := range accounts {
		...
		key := struct {
			...
			ownerUserID int64
			hasOwner    bool
		}{apiKey: apiKey, hasOwner: account.OwnerUserID != nil}
		if account.OwnerUserID != nil {
			key.ownerUserID = *account.OwnerUserID
		}
		if _, duplicate := seen[key]; duplicate {
			continue
		}
		seen[key] = struct{}{}
		identity := ollamaCloudUsageGroupIdentity{APIKey: apiKey}
		if account.OwnerUserID != nil {
	...
	rows, err := r.sql.QueryContext(ctx, `
		...
		FROM accounts
		JOIN jsonb_to_recordset($1::jsonb) AS requested(api_key text, owner_user_id bigint)
			ON credentials ->> 'api_key' = requested.api_key
			AND accounts.owner_user_id IS NOT DISTINCT FROM requested.owner_user_id
		WHERE accounts.deleted_at IS NULL
			AND `+ollamaCloudUsageEligibleSQL+`
		ORDER BY accounts.id
	`, string(encodedIdentities))
	if err != nil {
		return nil, err
	}
...
func (r *accountRepository) updateOllamaCloudUsageGroup(
	...
	apply := func(txCtx context.Context, client *dbent.Client) error {
		...
		`, string(encoded), apiKey, pq.Array(memberIDs), ollamaCloudUsageOwnerUserID(account))
		if err != nil {
			return err
		}
...
func lockOllamaCloudUsageGroup(
	...
	rows, err := client.QueryContext(ctx, `
		...
		ORDER BY id
		FOR NO KEY UPDATE
	`, apiKey, account.ID, account.Platform, account.Type, string(credentials), proxyID, ollamaCloudUsageOwnerUserID(account))
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func ollamaCloudUsageOwnerUserID(account *service.Account) any {
...
func (r *accountRepository) ListDueOllamaCloudUsageAccounts(
	...
	rows, err := r.sql.QueryContext(ctx, `
		WITH eligible AS (
			...
			SELECT owner_user_id, credentials ->> 'api_key' AS api_key,
				MAX(last_used_at) AS group_last_used_at
			FROM accounts
			WHERE deleted_at IS NULL
				AND `+ollamaCloudUsageEligibleSQL+`
				AND jsonb_typeof(credentials -> 'api_key') = 'string'
			GROUP BY owner_user_id, credentials ->> 'api_key'
		), joined AS (
			SELECT e.id, e.owner_user_id, e.api_key, e.snapshot, g.group_last_used_at,
			...
			FROM eligible e
			JOIN group_activity g ON g.owner_user_id IS NOT DISTINCT FROM e.owner_user_id
				AND g.api_key = e.api_key
		), parsed AS MATERIALIZED (
			SELECT id, owner_user_id, api_key, snapshot, group_last_used_at, status,
			...
			SELECT id, owner_user_id, api_key, group_last_used_at, due_class, due_at,
				row_number() OVER (
					PARTITION BY owner_user_id, api_key
					ORDER BY due_class,
						due_at NULLS FIRST,
						id
				) AS group_rank
