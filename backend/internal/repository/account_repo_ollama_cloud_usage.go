...
// owner-scoped API key before hydration, preventing one shared group from
// consuming a whole runner cycle without merging different owners.
func (r *accountRepository) ListDueOllamaCloudUsageAccounts(ctx context.Context, now time.Time, limit int) ([]service.Account, error) {
	...
	rows, err := r.sql.QueryContext(ctx, `
		WITH candidates AS (
			SELECT id, owner_user_id, credentials ->> 'api_key' AS api_key,
			...
			SELECT id, owner_user_id, api_key, next_refresh_at,
