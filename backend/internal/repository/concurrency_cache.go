...
var (
	...
	acquireLiveLeaseScript = redis.NewScript(`
		...
		local replacingUser = tonumber(ARGV[5])
		...
		local userAllowance = 0
		if replacingUser == 1 then userAllowance = 1 end
		if accountMax > 0 and accountCount >= accountMax then return 0 end
