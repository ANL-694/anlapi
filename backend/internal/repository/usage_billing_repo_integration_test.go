...
func TestUsageBillingRepositoryApply_ConcurrentReplayDeduplicatesBalanceBilling(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := NewUsageBillingRepository(client, integrationDB)
	...
	apiKey := mustCreateApiKey(t, client, &service.APIKey{
		UserID: user.ID,
		Key:    "fixture-usage-billing-concurrent-" + uuid.NewString(),
		Name:   "billing-concurrent",
	})

	requestID := uuid.NewString()
	newCommand := func() *service.UsageBillingCommand {
		return &service.UsageBillingCommand{
			RequestID:          requestID,
			APIKeyID:           apiKey.ID,
			UserID:             user.ID,
			BalanceCost:        1.25,
			RequestFingerprint: "concurrent-replay-fingerprint",
		}
	}
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			defer wg.Done()
			<-start
			result, err := repo.Apply(ctx, newCommand())
			results <- result
			errors <- err
		}()
	}
...
func TestUsageBillingRepositoryApply_RequestFingerprintConflict(t *testing.T) {
	...
	apiKey := mustCreateApiKey(t, client, &service.APIKey{
		UserID: user.ID,
		Key:    "fixture-usage-billing-conflict-" + uuid.NewString(),
...
func TestUsageBillingRepositoryApply_UpdatesAccountQuota(t *testing.T) {
	...
	apiKey := mustCreateApiKey(t, client, &service.APIKey{
		UserID: user.ID,
		Key:    "fixture-usage-billing-account-" + uuid.NewString(),
...
func TestUsageBillingRepositoryApply_EnqueuesSchedulerOutboxOnQuotaCrossing(t *testing.T) {
	...
	newFixture := func(t *testing.T, extra map[string]any) (int64, int64) {
		...
		apiKey := mustCreateApiKey(t, client, &service.APIKey{
			UserID: user.ID,
			Key:    "fixture-usage-billing-outbox-" + uuid.NewString(),
			Name:   "billing-outbox",
		})
		account := mustCreateAccount(t, client, &service.Account{
...
func TestUsageBillingRepositoryApply_DeduplicatesAgainstArchivedKey(t *testing.T) {
	...
	apiKey := mustCreateApiKey(t, client, &service.APIKey{
		UserID: user.ID,
		Key:    "fixture-usage-billing-archive-" + uuid.NewString(),
