...
func (r *accountBatchTaskRepository) CreateTask(ctx context.Context, input service.CreateAccountBatchTaskInput) (*service.AccountBatchTask, error) {
	...
	`, []any{input.Scope, input.Operation, service.AccountBatchTaskStatusPending, len(input.AccountIDs), input.CreatedBy, input.OwnerUserID})
	if err != nil {
		return nil, err
	}
...
func (r *accountBatchTaskRepository) GetTask(ctx context.Context, id int64) (*service.AccountBatchTask, error) {
	task, err := queryAccountBatchTask(ctx, r.db, `
		SELECT id, scope, operation, status, total, processed, success, failed, created_by, owner_user_id,
			error_message, started_at, finished_at, created_at, updated_at
		FROM account_batch_tasks WHERE id = $1
	`, []any{id})
	if err != nil {
		return nil, err
	}
	task.Items, err = r.listItems(ctx, id, "")
	if err != nil {
		return nil, err
	}
	return task, nil
...
func (r *accountBatchTaskRepository) ClaimNextPendingTask(ctx context.Context, staleRunningAfterSeconds int64) (*service.AccountBatchTask, error) {
	...
	task, err := queryAccountBatchTask(ctx, r.db, `
		WITH next AS (
			SELECT id FROM account_batch_tasks
			WHERE status = $1 OR (status = $2 AND started_at IS NOT NULL AND started_at < NOW() - ($3 * interval '1 second'))
			...
			SET status = $1, started_at = NULL, updated_at = NOW()
			FROM next WHERE items.task_id = next.id AND items.status = $2
			RETURNING items.id
		)
		UPDATE account_batch_tasks AS tasks
		SET status = $4, started_at = COALESCE(tasks.started_at, NOW()), finished_at = NULL,
	...
	`, []any{service.AccountBatchTaskStatusPending, service.AccountBatchTaskStatusRunning, staleRunningAfterSeconds, service.AccountBatchTaskStatusRunning})
...
func (r *accountBatchTaskRepository) MarkItemRunning(ctx context.Context, itemID int64) error {
	_, err := r.db.ExecContext(ctx, `UPDATE account_batch_task_items SET status = $1, started_at = COALESCE(started_at, NOW()), updated_at = NOW() WHERE id = $2 AND status = $3`, service.AccountBatchTaskStatusRunning, itemID, service.AccountBatchTaskStatusPending)
	return err
}
...
func (r *accountBatchTaskRepository) MarkItemSucceeded(ctx context.Context, itemID int64, result map[string]any) error {
	...
	_, err = r.db.ExecContext(ctx, `UPDATE account_batch_task_items SET status = $1, result = $2::jsonb, error_message = NULL, finished_at = NOW(), updated_at = NOW() WHERE id = $3`, service.AccountBatchTaskStatusSucceeded, payload, itemID)
	return err
}

func (r *accountBatchTaskRepository) MarkItemFailed(ctx context.Context, itemID int64, errorMessage string) error {
	_, err := r.db.ExecContext(ctx, `UPDATE account_batch_task_items SET status = $1, error_message = $2, finished_at = NOW(), updated_at = NOW() WHERE id = $3`, service.AccountBatchTaskStatusFailed, strings.TrimSpace(errorMessage), itemID)
	return err
}

func (r *accountBatchTaskRepository) RefreshTaskProgress(ctx context.Context, taskID int64) (*service.AccountBatchTask, error) {
	return queryAccountBatchTask(ctx, r.db, `
		WITH counts AS (
			SELECT COUNT(*) FILTER (WHERE status IN ($2, $3, $4))::integer AS processed,
			COUNT(*) FILTER (WHERE status = $3)::integer AS success,
			COUNT(*) FILTER (WHERE status = $4)::integer AS failed
			FROM account_batch_task_items WHERE task_id = $1
		)
		UPDATE account_batch_tasks AS tasks
		SET processed = counts.processed, success = counts.success, failed = counts.failed, updated_at = NOW()
		FROM counts WHERE tasks.id = $1
		RETURNING tasks.id, tasks.scope, tasks.operation, tasks.status, tasks.total, tasks.processed, tasks.success, tasks.failed,
			tasks.created_by, tasks.owner_user_id, tasks.error_message, tasks.started_at, tasks.finished_at, tasks.created_at, tasks.updated_at
	`, []any{taskID, service.AccountBatchTaskStatusCanceled, service.AccountBatchTaskStatusSucceeded, service.AccountBatchTaskStatusFailed})
}

func (r *accountBatchTaskRepository) MarkTaskSucceeded(ctx context.Context, taskID int64) error {
	_, err := r.db.ExecContext(ctx, `UPDATE account_batch_tasks SET status = $1, error_message = NULL, finished_at = NOW(), updated_at = NOW() WHERE id = $2`, service.AccountBatchTaskStatusSucceeded, taskID)
	return err
}

func (r *accountBatchTaskRepository) MarkTaskFailed(ctx context.Context, taskID int64, errorMessage string) error {
	_, err := r.db.ExecContext(ctx, `UPDATE account_batch_tasks SET status = $1, error_message = $2, finished_at = NOW(), updated_at = NOW() WHERE id = $3`, service.AccountBatchTaskStatusFailed, strings.TrimSpace(errorMessage), taskID)
	return err
}
...
func (r *accountBatchTaskRepository) listItems(ctx context.Context, taskID int64, status string) ([]service.AccountBatchTaskItem, error) {
	query := `SELECT id, task_id, account_id, status, error_message, result, started_at, finished_at, created_at, updated_at FROM account_batch_task_items WHERE task_id = $1`
	args := []any{taskID}
...
func scanAccountBatchTask(rows *sql.Rows) (service.AccountBatchTask, error) {
	...
	if err := rows.Scan(&task.ID, &task.Scope, &task.Operation, &task.Status, &task.Total, &task.Processed, &task.Success, &task.Failed, &task.CreatedBy, &ownerUserID, &errorMessage, &startedAt, &finishedAt, &task.CreatedAt, &task.UpdatedAt); err != nil {
		return task, err
	}
...
func scanAccountBatchTaskItem(rows *sql.Rows) (service.AccountBatchTaskItem, error) {
	...
	var resultJSON []byte
	var startedAt, finishedAt sql.NullTime
	if err := rows.Scan(&item.ID, &item.TaskID, &item.AccountID, &item.Status, &errorMessage, &resultJSON, &startedAt, &finishedAt, &item.CreatedAt, &item.UpdatedAt); err != nil {
		return item, err
	}
	if errorMessage.Valid {
		v := errorMessage.String
