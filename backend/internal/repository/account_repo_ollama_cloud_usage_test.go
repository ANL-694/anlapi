...
func TestUpdateOllamaCloudUsageSnapshotRowsAffectedZeroIsIdentityConflict(t *testing.T) {
	...
	mock.ExpectExec(`(?s)`+regexp.QuoteMeta("UPDATE accounts")).
		WithArgs(sqlmock.AnyArg(), "key", sqlmock.AnyArg(), nil).
...
func expectOllamaCloudUsageGroupLock(
	...
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
		WithArgs(apiKey, account.ID, account.Platform, account.Type, string(credentials), proxyID, ollamaCloudUsageOwnerUserID(account)).
...
func TestSaveAndDeleteOllamaCloudUsageSessionKeepCiphertextOutOfSQL(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE accounts.*ollama_cloud_usage_session.*ollama_cloud_usage_auto_refresh.*ollama_cloud_usage_snapshot`).
		WithArgs(`{"ollama_cloud_usage_auto_refresh":true,"ollama_cloud_usage_session":"cipher:wos-session=browser-cookie-secret"}`, "key", sqlmock.AnyArg(), nil).
	...
	mock.ExpectExec(`(?s)UPDATE accounts.*ollama_cloud_usage_session.*ollama_cloud_usage_auto_refresh.*ollama_cloud_usage_snapshot`).
		WithArgs(`{}`, "key", sqlmock.AnyArg(), nil).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
...
func TestListOllamaCloudUsageGroupAccountsUsesOneStrictBatchQuery(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT DISTINCT accounts.id")).
	...
	second.OwnerUserID = &ownerUserID
	...
	require.Contains(t, query, "JOIN jsonb_to_recordset($1::jsonb) AS requested(api_key text, owner_user_id bigint)")
	require.Contains(t, query, "accounts.owner_user_id IS NOT DISTINCT FROM requested.owner_user_id")
	...
	require.NotContains(t, query, "credentials ->> 'api_key' = ANY")
	require.NotContains(t, query, "~*")
	require.Len(t, capturedArgs, 1)
	encodedIdentities, ok := capturedArgs[0].(string)
	require.True(t, ok)
	require.JSONEq(t, `[
		{"api_key":"key","owner_user_id":null},
		{"api_key":"key","owner_user_id":42}
	]`, encodedIdentities)
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestDisableOllamaCloudUsageAutoRefreshUsesGroupIdentityCAS(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE accounts.*ollama_cloud_usage_auto_refresh`).
		WithArgs(`{"ollama_cloud_usage_auto_refresh":false,"ollama_cloud_usage_session":"cipher:wos-session=secret"}`, "key", sqlmock.AnyArg(), nil).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
	repo := newAccountRepositoryWithSQL(client, nil, nil)
