...
func helperProcessCommand(ctx context.Context, mode string) *exec.Cmd {
	cmd := exec.CommandContext(ctx, os.Args[0], "-test.run=TestPgDumperHelperProcess", "--", mode)
	cmd.Env = append(os.Environ(), "GO_WANT_PGDUMPER_HELPER=1")
	return cmd
}

func TestPgDumperHelperProcess(t *testing.T) {
	if os.Getenv("GO_WANT_PGDUMPER_HELPER") != "1" {
		return
	}
	mode := ""
	if index := len(os.Args) - 1; index >= 0 {
		mode = os.Args[index]
	}
	switch mode {
	case "success":
		_, _ = os.Stdout.Write([]byte("backup-data"))
		os.Exit(0)
	case "partial-failure":
		_, _ = os.Stdout.Write([]byte("partial-backup"))
		os.Exit(7)
	case "true":
		os.Exit(0)
	default:
...
func TestPgDumperHoldsMigrationLockThroughReaderClose(t *testing.T) {
	...
	dumper, createdMock := newTestPgDumper(t, func(ctx context.Context, name string, args ...string) *exec.Cmd {
		...
		return helperProcessCommand(ctx, "success")
	})
	mock = createdMock
...
func TestPgDumperReleasesMigrationLockWhenStdoutPipeSetupFails(t *testing.T) {
	dumper, mock := newTestPgDumper(t, func(ctx context.Context, _ string, _ ...string) *exec.Cmd {
		cmd := helperProcessCommand(ctx, "true")
...
func TestPgDumperReleasesMigrationLockWhenProcessFails(t *testing.T) {
	dumper, mock := newTestPgDumper(t, func(ctx context.Context, _ string, _ ...string) *exec.Cmd {
		return helperProcessCommand(ctx, "partial-failure")
	})
	expectBackupMigrationLock(mock)
...
func TestPgDumperDoesNotStartProcessWhenMigrationLockFails(t *testing.T) {
	...
	dumper, mock := newTestPgDumper(t, func(ctx context.Context, _ string, _ ...string) *exec.Cmd {
		commandCreated = true
		return helperProcessCommand(ctx, "true")
	})
	mock.ExpectQuery(regexp.QuoteMeta("SELECT pg_try_advisory_lock($1)")).
		WithArgs(migrationsAdvisoryLockID).
