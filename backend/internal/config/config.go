...
type Config struct {
	...
	// Modules contains the raw per-module configuration subtree consumed by the
	// ANL module runtime.
...
// ReceiptCodeStorageConfig configures optional S3-compatible storage for
// payment receipt images. When disabled, images remain inline in PostgreSQL.
...
type postgresDSNTarget struct {
	Host     string
	Port     int
	Database string
}

// parsePostgresDSNTarget extracts identity fields used to reject an OAuth
// Vault DSN that points at the business database. Credentials and options are
// intentionally ignored; URL and libpq keyword forms are both accepted.
func parsePostgresDSNTarget(raw string) (postgresDSNTarget, error) {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return postgresDSNTarget{}, fmt.Errorf("postgres DSN must not be empty")
	}
	...
	if strings.Contains(raw, "://") {
		...
		if err != nil || (u.Scheme != "postgres" && u.Scheme != "postgresql") {
			return postgresDSNTarget{}, fmt.Errorf("invalid postgres DSN")
		}
		host = strings.ToLower(strings.TrimSpace(u.Hostname()))
		if u.Port() != "" {
			parsedPort, err := strconv.Atoi(u.Port())
			if err != nil || parsedPort < 1 || parsedPort > 65535 {
				return postgresDSNTarget{}, fmt.Errorf("invalid postgres DSN port")
			}
			port = parsedPort
		}
	...
	var err error
	database, err = url.PathUnescape(strings.TrimSpace(database))
	if err != nil || host == "" || database == "" {
		return postgresDSNTarget{}, fmt.Errorf("postgres DSN host and database are required")
	}
	return postgresDSNTarget{Host: host, Port: port, Database: database}, nil
}
...
type UpdateConfig struct {
	// Repository 是用于版本检查、更新和回滚的 GitHub owner/repository。
	// 留空时保持官方 Sub2API 仓库，避免升级现有部署时改变行为。
	Repository string `mapstructure:"repository"`
	// DockerImage 是手动 Docker 回滚命令使用的镜像名。
	// 留空时保持官方镜像，避免升级现有部署时改变行为。
...
func setDefaults() {
	...
	// OAuth credential vault. External mode uses a separate PostgreSQL database.
	viper.SetDefault("oauth_vault.mode", "legacy")
	viper.SetDefault("oauth_vault.dsn", "")
	viper.SetDefault("oauth_vault.encryption_key", "")
	viper.SetDefault("oauth_vault.allow_legacy_fallback", false)
	...
	// Private gateway (Hub-to-anlapi service authentication).
...
func (c *Config) Validate() error {
	...
	if err := c.PrivateGateway.Validate(); err != nil {
		return fmt.Errorf("private_gateway: %w", err)
	}
	...
	// OAuth Vault validation is structural only; connectivity and PostgreSQL
	// extension checks happen when the external vault is opened at startup.
	c.OAuthVault.Mode = strings.ToLower(strings.TrimSpace(c.OAuthVault.Mode))
	if c.OAuthVault.Mode == "" {
	...
	case "external":
		...
		key, err := hex.DecodeString(strings.TrimSpace(c.OAuthVault.EncryptionKey))
		if err != nil || len(key) != 32 {
			return fmt.Errorf("oauth_vault.encryption_key must be hexadecimal AES-256 key")
		}
		vaultTarget, err := parsePostgresDSNTarget(c.OAuthVault.DSN)
		if err != nil {
			return fmt.Errorf("oauth_vault.dsn invalid: %w", err)
		}
		businessTarget, err := parsePostgresDSNTarget(c.Database.DSN())
		if err != nil {
			return fmt.Errorf("database DSN invalid: %w", err)
		}
		if vaultTarget == businessTarget {
			return fmt.Errorf("oauth_vault.dsn must differ from the business database DSN")
		}
	default:
