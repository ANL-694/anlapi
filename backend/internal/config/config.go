...
type Config struct {
	...
	PrivateGateway          PrivateGatewayConfig          `mapstructure:"private_gateway"`
...
type UpdateConfig struct {
	...
	// GitHubToken 用于访问私有仓库的 Release；为空时保持匿名访问。
	// 该字段只从配置/环境变量读取，不应写入日志或接口响应。
	GitHubToken string `mapstructure:"github_token" json:"-"`
}
...
// MissingCredentialKeys 返回 IsConfigured 所缺的配置键名，便于定位纯环境变量部署中的遗漏。
...
type ServerConfig struct {
	...
	TrustedProxiesConfigured bool      `mapstructure:"-" json:"-" yaml:"-"`   // 是否显式设置 trusted_proxies
...
// ForwardedClientIPSettings 是请求进入时使用的客户端 IP 解析快照。
// 开关关闭时，所有安全路径只使用 Gin 的 trusted_proxies 链或直连 peer 地址。
...
type SecurityConfig struct {
	...
	// TrustForwardedIPForAPIKeyACL 是兼容模式。仅在管理员明确打开时，原始转发头才会接管 trusted_proxies。
...
// NormalizeForwardedClientIPHeaders validates, canonicalizes, de-duplicates,
// and bounds custom client-IP header names before they are persisted or used.
...
// ForwardedClientIPSettings returns a copy so runtime saves cannot mutate a
// request's selection after it has been captured by middleware.
...
// SetForwardedClientIPSettings atomically publishes settings for subsequent
// requests. Existing requests keep their middleware-captured snapshot.
...
// SetTrustForwardedIPForAPIKeyACL is kept for compatibility with callers that
// only manage the legacy switch. It retains the current custom-header list.
...
type BillingConfig struct {
	...
	// UserPlatformQuotaCacheTTLSeconds controls cached quota state lifetime.
	UserPlatformQuotaCacheTTLSeconds int `mapstructure:"user_platform_quota_cache_ttl_seconds"`
	// UserPlatformQuotaSentinelTTLSeconds controls negative-cache lifetime for users without limits.
...
type GatewayConfig struct {
	...
	// CodexImageGenerationBridgeEnabled: 是否为 Codex `/v1/responses` 自动注入生图工具和桥接指令。
...
type DatabaseConfig struct {
	...
	// UserPlatformQuotaFlusherEnabled enables Redis-to-PostgreSQL quota snapshots.
	UserPlatformQuotaFlusherEnabled bool `mapstructure:"user_platform_quota_flusher_enabled"`
	// UserPlatformQuotaFlushIntervalMs controls the snapshot interval.
	UserPlatformQuotaFlushIntervalMs int `mapstructure:"user_platform_quota_flush_interval_ms"`
	// UserPlatformQuotaFlushBatchSize controls the maximum rows per snapshot batch.
...
func configureConfigSource(setConfigFile, addConfigPath func(string)) {
	...
	addConfigPath("/etc/anlapi")
	addConfigPath("/etc/anl-api")
...
func setDefaults() {
	...
	viper.SetDefault("private_gateway.enabled", false)
	viper.SetDefault("private_gateway.service_id", "")
	viper.SetDefault("private_gateway.signing_key", "")
	viper.SetDefault("private_gateway.api_key_id", int64(0))
	...
	viper.SetDefault("log.service_name", "anlapi")
	...
	// Online update access. A token is required only when the release
	// repository is private; keep the anonymous path working for public repos.
	...
	viper.SetDefault("dashboard_cache.key_prefix", "anlapi:")
	...
	// Viper 只会把 AllKeys 中出现的键解码到结构体。为纯环境变量部署注册空默认值，
	// 否则 IMAGE_STORAGE_* 凭证会被读取后静默丢弃，导致异步生图始终不可用。
...
func (c *Config) Validate() error {
	if err := c.PrivateGateway.Validate(); err != nil {
		return err
	}
	...
	if c.Gateway.OpenAIScheduler.StickyEscapeErrorRate < 0 || c.Gateway.OpenAIScheduler.StickyEscapeErrorRate > 1 {
		return fmt.Errorf("gateway.openai_scheduler.sticky_escape_error_rate must be within [0,1]")
	}
