...
func (UsageLog) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("actual_cost").
			Default(0).
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"}),
		// PricingVersion is the effective price-card snapshot used for this request.
		// Historical rows remain NULL because the snapshot was introduced later.
		field.String("pricing_version").
			MaxLen(128).
			Optional().
			Nillable(),
