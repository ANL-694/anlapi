-- Account ownership, public sharing policies, and immutable settlement records.
-- This migration is additive and keeps the official 176 billing tables intact.
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'accounts_share_mode_check') THEN
        ALTER TABLE accounts ADD CONSTRAINT accounts_share_mode_check
            CHECK (share_mode IN ('private', 'public'));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'accounts_share_status_check') THEN
        ALTER TABLE accounts ADD CONSTRAINT accounts_share_status_check
...
CREATE TABLE IF NOT EXISTS user_balance_ledger (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT user_balance_ledger_direction_check CHECK (direction IN ('debit', 'credit')),
    CONSTRAINT user_balance_ledger_amount_check CHECK (amount >= 0)
);
...
CREATE TABLE IF NOT EXISTS account_share_settlement_entries (
    ...
    CONSTRAINT account_share_settlement_amounts_check
        CHECK (
            consumer_charge >= 0 AND account_cost >= 0 AND
            owner_share_ratio >= 0 AND owner_share_ratio <= 1 AND
            owner_credit >= 0 AND platform_fee >= 0 AND owner_credit <= consumer_charge
        )
);
