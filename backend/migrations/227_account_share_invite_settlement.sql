-- Extend account-share settlement with invitation snapshots.
-- Kept separate from 224 because 224 may already be applied in production.
...
SET inviter_bound_at = COALESCE(ua.inviter_bound_at, ua.created_at),
    invite_reward_expires_at = COALESCE(
        ...
        CASE
            ...
            THEN COALESCE(ua.inviter_bound_at, ua.created_at)
                 + make_interval(days => COALESCE((SELECT days FROM duration), 0))
            ELSE NULL
        END
    ),
    updated_at = NOW()
