...
> 本文档记录 anlapi 的开发环境、常见坑点和维护注意事项，供项目维护者参考。
...
| 数据库凭据 | 使用本机 `.env` 或隔离测试配置，不在仓库固定账号和密码 |
| 超级用户 | 使用本机 PostgreSQL 管理员账号，不在仓库记录密码 |
...
psql -U anlapi -h 127.0.0.1 -d anlapi -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER anlapi WITH PASSWORD '<new-local-password>';
   ```
...
```bash
# 连接数据库
psql -U anlapi -h 127.0.0.1 -d anlapi
...
# 执行 SQL 文件
psql -U anlapi -h 127.0.0.1 -d anlapi -f migration.sql
```
