...
**ANLAPI · AI API ゲートウェイと利用量管理プラットフォーム**
...
ANLAPI は、Sub2API 互換のゲートウェイコアを基盤に独立して保守している製品レイヤーです。ユーザー／管理者ワークスペース、アカウントプール、モデルと API キーのアクセス制御、利用量・請求、決済、明示的なエラー状態、デプロイ検証を提供し、互換ゲートウェイルートとプロバイダー連携を維持します。本プロジェクトは [Sub2API](https://github.com/Wei-Shaw/sub2api) を基盤としており、上流のライセンスと帰属表示に従います。
