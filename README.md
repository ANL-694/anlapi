...
- Go 网关服务与 Vue 3 控制台的前后端协同实现。
- 多上游账号、分组路由、失败切换、用量统计和计费链路。
- PostgreSQL 迁移、Redis 缓存、Docker Compose、systemd 与灾备脚本。
- OAuth、API Key、Passkey、管理员合规和请求审计等安全边界。
- 面向长连接、SSE、WebSocket、异步图像任务和 OpenAI 兼容协议的运行时处理。
- 面向可维护性的测试、类型检查、国际化审计和发布前安全检查。

上游来源、兼容接口和历史迁移标识会在代码与文档中保留，并在对应位置明确说明；这些标识不是当前产品名称。
