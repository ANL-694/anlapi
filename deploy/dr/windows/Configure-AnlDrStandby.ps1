...
$supervisor = Join-Path $AppRoot 'supervise-anlapi.ps1'
...
$legacyTaskNames = @('ANL-API', 'ANL-API-DR-Postgres', 'ANL-API-DR-Backup')
...
Unregister-ScheduledTask -TaskName 'ANLAPI' -Confirm:$false -ErrorAction SilentlyContinue
Register-ScheduledTask -TaskName 'ANLAPI' -Action $appAction -Principal $principal -Settings $persistentSettings -Description 'ANL API manual DR application task; no automatic trigger' | Out-Null
...
Register-ScheduledTask -TaskName 'ANLAPI-DR-Postgres' -Action $postgresAction -Trigger $startupTrigger -Principal $principal -Settings $persistentSettings -Description 'Keep the domestic ANL DR PostgreSQL subscriber available' -Force | Out-Null
Start-ScheduledTask -TaskName 'ANLAPI-DR-Postgres'
...
Register-ScheduledTask -TaskName 'ANLAPI-DR-Backup' -Action $backupAction -Trigger $backupTrigger -Principal $principal -Settings $backupSettings -Description 'Hourly ANL API domestic core database backup' -Force | Out-Null

$appTask = Get-ScheduledTask -TaskName 'ANLAPI'
if ($appTask.State -ne 'Ready') { throw ('ANLAPI task is not in standby state: ' + $appTask.State) }
