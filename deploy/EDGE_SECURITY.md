...
ANLAPI supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to ANLAPI. An explicit empty list
...
`gzip_types` and do not use `gzip_types *` for ANLAPI. The
...
ANLAPI `server.trusted_proxies` with the Caddy address/private subnet so the
