...
ANLAPI 是基于 Sub2API 兼容网关核心独立维护的产品层，提供用户端与管理端工作区、账号池与路由运营、模型/API Key 权限、用量与账单、支付、明确错误状态和部署验证，同时保留兼容网关路由与模型渠道能力。

架构说明见 [ANLAPI Architecture](docs/architecture/ANL_GATEWAY_ARCHITECTURE.md)，产品概览和简历项目描述见 [ANLAPI Product Overview](docs/product/ANL_GATEWAY_PRODUCT_OVERVIEW.md)。本项目基于 [Sub2API](https://github.com/Wei-Shaw/sub2api)，遵循上游许可证和署名要求。
